...
gulp.task("hygiene-watch", () =>
    gulp.watch(tsFilter, gulp.series("hygiene-modified"))
);

gulp.task("hygiene", done =>
    run(
        { mode: "all", skipFormatCheck: true, skipIndentationCheck: true },
        done
    )
);
...
gulp.task("hygiene-watch-branch", () =>
    gulp.watch(tsFilter, gulp.series("hygiene-branch"))
);
...
gulp.task("clean:cleanExceptTests", () =>
    del(["clean:vsix", "out/client", "out/datascience-ui", "out/server"])
);
...
gulp.task(
    "clean",
    gulp.parallel("output:clean", "cover:clean", "clean:vsix", "clean:out")
);
...
gulp.task("check-datascience-dependencies", () =>
...
gulp.task("webpack", async () => {
    ...
    await buildWebPack("sourceMaps", [
        "--config",
        "./build/webpack/webpack.extension.sourceMaps.config.js"
    ]);
    await buildWebPack("extension", [
        "--config",
        "./build/webpack/webpack.extension.config.js"
    ]);
    await buildWebPack("debugAdapter", [
        "--config",
        ...
        .filter(
            item =>
                allowedWarnings.findIndex(allowedWarning =>
                    item.startsWith(allowedWarning)
                ) == -1
        );
    ...
    await Promise.all(
        requirements.map(async requirement => {
            const success = await spawnAsync(
                process.env.CI_PYTHON_PATH || "python3",
                args.concat(requirement)
            )
                .then(() => true)
                .catch(ex => {
                    console.error(
                        "Failed to install Python Libs using 'python3'",
                        ex
                    );
                    return false;
                });
            if (!success) {
                console.info(
                    "Failed to install Python Libs using 'python3', attempting to install using 'python'"
                );
                await spawnAsync("python", args.concat(requirement)).catch(ex =>
                    console.error(
                        "Failed to install Python Libs using 'python'",
                        ex
                    )
                );
            }
...
async function checkDatascienceDependencies() {
    ...
    const existingModulesList = JSON.parse(
        await fsExtra
            .readFile(existingModulesFile)
    ...
    const contents = await fsExtra
        .readFile(statsOutput)
        .then(data => data.toString());
    ...
    if (
        modulesInPackageLock.some(
            dependency =>
                dependency.indexOf("/") !== dependency.lastIndexOf("/")
        )
    ) {
        throwAndLogError(
            "Dependencies detected with more than one '/', please update this script."
        );
    }
    json.chunks[0].modules.forEach(m => {
        ...
        moduleName1 = moduleName1.endsWith("!.")
            ? moduleName1.substring(0, moduleName1.length - 2)
            : moduleName1;
        const moduleName2 = `${nameWithoutNodeModules.split("/")[1]}/${
            nameWithoutNodeModules.split("/")[2]
        }`;

        const matchedModules = modulesInPackageLock.filter(
            dependency =>
                dependency === moduleName2 || dependency === moduleName1
        );
        switch (matchedModules.length) {
            ...
            default: {
                throwAndLogError(
                    `Exact Dependency not found in package-lock.json, Dependency = '${name}'`
                );
            }
        }
    ...
    if (newModules.size > 0) {
        errorMessages.push(
            `Add the untracked dependencies '${Array.from(
                newModules.values()
            ).join(", ")}' to ${existingModulesFileName}`
        );
    }
...
function buildDebugAdapterCoverage(done) {
    const matches = glob.sync(
        path.join(__dirname, "debug_coverage*/coverage.json")
    );
    matches.forEach(coverageFile => {
        const finalCoverageFile = path.join(
            path.dirname(coverageFile),
            "coverage-final-upload.json"
        );
        const remappedCollector = remapIstanbul.remap(
...
function getTsProject(options) {
    const tsOptions =
        options.mode === "compile"
            ? undefined
            : { strict: true, noImplicitAny: false, noImplicitThis: false };
    const mode = tsOptions && tsOptions.mode ? tsOptions.mode : "";
    return tsProjectMap[mode]
        ? tsProjectMap[mode]
        : (tsProjectMap[mode] = ts.createProject("tsconfig.json", tsOptions));
}
...
function getLinter(options) {
    configuration = configuration
        ? configuration
        : tslint.Configuration.findConfiguration(null, ".");
...
const hygiene = (options, done) => {
    ...
    const fileListToProcess =
        options.mode === "compile" ? undefined : getFileListToProcess(options);
    if (
            ...
            .then(result => {
                if (result.error) {
                    ...
                    if (message.startsWith(__dirname)) {
                        ...
                        message = message.startsWith(path.sep)
                            ? message.substr(1)
                            : message;
        ...
        return (
            failures
                .map(failure => {
                    ...
                    const line = position.lineAndCharacter
                        ? position.lineAndCharacter.line
                        : position.line;
                    ...
                    const message = `ERROR: (${failure.ruleName}) ${relative(
                        __dirname,
                        name
                    )}[${line + 1}, ${character + 1}]: ${failure.failure}`;
    ...
    const files =
        options.mode === "compile"
            ? tsProject.src()
            : getFilesToProcess(fileListToProcess);
        ...
        .pipe(
            sourcemaps.mapSources(function(sourcePath, file) {
                ...
                const qualifiedSourcePath = path
                    ...
                    .replace("out\\", "src\\");
                if (
                    !fs.existsSync(path.join(qualifiedSourcePath, tsFileName))
                ) {
                    const tsxFileName = path
                        .basename(file.path)
                        .replace(/js$/, "tsx");
                    if (
                        !fs.existsSync(
                            path.join(qualifiedSourcePath, tsxFileName)
                        )
                    ) {
                        console.error(
                            `ERROR: (source-maps) ${
                                file.path
                            }[1,1]: Source file not found`
                        );
                    } else {
                ...
                return path.join(
                    path.relative(path.dirname(file.path), qualifiedSourcePath),
                    tsFileName
                );
            })
        )
        ...
        .pipe(
            es.through(null, function() {
                if (errorCount > 0) {
                    ...
                    console.log(
                        colors.green(
                            `Hygiene passed with 0 errors 👍 (completed in ${new Date().getTime() -
                                started}ms).`
                        )
                    );
                }
...
function run(options, done) {
    ...
    options.exitOnError =
        typeof options.exitOnError === "undefined" ? isCI : options.exitOnError;
...
function getModifiedFilesSync() {
    if (isCI) {
        const isAzurePR =
            getAzureDevOpsVarValue("System.PullRequest.SourceBranch") !==
            undefined;
        const isTravisPR =
        ...
        const targetBranch =
            ...
            getAzureDevOpsVarValue("System.PullRequest.TargetBranch");
        ...
        const repo =
            ...
            getAzureDevOpsVarValue("Build.Repository.Name");
        ...
        try {
            cp.execSync(
                `git remote set-branches --add ${originOrUpstream} master`,
