    ...
    gulp.series(
        (done) => {
            if (process.env.VSC_JUPYTER_FORCE_ANALYZER) {
                process.env.XVSC_JUPYTER_FORCE_ANALYZER = '1';
            }
            process.env.VSC_JUPYTER_FORCE_ANALYZER = '1';
            done();
        },
        'compile-webviews',
        () => checkDatascienceDependencies(),
        (done) => {
            if (!process.env.XVSC_JUPYTER_FORCE_ANALYZER) {
                delete process.env.VSC_JUPYTER_FORCE_ANALYZER;
            }
            done();
        }
    )
