...
export function getBinaryFromKnownPaths(): string | undefined {
...
function getPyenvBinary(): string | undefined {
...
function getVirtualEnvEnvironment(
    executable: string,
    folderPath: string,
    manager: EnvManager | undefined,
): PythonEnvironment | undefined {
    const pyenvCfg = findAndParsePyvenvCfg(executable);
    if (pyenvCfg) {
...
export function listPyenvEnvironments(manager: EnvManager | undefined): PythonEnvironment[] | undefined {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (stats.isDirectory()) {
                const executable = findPythonBinaryPath(folderPath);
                if (executable) {
