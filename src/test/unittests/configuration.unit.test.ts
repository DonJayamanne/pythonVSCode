        ...
        suite(productName.name, () => {
            ...
            setup(() => {
                ...
                configurationService
                    .setup(c => c.getSettings(workspaceUri))
                ...
                serviceContainer
                    .setup(c =>
                        c.get(
                            ...
                            typeMoq.It.isValue(TEST_OUTPUT_CHANNEL)
                        )
                    )
                    .returns(() => outputChannel.object);
                serviceContainer
                    .setup(c => c.get(typeMoq.It.isValue(IInstaller)))
                    .returns(() => installer.object);
                serviceContainer
                    .setup(c =>
                        c.get(typeMoq.It.isValue(IConfigurationService))
                    )
                ...
                serviceContainer
                    .setup(c =>
                        c.get(
                            typeMoq.It.isValue(ITestConfigurationManagerFactory)
                        )
                    )
            ...
            test("Enable Test when setting unitTest.promptToConfigure is enabled", async () => {
                ...
                workspaceConfig
                    .setup(w =>
                        w.get(typeMoq.It.isValue("unitTest.promptToConfigure"))
                    )
                    .returns(() => true)
                    .verifiable(typeMoq.Times.once());

                await testConfigService.target.enableTest(
                    workspaceUri,
                    product
                );
            ...
            test("Enable Test when setting unitTest.promptToConfigure is disabled", async () => {
                ...
                workspaceConfig
                    .setup(w =>
                        w.update(
                            typeMoq.It.isValue("unitTest.promptToConfigure"),
            ...
            test("Enable Test when setting unitTest.promptToConfigure is disabled and fail to update the settings", async () => {
                ...
                const promise = testConfigService.target.enableTest(
                    workspaceUri,
                    product
                );
            ...
            test("Select Test runner displays 3 items", async () => {
                ...
                appShell
                    .setup(s =>
                        s.showQuickPick(
                            typeMoq.It.isAny(),
                            typeMoq.It.isObjectWith({ placeHolder })
                        )
                    )
            ...
            test("Ensure selected item is returned", async () => {
                ...
                const selectedItem = await testConfigService.target.selectTestRunner(
                    placeHolder
                );
            ...
            test("Prompt to enable a test if a test framework is not enabled", async () => {
                unitTestSettings
                    .setup(u => u.pyTestEnabled)
                    .returns(() => false);
                unitTestSettings
                    .setup(u => u.unittestEnabled)
                    .returns(() => false);
                unitTestSettings
                    .setup(u => u.nosetestsEnabled)
                    .returns(() => false);
                ...
                try {
                    await testConfigService.target.displayTestFrameworkError(
                        workspaceUri
                    );
                } catch {
                ...
                expect(exceptionThrown).to.be.equal(
                    true,
            ...
            test("Prompt to select a test if a test framework is not enabled", async () => {
                ...
                expect(selectTestRunnerInvoked).to.be.equal(
                    true,
                    "Method not invoked"
                );
                expect(exceptionThrown).to.be.equal(
            ...
            test("Configure selected test framework and disable others", async () => {
                ...
                testConfigService
                    .setup(t =>
                        t.enableTest(
                            typeMoq.It.isValue(workspaceUri),
                ...
                expect(selectTestRunnerInvoked).to.be.equal(
                    true,
                    "Select Test Runner not invoked"
                );
                expect(enableTestInvoked).to.be.equal(
                    true,
                    "Enable Test not invoked"
                );
                appShell.verifyAll();
                factory.verifyAll();
                configMgr.verifyAll();
