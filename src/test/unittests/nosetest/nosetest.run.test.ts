...
suite("Unit Tests - nose - run against actual python process", () => {
        ...
        procService.onExecObservable((file, args, options, callback) => {
            if (args.indexOf("--collect-only") >= 0) {
                callback({
                    out: fs
                        .readFileSync(
                            path.join(UNITTEST_TEST_FILES_PATH, outputFileName),
                            "utf8"
                        )
                        .replace(
    ...
    test("Run Specific Test File", async () => {
        ...
        const testFileToRun = tests.testFiles.find(t =>
            t.fullPath.endsWith("test_root.py")
        );
    ...
    test("Run Specific Test Suite", async () => {
        ...
        const testSuiteToRun = tests.testSuites.find(
            s => s.xmlClassName === "test_root.Test_Root_test1"
        );
    ...
    test("Run Specific Test Function", async () => {
        ...
        const testFnToRun = tests.testFunctions.find(
            f => f.xmlClassName === "test_root.Test_Root_test1"
        );
