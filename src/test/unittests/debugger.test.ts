...
suite("Unit Tests - debugging", () => {
    ...
    function initializeDI() {
        ...
        ioc.serviceManager.add<IArgumentsHelper>(
            IArgumentsHelper,
            ArgumentsHelper
        );
        ioc.serviceManager.add<ITestRunner>(ITestRunner, TestRunner);
        ioc.serviceManager.add<IXUnitParser>(IXUnitParser, XUnitParser);
        ioc.serviceManager.add<IUnitTestHelper>(
            IUnitTestHelper,
            UnitTestHelper
        );
        ioc.serviceManager.add<IArgumentsService>(
            IArgumentsService,
        ...
        ioc.serviceManager.add<ITestManagerRunner>(
            ITestManagerRunner,
            PytestManagerRunner,
            PYTEST_PROVIDER
        );
        ...
        ioc.serviceManager.addSingleton<ITestDebugLauncher>(
            ITestDebugLauncher,
            MockDebugLauncher
        );
        ioc.serviceManager.addSingleton<ITestMessageService>(
            ITestMessageService,
            TestMessageService,
            PYTEST_PROVIDER
        );
    ...
    async function testStartingDebugger(testProvider: TestProvider) {
        const testManager = ioc.serviceContainer.get<ITestManagerFactory>(
            ITestManagerFactory
        )(testProvider, rootWorkspaceUri!, testFilesPath);
        const mockDebugLauncher = ioc.serviceContainer.get<MockDebugLauncher>(
            ITestDebugLauncher
        );
        ...
        const runningPromise = testManager.runTest(
            CommandSource.commandPalette,
            { testFunction },
            false,
            true
        ...
        runningPromise
            .then(() =>
                deferred.reject("Debugger stopped when it shouldn't have")
            )
