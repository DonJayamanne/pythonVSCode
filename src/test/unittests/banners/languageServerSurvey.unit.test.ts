...
suite("Language Server Survey Banner", () => {
    ...
    const message =
        "Can you please take 2 minutes to tell us how the Experimental Debugger is working for you?";
    ...
    test("Is debugger enabled upon creation?", () => {
        ...
        expect(testBanner.enabled).to.be.equal(
            true,
            "Sampling 100/100 should always enable the banner."
        );
    });
    ...
    test("shouldShowBanner must return false when Banner is implicitly disabled by sampling", () => {
        ...
        expect(testBanner.enabled).to.be.equal(
            false,
            "We implicitly disabled the banner, it should never show."
        );
    });
    ...
    languageServerVersions.forEach(async (languageServerVersion: string) => {
        test(`Survey URL is as expected for Language Server version '${languageServerVersion}'.`, async () => {
            ...
            expect(receivedUri).is.equal(
                expectedUri,
                "Uri given to launch mock is incorrect."
            );
...
function preparePopup(
    ...
    const myfactory: typemoq.IMock<
        IPersistentStateFactory
    > = typemoq.Mock.ofType<IPersistentStateFactory>();
    const enabledValState: typemoq.IMock<
        IPersistentState<boolean>
    > = typemoq.Mock.ofType<IPersistentState<boolean>>();
    const attemptCountState: typemoq.IMock<
        IPersistentState<number>
    > = typemoq.Mock.ofType<IPersistentState<number>>();
    const completionCountState: typemoq.IMock<
        IPersistentState<number>
    > = typemoq.Mock.ofType<IPersistentState<number>>();
    enabledValState
    ...
    myfactory
        .setup(a =>
            a.createGlobalPersistentState(
                typemoq.It.isValue(LSSurveyStateKeys.ShowBanner),
