...
suite("Propose New Language Server Banner", () => {
    ...
    test("Is debugger enabled upon creation?", () => {
        ...
        const testBanner: ProposeLanguageServerBanner = preparePopup(
            enabledValue,
            100,
            appShell.object,
            config.object
...
function preparePopup(
    ...
    const val: typemoq.IMock<IPersistentState<boolean>> = typemoq.Mock.ofType<
    ...
    myfactory
        .setup(a =>
            a.createGlobalPersistentState(
                typemoq.It.isValue(ProposeLSStateKeys.ShowBanner),
