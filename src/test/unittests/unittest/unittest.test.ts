...
import { isPythonVersion, rootWorkspaceUri, updateSetting } from "../../common";
...
suite("Unit Tests - unittest - discovery against actual python process", () => {
    ...
    test("Discover Tests (single test file)", async () => {
        ...
        assert.equal(
            tests.testFiles.some(
                t =>
                    t.name === "test_one.py" &&
                    t.nameToRun === "test_one.Test_test1.test_A"
            ),
            true,
            "Test File not found"
        );
    ...
    test("Discover Tests (many test files, subdir included)", async () => {
        ...
        assert.equal(
            tests.testFiles.some(
                t =>
                    t.name === "test_two.py" &&
                    t.nameToRun === "test_two.Test_test2.test_2A"
            ),
            true,
        ...
        assert.equal(
            tests.testFiles.some(
                t =>
                    t.name === "test_three.py" &&
                    t.nameToRun === "more_tests.test_three.Test_test3.test_3A"
            ),
            true,
    ...
    test("Run single test", async () => {
        ...
        const testsDiscovered: Tests = await testManager.discoverTests(
            CommandSource.ui,
            true,
            true
        );
        ...
        assert.notEqual(
            testFile,
            undefined,
            "No test file suffixed with _3A in test files."
        );
        assert.equal(
            testFile!.suites.length,
            1,
            "Expected only 1 test suite in test file three."
        );
        const testFunc:
            | TestFunction
            | undefined = testFile!.suites[0].functions.find(
            (value: TestFunction) => value.name === "test_3A"
        );
        assert.notEqual(
            testFunc,
            undefined,
            "No test in file test_three.py named test_3A"
        );
        const testsToRun: TestsToRun = {
            testFunction: [testFunc!]
        };
        const testRunResult: Tests = await testManager.runTest(
            CommandSource.ui,
            testsToRun
        );
        assert.equal(
        ...
        assert.equal(
            testRunResult.summary.errors,
            0,
            "Unexpected: Test file ran with errors."
        );
        assert.equal(
            testRunResult.summary.failures,
            0,
            "Unexpected: Test has failed during test run."
        );
        assert.equal(
            testRunResult.summary.passed,
            1,
            `Only one test should have passed during our test run. Instead, ${
                testRunResult.summary.passed
            } passed.`
        );
        assert.equal(
            testRunResult.summary.skipped,
            0,
            `Expected to have skipped 0 tests during this test-run. Instead, ${
                testRunResult.summary.skipped
            } where skipped.`
        );
    });
    ...
    test("Ensure correct test count for running a set of tests multiple times", async function() {
        ...
        const testManager = factory(
            "unittest",
            rootWorkspaceUri!,
            UNITTEST_COUNTS_TEST_FILE_PATH
        );
        const testsDiscovered: Tests = await testManager.discoverTests(
            CommandSource.ui,
            true,
        ...
        assert.equal(
            testsFile!.suites.length,
            1,
            "Expected only 1 test suite in counter test file."
        );
        const testsToRun: TestsToRun = {
        ...
        let testRunResult: Tests = await testManager.runTest(
            CommandSource.ui,
            testsToRun
        );
        assert.equal(
    ...
    test("Re-run failed tests results in the correct number of tests counted", async function() {
        ...
        testRunResult = await testManager.runTest(
            CommandSource.ui,
            testsToRun,
            true
        );
