...
suite("Unit Tests - unittest - run with mocked process output", () => {
    ...
    async function ignoreTestLauncher() {
        ...
        procService.onExecObservable((file, args, options, callback) => {
            if (
                args.length > 1 &&
                args[0].endsWith("visualstudio_py_testlauncher.py")
            ) {
    ...
    function injectTestSocketServerResults(results: {}[]) {
        ...
        const socketServer = ioc.serviceContainer.get<MockUnitTestSocketServer>(
            IUnitTestSocketServer
        );
        socketServer.reset();
    ...
    test("Run Specific Test File", async () => {
        ...
        const testFileToTest = tests.testFiles.find(
            f => f.name === "test_unittest_one.py"
        )!;
    ...
    test("Run Specific Test Suite", async () => {
        ...
        // tslint:disable-next-line:no-non-null-assertion
        const testSuiteToTest = tests.testSuites.find(
            s => s.testSuite.name === "Test_test_one_1"
        )!.testSuite;
