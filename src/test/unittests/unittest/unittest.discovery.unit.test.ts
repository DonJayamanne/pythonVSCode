...
suite("Unit Tests - Unittest - Discovery", () => {
    ...
    setup(() => {
        ...
        serviceContainer
            .setup(s =>
                s.get(typeMoq.It.isValue(IArgumentsHelper), typeMoq.It.isAny())
            )
            .returns(() => argsHelper.object);
        serviceContainer
            .setup(s =>
                s.get(typeMoq.It.isValue(ITestRunner), typeMoq.It.isAny())
            )
    ...
    test("Ensure discovery is invoked with the right args with start directory defined with -s", async () => {
        ...
        argsHelper
            .setup(a =>
                a.getOptionValues(
                    typeMoq.It.isValue(args),
                    typeMoq.It.isValue("-s")
                )
            )
            .returns(() => dir)
        ...
        runner
            .setup(r =>
                r.run(typeMoq.It.isValue(UNITTEST_PROVIDER), typeMoq.It.isAny())
            )
        ...
        testParser
            .setup(t =>
                t.parse(typeMoq.It.isValue(runOutput), typeMoq.It.isAny())
            )
            .returns(() => tests)
            .verifiable(typeMoq.Times.once());
    ...
    test("Ensure discovery is invoked with the right args with start directory defined with --start-directory", async () => {
        ...
        argsHelper
            .setup(a =>
                a.getOptionValues(
                    typeMoq.It.isValue(args),
                    typeMoq.It.isValue("--start-directory")
                )
            )
            .returns(() => dir)
            .verifiable(typeMoq.Times.once());
    ...
    test("Ensure discovery resolves test suites in n-depth directories", async () => {
        const testHelper: TestsHelper = new TestsHelper(
        ...
        tests.testFunctions.forEach(fn => {
            if (fn.parentTestSuite) {
                const testPrefix: boolean = fn.testFunction.nameToRun.startsWith(
                    fn.parentTestSuite.nameToRun
                );
                expect(testPrefix).to.equal(
                    true,
                    [
                        `function ${fn.testFunction.name} has a parent suite ${
                            fn.parentTestSuite.name
                        }, `,
                        `but the parent suite 'nameToRun' (${
                            fn.parentTestSuite.nameToRun
                        }) isn't the `,
                        `prefix to the functions 'nameToRun' (${
                            fn.testFunction.nameToRun
                        })`
                    ].join("")
                );
            }
    ...
    test("Ensure discovery resolves test files in n-depth directories", async () => {
        ...
        tests.testFunctions.forEach(fn => {
            if (fn.parentTestSuite) {
                const testPrefix: boolean = fn.testFunction.nameToRun.startsWith(
                    fn.parentTestFile.nameToRun
                );
                expect(testPrefix).to.equal(
                    true,
                    [
                        `function ${fn.testFunction.name} was found in file ${
                            fn.parentTestFile.name
                        }, `,
                        `but the parent file 'nameToRun' (${
                            fn.parentTestFile.nameToRun
                        }) isn't the `,
                        `prefix to the functions 'nameToRun' (${
                            fn.testFunction.nameToRun
