...
Run the command `python <ExtensionDir>/pythonFiles/testing_tools/run_adapter.py discover pytest --collect-only -s --cache-clear` to get the JSON output.
*/
...
suite('Unit Tests - pytest - discovery with mocked process output', () => {
    ...
    async function injectTestDiscoveryOutput(output: string) {
        ...
        procService.onExec((file, args, options, callback) => {
    ...
    test('Discover Tests (single test file)', async () => {
        await injectTestDiscoveryOutput(JSON.stringify([
            {
                rootid: '.',
    ...
    test('Discover Tests (pattern = test_)', async () => {
        ...
        assert.equal(tests.testFiles.some(t => t.name === 'test_foreign_nested_tests.py'), true, 'Test File not found');
        assert.equal(tests.testFiles.some(t => t.name === 'test_unittest_one.py'), true, 'Test File not found');
        assert.equal(tests.testFiles.some(t => t.name === 'test_unittest_two.py'), true, 'Test File not found');
        assert.equal(tests.testFiles.some(t => t.name === 'unittest_three_test.py'), true, 'Test File not found');
        assert.equal(tests.testFiles.some(t => t.name === 'test_pytest.py'), true, 'Test File not found');
        assert.equal(tests.testFiles.some(t => t.name === 'test_another_pytest.py'), true, 'Test File not found');
        assert.equal(tests.testFiles.some(t => t.name === 'test_root.py'), true, 'Test File not found');
    });
