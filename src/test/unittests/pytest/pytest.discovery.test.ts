...
const unitTestTestFilesCwdPath = path.join(
    EXTENSION_ROOT_DIR,
    "src",
    "test",
    "pythonFiles",
...
suite("Unit Tests - pytest - discovery with mocked process output", () => {
    ...
    test("Discover Tests (single test file)", async () => {
        ...
        assert.equal(
            diagnosticCollectionUris.length,
            0,
            "Should not have diagnostics yet"
        );
        assert.equal(
    ...
    test("Discover Tests (pattern = test_)", async () => {
        ...
        assert.equal(
            tests.testFiles.some(
                t =>
                    t.name === "tests/test_unittest_one.py" &&
                    t.nameToRun === t.name
            ),
            true,
            "Test File not found"
        ...
        assert.equal(
            tests.testFiles.some(
                t =>
                    t.name === "tests/test_unittest_two.py" &&
                    t.nameToRun === t.name
            ),
            true,
            "Test File not found"
        ...
        assert.equal(
            tests.testFiles.some(
                t =>
                    t.name === "tests/unittest_three_test.py" &&
                    t.nameToRun === t.name
            ),
            true,
            "Test File not found"
        ...
        assert.equal(
            tests.testFiles.some(
                t => t.name === "tests/test_pytest.py" && t.nameToRun === t.name
            ),
            true,
            "Test File not found"
        );
        assert.equal(
            tests.testFiles.some(
                t =>
                    t.name === "tests/test_another_pytest.py" &&
                    t.nameToRun === t.name
            ),
            true,
            "Test File not found"
    ...
    test("Discover Tests (with config)", async () => {
        ...
        const testManager = factory(
            "pytest",
            rootWorkspaceUri!,
            UNITTEST_TEST_FILES_PATH_WITH_CONFIGS
        );
        const tests = await testManager.discoverTests(
            CommandSource.ui,
            true,
        ...
        assert.equal(
            tests.testFiles.some(
                t =>
                    t.name === "other/test_unittest_one.py" &&
                    t.nameToRun === t.name
            ),
            true,
            "Test File not found"
        ...
        assert.equal(
            tests.testFiles.some(
                t => t.name === "other/test_pytest.py" && t.nameToRun === t.name
            ),
            true,
            "Test File not found"
        );
    ...
    test("Setting cwd should return tests", async () => {
        ...
        assert.equal(
            tests.testFolders.length,
            1,
            "Incorrect number of test folders"
        );
        assert.equal(
            tests.testFunctions.length,
            1,
