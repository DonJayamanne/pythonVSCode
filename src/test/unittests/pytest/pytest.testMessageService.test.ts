...
async function testMessageProperties(
    message: IPythonUnitTestMessage,
    expectedMessage: IPythonUnitTestMessage,
    ...
    assert.equal(
        ...
        "IPythonUnitTestMessage code"
    );
    assert.equal(
        message.message,
        expectedMessage.message,
        "IPythonUnitTestMessage message"
    );
    assert.equal(
        message.severity,
        expectedMessage.severity,
        "IPythonUnitTestMessage severity"
    );
    assert.equal(
        message.provider,
        expectedMessage.provider,
        "IPythonUnitTestMessage provider"
    );
    assert.isNumber(message.testTime, "IPythonUnitTestMessage testTime");
    assert.equal(
        message.status,
        expectedMessage.status,
        "IPythonUnitTestMessage status"
    );
    assert.equal(
        message.testFilePath,
        expectedMessage.testFilePath,
        "IPythonUnitTestMessage testFilePath"
    );
    if (status !== TestStatus.Pass) {
        assert.equal(
            message.locationStack![0].lineText,
            expectedMessage.locationStack![0].lineText,
            "IPythonUnitTestMessage line text"
        );
        assert.equal(
            message.locationStack![0].location.uri.fsPath,
            expectedMessage.locationStack![0].location.uri.fsPath,
            "IPythonUnitTestMessage locationStack fsPath"
        );
    ...
    filterdTestScenarios.forEach(scenario => {
        suite(scenario.scenarioName, async () => {
            ...
            suiteSetup(async () => {
                ...
                const parsedTests: Tests = parser.parse(
                    discoveryOutput,
                    options
                );
                ...
                const testMessageService = new TestMessageService(
                    ioc.serviceContainer
                );
                testMessages = await testMessageService.getFilteredTestMessages(
                    UNITTEST_TEST_FILES_PATH,
                    parsedTests
                );
            });
            ...
            scenario.testDetails!.forEach(td => {
                suite(td.nameToRun, () => {
                    ...
                    suiteSetup(async () => {
                        ...
                        if (
                            td.status === TestStatus.Error ||
                            td.status === TestStatus.Fail
                        ) {
                            expectedSeverity =
                                PythonUnitTestMessageSeverity.Error;
                        } else if (td.status === TestStatus.Skipped) {
                            expectedSeverity =
                                PythonUnitTestMessageSeverity.Skip;
                        } else {
                            expectedSeverity =
                                PythonUnitTestMessageSeverity.Pass;
                        }
                        const expectedLocationStack = await getExpectedLocationStackFromTestDetails(
                            td
                        );
                        expectedMessage = {
                            code: td.nameToRun,
                        ...
                        testMessage = testMessages.find(
                            tm => tm.code === td.nameToRun
                        )!;
                    });
                    test("Message", async () => {
