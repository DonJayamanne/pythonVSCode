...
import {
    ...
    testScenarios
} from "./pytest_run_tests_data";
import { EnvironmentActivationService } from "../../../client/interpreter/activation/service";
import { IEnvironmentActivationService } from "../../../client/interpreter/activation/types";

const UNITTEST_TEST_FILES_PATH = path.join(
    EXTENSION_ROOT_DIR,
    "src",
    "test",
    "pythonFiles",
 ...
 */
async function getScenarioTestsToRun(
    scenario: ITestScenarioDetails,
    tests: Tests
): Promise<TestsToRun> {
    const generateTestsToRun =
        scenario.testSuiteIndex || scenario.testFunctionIndex;
    if (scenario.testsToRun === undefined && generateTestsToRun) {
        ...
        if (scenario.testSuiteIndex) {
            scenario.testsToRun.testSuite!.push(
                tests.testSuites[scenario.testSuiteIndex].testSuite
            );
        }
        if (scenario.testFunctionIndex) {
            scenario.testsToRun.testFunction!.push(
                tests.testSuites[scenario.testFunctionIndex].testSuite
            );
        }
    }
 ...
 * @param failedRun Whether or not the current test run is for failed tests from a previous run.
 */
function getExpectedSummaryCount(
    testDetails: ITestDetails[],
    failedRun: boolean
): IResultsSummaryCount {
 ...
 */
function getRelevantTestDetailsForFile(
    testDetails: ITestDetails[],
    fileName: string
): ITestDetails[] {
...
function getIssueCountFromRelevantTestDetails(
    ...
    const relevantIssueDetails = testDetails.filter(td => {
        return (
            td.status !== TestStatus.Pass && !(failedRun && td.passOnFailedRun)
        );
    });
    ...
    return (
        relevantIssueDetails.length +
        (failedRun ? skippedTestDetails.length : 0)
    );
}
 ...
 * @param testDetails All the test details for a scenario.
 */
function getUniqueIssueFilesFromTestDetails(
    testDetails: ITestDetails[]
): string[] {
    return testDetails.reduce<string[]>((filtered, issue) => {
        if (
            filtered.indexOf(issue.fileName) === -1 &&
            issue.fileName !== undefined
        ) {
 ...
 */
function getRelevantSkippedIssuesFromTestDetailsForFile(
    testDetails: ITestDetails[],
    fileName: string
): ITestDetails[] {
    return testDetails.filter(td => {
...
function getTestFuncFromResultsByTestFileAndName(
    ...
    return results.testFunctions.find(test => {
        return (
            fileSystem.arePathsSame(
                vscode.Uri.file(test.parentTestFile.fullPath).fsPath,
                testFileUri.fsPath
 ...
 */
async function getExpectedDiagnosticFromTestDetails(
    testDetails: ITestDetails
    ...
    if (testDetails.imported) {
        expectedSourceTestFilePath = path.join(
            UNITTEST_TEST_FILES_PATH,
            testDetails.sourceFileName!
        );
    }
    const expectedSourceTestFileUri = vscode.Uri.file(
        expectedSourceTestFilePath
    );
    const diagMsgPrefix = new UnitTestDiagnosticService().getMessagePrefix(
        testDetails.status
    );
    const expectedDiagMsg = `${diagMsgPrefix ? `${diagMsgPrefix}: ` : ""}${
        testDetails.message
    }`;
    ...
    relatedInfo.push(
        new vscode.DiagnosticRelatedInformation(
            new vscode.Location(
                expectedSourceTestFileUri,
                testDetails.testDefRange!
            ),
    ...
    const expectedDiagnostic = new vscode.Diagnostic(
        expectedDiagRange!,
        expectedDiagMsg,
        expectedSeverity
    );
...
async function testResultsSummary(
    results: Tests,
    expectedSummaryCount: IResultsSummaryCount
) {
    const totalTests =
    ...
    assert.equal(
        results.summary.skipped,
        expectedSummaryCount.skips,
        "Skipped"
    );
    assert.equal(
        results.summary.failures,
        expectedSummaryCount.failures,
        "Failures"
    );
...
async function testDiagnosticRelatedInformation(
    ...
    assert.equal(
        relatedInfo.message,
        expectedRelatedInfo.message,
        "DiagnosticRelatedInfo definition"
    );
    assert.equal(
...
suite("Unit Tests - pytest - run with mocked process output", () => {
        ...
        procService.onExecObservable((file, args, options, callback) => {
            if (args.indexOf("--collect-only") >= 0) {
                callback({
                    out: fs
                        .readFileSync(
                            path.join(PYTEST_RESULTS_PATH, outputFileName),
                            "utf8"
                        )
                        .replace(
    ...
    async function injectTestRunOutput(
        outputFileName: string,
        failedOutput: boolean = false
    ) {
        const procService = (await ioc.serviceContainer
            .get<IProcessServiceFactory>(IProcessServiceFactory)
            .create()) as MockProcessService;
    ...
    function getScenarioTestDetails(
        scenario: ITestScenarioDetails,
        failedRun: boolean
    ): ITestDetails[] {
    ...
    testScenarios.forEach(scenario => {
        suite(scenario.scenarioName, () => {
            ...
            suiteSetup(async () => {
                ...
                factory = ioc.serviceContainer.get<ITestManagerFactory>(
                    ITestManagerFactory
                );
                testManager = factory(
                    "pytest",
                ...
                scenario.testsToRun = await getScenarioTestsToRun(
                    scenario,
                    tests
                );
            });
            ...
            const shouldRunProperly = (suiteName, failedRun = false) => {
                suite(suiteName, () => {
                    ...
                    const uniqueIssueFiles = getUniqueIssueFilesFromTestDetails(
                        testDetails
                    );
                    let expectedSummaryCount;
                    suiteSetup(async () => {
                        testDetails = getScenarioTestDetails(
                            scenario,
                            failedRun
                        );
                        results = await getResultsFromTestManagerRunTest(
                            testManager,
                            scenario.testsToRun,
                            failedRun
                        );
                        expectedSummaryCount = getExpectedSummaryCount(
                            testDetails,
                            failedRun
                        );
                    });
                    ...
                    uniqueIssueFiles.forEach(fileName => {
                        suite(fileName, () => {
                            ...
                            const relevantTestDetails = getRelevantTestDetailsForFile(
                                testDetails,
                                fileName
                            );
                            ...
                            suiteSetup(async () => {
                                ...
                                diagnostics = testManager.diagnosticCollection.get(
                                    testFileUri
                                )!;
                                expectedDiagnosticCount = getIssueCountFromRelevantTestDetails(
                                    relevantTestDetails,
                                    relevantSkippedIssues,
                                    failedRun
                                );
                            });
                            test("Test DiagnosticCollection", async () => {
                                assert.equal(
                                    diagnostics.length,
                            ...
                            const validateTestFunctionAndDiagnostics = (
                                td: ITestDetails
                            ) => {
                                suite(td.testName, () => {
                                    ...
                                    if (
                                        td.status !== TestStatus.Pass &&
                                        !(failedRun && td.passOnFailedRun)
                                    ) {
                                        suite("Diagnostic", async () => {
                                            suiteSetup(async () => {
                                                diagnostic = getDiagnosticForTestFunc(
                                                    diagnostics,
                                                    testFunc
                                                )!;
                                                expectedDiagnostic = await getExpectedDiagnosticFromTestDetails(
                                                    td
                                                );
                                            });
