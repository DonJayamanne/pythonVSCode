...
const updateMethods: (keyof ITestConfigSettingsService)[] = [
    "updateTestArgs",
    "disable",
    "enable"
];
...
suite("Unit Tests - ConfigSettingsService", () => {
    [Product.pytest, Product.unittest, Product.nosetest].forEach(prodItem => {
        ...
        updateMethods.forEach(updateMethod => {
            suite(
                `Test '${updateMethod}' method with ${productName.name}`,
                () => {
                    ...
                    setup(() => {
                        ...
                        testConfigSettingsService = new TestConfigSettingsService(
                    ...
                    test("Update Test Arguments with workspace Uri without workspaces", async () => {
                        ...
                        workspaceService
                            .setup(w =>
                                w.getConfiguration(typeMoq.It.isValue("python"))
                            )
                        ...
                        pythonConfig
                            .setup(p =>
                                p.update(
                                    typeMoq.It.isValue(configName),
                                    typeMoq.It.isValue(configValue)
                                )
                            )
                        ...
                        if (updateMethod === "updateTestArgs") {
                            await testConfigSettingsService.updateTestArgs(
                                workspaceUri,
                                product,
                                configValue
                            );
                        } else {
                            await testConfigSettingsService[updateMethod](
                                workspaceUri,
                                product
                            );
                        }
                    ...
                    test("Expect an exception when updating Test Arguments with workspace Uri with more than one workspace and uri does not belong to a workspace", async () => {
                        ...
                        const promise = testConfigSettingsService.updateTestArgs(
                            workspaceUri,
                            product,
                            configValue
                        );
