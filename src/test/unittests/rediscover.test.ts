...
const testFileWithFewTests = path.join(
    testFilesPath,
    "tests",
    "test_debugger_two.txt"
);
const testFileWithMoreTests = path.join(
    testFilesPath,
    "tests",
    "test_debugger_two.updated.txt"
);
...
suite("Unit Tests re-discovery", () => {
    ...
    async function discoverUnitTests(testProvider: TestProvider) {
        ...
        let tests = await testManager.discoverTests(
            CommandSource.ui,
            true,
            true
        );
        ...
        assert.equal(
            tests.testFunctions.length,
            4,
            "Incorrect number of updated test functions"
        );
    }
