...
suite("Unit Tests Stopping Discovery and Runner", () => {
    ...
    test("Running tests should not stop existing discovery", async () => {
        ...
        const discoveryPromise = mockTestManager.discoverTests(
            CommandSource.auto
        );
        ...
        runningPromise
            .then(() =>
                Promise.reject("Debugger stopped when it shouldn't have")
            )
    ...
    test("Discovering tests should stop running tests", async () => {
        ...
        await mockTestManager.discoverTests(
            CommandSource.ui,
            true,
            false,
            true
        );
        await expect(runPromise).to.eventually.be.rejectedWith(
            CANCELLATION_REASON
        );
    });
});
