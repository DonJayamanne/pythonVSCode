...
suite("ArgsService: Common", () => {
    [Product.unittest, Product.nosetest, Product.pytest].forEach(product => {
        const productNames = getNamesAndValues(Product);
        const productName = productNames.find(item => item.value === product)!
            .name;
        suite(productName, () => {
            ...
            setup(function() {
                ...
                serviceContainer
                    .setup(s =>
                        s.get(typeMoq.It.isValue(ILogger), typeMoq.It.isAny())
                    )
                ...
                serviceContainer
                    .setup(s =>
                        s.get(
                            typeMoq.It.isValue(IArgumentsHelper),
                            typeMoq.It.isAny()
                        )
                    )
                    .returns(() => argsHelper);
                ...
                switch (product) {
                    case Product.unittest: {
                        argumentsService = new UnitTestArgumentsService(
                    ...
                    case Product.nosetest: {
                        argumentsService = new NoseTestArgumentsService(
                            serviceContainer.object
                        );
                        moduleName = "nose";
                        break;
                    ...
                    case Product.pytest: {
                        moduleName = "pytest";
                        argumentsService = new PyTestArgumentsService(
                            serviceContainer.object
                        );
                        break;
                    }
            ...
            test("Check for new/unrecognized options with values", () => {
                ...
                const optionsNotFound = expectedWithArgs.filter(
                    item => options.withArgs.indexOf(item) === -1
                );
            ...
            test("Check for new/unrecognized options without values", () => {
                const options = argumentsService.getKnownOptions();
                const optionsNotFound = expectedWithoutArgs.filter(
                    item => options.withoutArgs.indexOf(item) === -1
                );
            ...
            test("Test filtering of arguments", () => {
                ...
                for (let i = 0; i < 5; i += 1) {
                    ...
                    if (i % 2 === 0) {
                        argumentsToRemove.push(
                            knownOptions.withArgs[i],
                            knownOptions.withoutArgs[i]
                        );
                    } else {
                        expectedFilteredArgs.push(
                            knownOptions.withArgs[i],
                            `Random Value ${i}`
                        );
                ...
                const filteredArgs = argumentsService.filterArguments(
                    args,
                    argumentsToRemove
                );
...
function getMatches(pattern, str) {
    ...
    return matches
        ...
        .reduce<string[]>(
            (items, item) =>
                items.indexOf(item) === -1 ? items.concat([item]) : items,
            []
        );
}
