...
suite("Multiroot Linting", () => {
    ...
    async function testLinterInWorkspaceFolder(
        ...
        const errorMessage = mustHaveErrors
            ? "No errors returned by linter"
            : "Errors returned by linter";
        assert.equal(messages.length > 0, mustHaveErrors, errorMessage);
    }
    async function enableDisableSetting(
        workspaceFolder,
        configTarget: ConfigurationTarget,
        setting: string,
        value: boolean
    ): Promise<void> {
        const config = ioc.serviceContainer.get<IConfigurationService>(
            IConfigurationService
        );
