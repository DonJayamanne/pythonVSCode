...
suite("Linting - Multiple Linters Enabled Test", () => {
    ...
    setup(async () => {
        ...
        api.serviceManager.rebind<IPythonToolExecutionService>(
            IPythonToolExecutionService,
            MockPythonToolExecService
        );
    });
    ...
    test("Multiple linters", async () => {
        ...
        const document = await workspace.openTextDocument(
            path.join(pythoFilesPath, "print.py")
        );
        ...
        const collection = (await commands.executeCommand(
            "python.runLinting"
        )) as DiagnosticCollection;
        assert.notEqual(
            collection,
            undefined,
            "python.runLinting did not return valid diagnostics collection."
        );

        const messages = collection!.get(document.uri);
        assert.notEqual(messages!.length, 0, "No diagnostic messages.");
        assert.notEqual(
            messages!.filter(x => x.source === "pylint").length,
            0,
            "No pylint messages."
        );
        assert.notEqual(
            messages!.filter(x => x.source === "flake8").length,
            0,
            "No flake8 messages."
        );
    });
