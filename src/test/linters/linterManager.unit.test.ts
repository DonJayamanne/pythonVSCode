...
suite("Linting - Linter Manager", () => {
    ...
    setup(() => {
        ...
        when(svcContainer.get<IApplicationShell>(IApplicationShell)).thenReturn(
            instance(shell)
        );
        when(svcContainer.get<IDocumentManager>(IDocumentManager)).thenReturn(
            instance(docManager)
        );
        when(svcContainer.get<ICommandManager>(ICommandManager)).thenReturn(
            instance(cmdManager)
        );
        when(svcContainer.get<ILintingEngine>(ILintingEngine)).thenReturn(
            instance(lintingEngine)
        );
        when(
            svcContainer.get<IConfigurationService>(IConfigurationService)
        ).thenReturn(instance(configService));
        when(svcContainer.get<IWorkspaceService>(IWorkspaceService)).thenReturn(
            instance(workspaceService)
        );
        linterManager = new LinterManagerTest(
    ...
    test("Get all linters will return a list of all linters", () => {
        ...
        expect(linters.map(item => item.id).sort()).to.be.deep.equal(
            linterProducts.sort()
        );
    });
    ...
    test("Get linter info for non-linter product should throw an exception", () => {
        ...
        getNamesAndValues<Product>(Product).forEach(prod => {
            if (
                productService.getProductType(prod.value) === ProductType.Linter
            ) {
                const info = linterManager.getLinterInfo(prod.value);
                expect(info.id).to.equal(ProductNames.get(prod.value));
                expect(info).not.to.be.equal(
                    undefined,
                    "should not be unedfined"
                );
            } else {
                expect(() =>
                    linterManager.getLinterInfo(prod.value)
                ).to.throw();
            }
        });
    });
    test("Pylint configuration file watch", async () => {
        ...
        assert.equal(
            pylint.configFileNames.length,
            2,
            "Pylint configuration file count is incorrect."
        );
        assert.notEqual(
    ...
    [undefined, Uri.parse("something")].forEach(resource => {
        ...
        test(`getActiveLinters will check if linter is enabled and in silent mode ${testResourceSuffix}`, async () => {
            ...
            const linters = await linterManager.getActiveLinters(
                true,
                resource
            );
        ...
        test(`getActiveLinters will check if linter is enabled and not in silent mode ${testResourceSuffix}`, async () => {
            ...
            verify(linterInfo.isEnabled(resource)).once();
            expect(linters[0]).to.deep.equal(instanceOfLinterInfo);
            expect(enableUnconfiguredLintersInvoked).to.equal(
                true,
                "not invoked"
            );
        });
        ...
        test(`setActiveLintersAsync with invalid products does nothing ${testResourceSuffix}`, async () => {
            ...
            await linterManager.setActiveLintersAsync(
                [Product.ctags, Product.pytest],
                resource
            );

            expect(getActiveLintersInvoked).to.be.equal(
                false,
        ...
        test(`setActiveLintersAsync with single product will disable it then enable it ${testResourceSuffix}`, async () => {
            ...
            linterManager.getActiveLinters = () =>
                Promise.resolve([instanceOfLinterInfo]);
        ...
        test(`setActiveLintersAsync with single product will disable all existing then enable the necessary two ${testResourceSuffix}`, async () => {
            ...
            [
                ...
                when(
                    linterInfo.enableAsync(anything(), resource)
                ).thenResolve();
            });

            linterManager.getActiveLinters = () =>
                Promise.resolve(Array.from(linterInstances.values()));
            linterManager.enableLintingAsync = () => Promise.resolve();

            const lintersToEnable = [
                Product.flake8,
                Product.mypy,
                Product.pydocstyle
            ];
            await linterManager.setActiveLintersAsync(
                [Product.flake8, Product.mypy, Product.pydocstyle],
                resource
            );
