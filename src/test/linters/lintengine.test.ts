...
suite("Linting - LintingEngine", () => {
    ...
    setup(async () => {
        ...
        lintManager
            .setup(x => x.isLintingEnabled(TypeMoq.It.isAny()))
            .returns(async () => true);
        serviceContainer
            .setup(c =>
        ...
        serviceContainer
            .setup(c =>
                c.get(TypeMoq.It.isValue(ILintingEngine), TypeMoq.It.isAny())
            )
    ...
    function mockTextDocument(
        ...
        fileSystem
            .setup(x => x.fileExists(TypeMoq.It.isAnyString()))
        ...
        if (scheme) {
            doc.setup(d => d.uri).returns(() =>
                Uri.parse(`${scheme}:${fileName}`)
            );
        } else {
