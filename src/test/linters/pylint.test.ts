...
suite("Linting - Pylint", () => {
    ...
    setup(() => {
        ...
        serviceManager.addSingletonInstance<IPlatformService>(
            IPlatformService,
        ...
        serviceManager.addSingletonInstance<ILinterManager>(
            ILinterManager,
            linterManager
        );
        ...
        serviceManager.addSingletonInstance<IInstaller>(
            IInstaller,
            installer.object
        );
    });
    ...
    test("pylintrc in the file folder", async () => {
        fileSystem
            .setup(x => x.fileExists(path.join(basePath, pylintrc)))
            .returns(() => Promise.resolve(true));
        let result = await Pylint.hasConfigurationFile(
            fileSystem.object,
            basePath,
            platformService.object
        );
        expect(result).to.be.equal(
            true,
            `'${pylintrc}' not detected in the file folder.`
        );

        fileSystem
            .setup(x => x.fileExists(path.join(basePath, dotPylintrc)))
            .returns(() => Promise.resolve(true));
        result = await Pylint.hasConfigurationFile(
            fileSystem.object,
            basePath,
            platformService.object
        );
        expect(result).to.be.equal(
            true,
            `'${dotPylintrc}' not detected in the file folder.`
        );
    });
    test("pylintrc up the module tree", async () => {
        ...
        fileSystem
            .setup(x => x.fileExists(module1))
            .returns(() => Promise.resolve(true));
        fileSystem
            .setup(x => x.fileExists(module2))
            .returns(() => Promise.resolve(true));
        fileSystem
            .setup(x => x.fileExists(module3))
            .returns(() => Promise.resolve(true));
        fileSystem
            .setup(x => x.fileExists(rc))
            .returns(() => Promise.resolve(true));

        const result = await Pylint.hasConfigurationFile(
            fileSystem.object,
            basePath,
            platformService.object
        );
        expect(result).to.be.equal(
            true,
            `'${pylintrc}' not detected in the module tree.`
        );
    });
    test(".pylintrc up the module tree", async () => {
        ...
        expect(result).to.be.equal(
            true,
            `'${dotPylintrc}' not detected in the module tree.`
        );
    });
    test(".pylintrc up the ~ folder", async () => {
        ...
        expect(result).to.be.equal(
            true,
            `'${dotPylintrc}' not detected in the ~ folder.`
        );
    });
    test("pylintrc up the ~/.config folder", async () => {
        ...
        expect(result).to.be.equal(
            true,
            `'${pylintrc}' not detected in the  ~/.config folder.`
        );
    });
    test("pylintrc in the /etc folder", async () => {
        ...
        expect(result).to.be.equal(
            true,
            `'${pylintrc}' not detected in the /etc folder.`
        );
    });
    test("pylintrc between file and workspace root", async () => {
        ...
        const result = await Pylint.hasConfigrationFileInWorkspace(
            fileSystem.object,
            basePath,
            root
        );
        expect(result).to.be.equal(
            true,
            `'${pylintrc}' not detected in the workspace tree.`
        );
    });
    ...
    async function testPylintArguments(
        fileFolder: string,
        wsRoot: string,
        expectedMinArgs: boolean
    ): Promise<void> {
        ...
        document
            .setup(x => x.uri)
            .returns(() => Uri.file(path.join(fileFolder, "test.py")));

        const wsf = TypeMoq.Mock.ofType<WorkspaceFolder>();
        wsf.setup(x => x.uri).returns(() => Uri.file(wsRoot));

        workspace
            .setup(x => x.getWorkspaceFolder(TypeMoq.It.isAny()))
            .returns(() => wsf.object);
    ...
    test("Negative column numbers should be treated 0", async () => {
        ...
        execService
            ...
            .returns(() =>
                Promise.resolve({ stdout: linterOutput, stderr: "" })
            );
        ...
        const messages = await pylinter.lint(
            document.object,
            new CancellationTokenSource().token
        );
