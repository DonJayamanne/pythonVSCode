...
suite("Linting - Linter Commands", () => {
    ...
    setup(() => {
        ...
        when(svcContainer.get<ILinterManager>(ILinterManager)).thenReturn(
            instance(manager)
        );
    ...
    test("Commands are registered", () => {
        verify(
            cmdManager.registerCommand(Commands.Set_Linter, anything())
        ).once();
        verify(
            cmdManager.registerCommand(Commands.Enable_Linter, anything())
        ).once();
        verify(
            cmdManager.registerCommand(Commands.Run_Linter, anything())
        ).once();
    });
    ...
    async function testEnableLintingWithCurrentState(
        ...
        selectedState: "on" | "off" | undefined
    ) {
        when(manager.isLintingEnabled(true, anything())).thenResolve(
            currentState
        );
        ...
        if (selectedState) {
            verify(
                manager.enableLintingAsync(selectedState === "on", anything())
            ).once();
        } else {
    ...
    test("Set Linter should display a quickpick and with message about multiple linters being enabled", async () => {
        const activeLinters: ILinterInfo[] = [
            { id: "linterId" } as any,
            { id: "linterId2" } as any
        ];
        when(manager.getAllLinterInfos()).thenReturn([]);
    ...
    test("Selecting a linter should display warning message about multiple linters", async () => {
        ...
        when(shell.showWarningMessage(anything(), "Yes", "No")).thenResolve(
            "Yes" as any
        );
        const expectedQuickPickOptions = {
            matchOnDetail: true,
        ...
        expect(quickPickOptions).to.deep.equal(expectedQuickPickOptions);
        verify(
            manager.setActiveLintersAsync(deepEqual(["Three"]), anything())
        ).once();
    });
});
