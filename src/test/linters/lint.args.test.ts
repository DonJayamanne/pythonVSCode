...
suite("Linting - Arguments", () => {
    [undefined, path.join("users", "dev_user")].forEach(workspaceUri => {
        [
            Uri.file(
                path.join("users", "dev_user", "development path to", "one.py")
            ),
            Uri.file(path.join("users", "dev_user", "development", "one.py"))
        ].forEach(fileUri => {
            suite(
                `File path ${
                    fileUri.fsPath.indexOf(" ") > 0 ? "with" : "without"
                } spaces and ${workspaceUri ? "without" : "with"} a workspace`,
                () => {
                    ...
                    setup(async () => {
                        ...
                        serviceManager.addSingletonInstance<
                            IInterpreterService
                        >(IInterpreterService, interpreterService.object);
                        ...
                        serviceManager.addSingletonInstance<ILintingEngine>(
                            ILintingEngine,
                            engine.object
                        );

                        docManager = TypeMoq.Mock.ofType<IDocumentManager>();
                        serviceManager.addSingletonInstance<IDocumentManager>(
                            IDocumentManager,
                            docManager.object
                        );
                        ...
                        lintSettings
                            .setup(x => x.lintOnSave)
                            .returns(() => true);

                        settings = TypeMoq.Mock.ofType<IPythonSettings>();
                        settings
                            .setup(x => x.linting)
                            .returns(() => lintSettings.object);
                        ...
                        serviceManager.addSingletonInstance<
                            IConfigurationService
                        >(IConfigurationService, configService.object);

                        const workspaceFolder:
                            | WorkspaceFolder
                            | undefined = workspaceUri
                            ? {
                                  uri: Uri.file(workspaceUri),
                                  index: 0,
                                  name: ""
                              }
                            : undefined;
                        ...
                        workspaceService
                            .setup(w =>
                                w.getWorkspaceFolder(TypeMoq.It.isAny())
                            )
                        ...
                        serviceManager.addSingletonInstance<ILogger>(
                            ILogger,
                            logger.object
                        );
