...
suite("Linter Availability Provider tests", () => {
    ...
    test("Prompt will be performed when linter is not configured at all for the workspace, workspace-folder, or the user", async () => {
        ...
        const result = availabilityProvider.isLinterUsingDefaultConfiguration(
            linterInfo
        );
    ...
    async function testForLinterPromptResponse(
        promptReply: { title: string; enabled: boolean } | undefined
    ): Promise<boolean> {
        // arrange
        const [
            appShellMock,
        ...
        const linterInfo = new class extends LinterInfo {
            public testIsEnabled: boolean = promptReply
                ? promptReply.enabled
                : false;

            public async enableAsync(
                enabled: boolean,
                resource?: Uri
            ): Promise<void> {
        ...
        }(Product.pylint, "pylint", configServiceMock.object, [
            ".pylintrc",
            "pylintrc"
        ]);
        ...
        appShellMock
            .setup(ap =>
                ap.showInformationMessage(
                    TypeMoq.It.isValue(
                        `Linter ${linterInfo.id} is installed but not enabled.`
                    ),
                    TypeMoq.It.isAny(),
                    TypeMoq.It.isAny()
                )
        ...
        const result = await availabilityProvider.promptToConfigureAvailableLinter(
            linterInfo
        );
        if (promptReply) {
    ...
    async function performTestOfOverallImplementation(
        options: AvailablityTestOverallOptions
    ): Promise<boolean> {
        // arrange
        const [
            appShellMock,
        ...
        installerMock
            .setup(im => im.isInstalled(linterInfo.product, TypeMoq.It.isAny()))
    ...
    test("Discovery of linter is available in the environment returns true when it succeeds and is present", async () => {
        ...
        setupInstallerForAvailabilityTest(
            linterInfo,
            linterIsInstalled,
            installerMock
        );
        ...
        const result = await availabilityProvider.isLinterAvailable(
            linterInfo.product
        );
...
function setupWorkspaceMockForLinterConfiguredTests(
    ...
    workspaceServiceMock
        .setup(ws =>
            ws.getConfiguration(
                TypeMoq.It.isValue("python.linting"),
                TypeMoq.It.isAny()
            )
        )
...
function getDependenciesForAvailabilityTests(): [
    ...
    return [
        ...
        new LinterInfo(Product.pylint, "pylint", configServiceMock.object, [
            ".pylintrc",
            "pylintrc"
        ])
    ];
