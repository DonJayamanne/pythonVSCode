...
const pythoFilesPath = path.join(
    __dirname,
    "..",
    "..",
    "..",
...
const pydocstyleMessagseToBeReturned: ILintMessage[] = [
    ...
    },
    {
        ...
        severity: LintMessageSeverity.Information,
        message:
            "First word of the first line should be properly capitalized ('This', not 'this')",
        column: 4,
        line: 11,
        type: "",
        provider: "pydocstyle"
    ...
    },
    {
        ...
        severity: LintMessageSeverity.Information,
        message:
            "First word of the first line should be properly capitalized ('And', not 'and')",
        column: 4,
        line: 15,
        type: "",
        provider: "pydocstyle"
    ...
    },
    {
        ...
        severity: LintMessageSeverity.Information,
        message:
            "First word of the first line should be properly capitalized ('Test', not 'test')",
        column: 4,
        line: 21,
        type: "",
        provider: "pydocstyle"
...
suite("Linting - General Tests", () => {
    ...
    function makeSettingKey(product: Product): PythonSettingKeys {
        return `linting.${
            linterManager.getLinterInfo(product).enabledSettingName
        }` as PythonSettingKeys;
    }
    ...
    async function testLinterMessages(
        ...
        const outputChannel = ioc.serviceContainer.get<MockOutputChannel>(
            IOutputChannel,
        ...
        const linter = await linterManager.createLinter(
            product,
            outputChannel,
            ioc.serviceContainer
        );
        ...
        if (messagesToBeReceived.length === 0) {
            assert.equal(
                messages.length,
                0,
                `No errors in linter, Output - ${outputChannel.output}`
            );
        } else {
    ...
    test("PyLint with config in root", async () => {
        await fs.copy(
            path.join(pylintConfigPath, ".pylintrc"),
            path.join(workspaceUri.fsPath, ".pylintrc")
        );
        await testLinterMessages(
            Product.pylint,
            path.join(pylintConfigPath, "file2.py"),
            []
        );
    });
    test("Flake8 with config in root", async () => {
        await testLinterMessages(
            Product.flake8,
            path.join(flake8ConfigPath, "file.py"),
            filteredFlake8MessagesToBeReturned
        );
    });
    test("Pep8 with config in root", async () => {
        await testLinterMessages(
            Product.pep8,
            path.join(pep8ConfigPath, "file.py"),
            filteredPep88MessagesToBeReturned
        );
    });
    test("Pydocstyle with config in root", async () => {
        ...
        await fs.copy(
            path.join(pydocstyleConfigPath27, ".pydocstyle"),
            path.join(workspaceUri.fsPath, ".pydocstyle")
        );
        await testLinterMessages(
            Product.pydocstyle,
            path.join(pydocstyleConfigPath27, "file.py"),
            []
        );
    });
