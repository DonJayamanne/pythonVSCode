...
suite("Variable Extraction", () => {
    ...
    setup(async () => {
        initializeDI();
        refactorTargetFile = path.join(
            refactorTargetFileDir,
            `refactor${new Date().getTime()}.py`
        );
    ...
    async function testingVariableExtraction(
        ...
        const pythonSettings = getExtensionSettings(
            Uri.file(refactorTargetFile)
        );
        ...
        const mockTextDoc = await workspace.openTextDocument(
            refactorTargetFile
        );
        const expectedTextEdits = getTextEditsFromPatch(
            mockTextDoc.getText(),
            DIFF
        );
        try {
            ...
            if (shouldError) {
                assert.fail(
                    "No error",
                    "Error",
                    "Extraction should fail with an error",
                    ""
                );
            }
            const textEdits = getTextEditsFromPatch(
                mockTextDoc.getText(),
                DIFF
            );
            assert.equal(
                ...
                1,
                "Invalid number of items in response"
            );
            assert.equal(
                textEdits.length,
                expectedTextEdits.length,
                "Invalid number of Text Edits"
            );
            textEdits.forEach(edit => {
                const foundEdit = expectedTextEdits.filter(
                    item =>
                        item.newText === edit.newText &&
                        item.range.isEqual(edit.range)
                );
    ...
    async function testingVariableExtractionEndToEnd(
        ...
        editor.selections = [
            new Selection(rangeOfTextToExtract.start, rangeOfTextToExtract.end)
        ];
        editor.selection = new Selection(
            rangeOfTextToExtract.start,
            rangeOfTextToExtract.end
        );
        try {
            ...
            const newVarRefLine = textDocument.lineAt(
                newVarDefLine.lineNumber + 1
            );

            assert.equal(
                newVarDefLine.text.trim().indexOf("newvariable"),
                0,
                "New Variable not created"
            );
            assert.equal(
                newVarDefLine.text.trim().endsWith('= "STARTED"'),
                true,
                "Started Text Assigned to variable"
            );
            assert.equal(
                newVarRefLine.text.indexOf("(newvariable") >= 0,
                true,
                "New Variable not being used"
            );
        } catch (error) {
            if (!shouldError) {
