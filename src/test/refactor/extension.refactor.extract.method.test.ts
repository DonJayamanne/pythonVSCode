...
suite("Method Extraction", () => {
    ...
    async function testingMethodExtraction(
        shouldError: boolean,
        startPos: Position,
        endPos: Position
    ): Promise<void> {
    ...
    async function testingMethodExtractionEndToEnd(
        ...
        try {
            ...
            const newMethodRefLine = textDocument.lineAt(
            ...
            assert.equal(
                newMethodRefLine.text.trim().startsWith("self.newmethod"),
                true,
                "New Method not being used"
            );
        } catch (error) {
            if (!shouldError) {
                assert.equal("Error", "No error", `${error}`);
