...
export class IocContainer {
    ...
    constructor() {
        ...
        this.serviceManager.addSingletonInstance<IServiceContainer>(
            IServiceContainer,
            this.serviceContainer
        );
        this.serviceManager.addSingletonInstance<Disposable[]>(
            IDisposableRegistry,
            this.disposables
        );
        this.serviceManager.addSingleton<Memento>(
            IMemento,
            MockMemento,
            GLOBAL_MEMENTO
        );
        ...
        const testOutputChannel = new MockOutputChannel(
            "Python Test - UnitTests"
        );
    ...
    public registerFileSystemTypes() {
        this.serviceManager.addSingleton<IPlatformService>(
            IPlatformService,
            PlatformService
        );
    ...
    public registerMockProcessTypes() {
        ...
        const processService = new MockProcessService(
            new ProcessService(new BufferDecoder(), process.env as any)
        );
        processServiceFactory
            .setup(f => f.create(TypeMoq.It.isAny()))
            .returns(() => Promise.resolve(processService));
    ...
    public registerMockProcess() {
        ...
        this.serviceManager.addSingleton<ICurrentProcess>(
            ICurrentProcess,
            MockProcess
        );
    }
