...
suite("Terminal - Code Execution", () => {
    ["Terminal Execution", "Repl Execution", "Django Execution"].forEach(
        testSuiteName => {
            ...
            suite(`${testSuiteName} (validation of title)`, () => {
                    ...
                    terminalFactory
                        .setup(f =>
                            f.getTerminalService(
                                TypeMoq.It.isAny(),
                                TypeMoq.It.isValue(expectedTerminalTitle)
                            )
                        )
                ...
                test("Ensure terminal is created upon invoking initializeRepl (windows)", async () => {
                    await ensureTerminalIsCreatedUponInvokingInitializeRepl(
                        true,
                        false,
                        false
                    );
            ...
            suite(testSuiteName, async function() {
                ...
                async function ensureWeSetCurrentDirectoryBeforeExecutingAFile(
                    isWindows: boolean
                ): Promise<void> {
                    const file = Uri.file(
                        path.join("c", "path", "to", "file", "one.py")
                    );
                    terminalSettings
                    ...
                    workspaceFolder
                        .setup(w => w.uri)
                        .returns(() => Uri.file(path.join("c", "path", "to")));
                ...
                test("Ensure we set current directory before executing file (non windows)", async () => {
                    await ensureWeSetCurrentDirectoryBeforeExecutingAFile(
                        false
                    );
                });
                ...
                async function ensureWeWetCurrentDirectoryAndQuoteBeforeExecutingFile(
                    isWindows: boolean
                ): Promise<void> {
                    const file = Uri.file(
                        path.join(
                    ...
                    const dir = path
                        ...
                        .fileToCommandArgument();
                    terminalService.verify(
                        async t => t.sendText(TypeMoq.It.isValue(`cd ${dir}`)),
                        TypeMoq.Times.once()
                    );
                }
                ...
                async function ensureWeDoNotSetCurrentDirectoryBeforeExecutingFileInSameDirectory(
                    ...
                    terminalService.verify(
                        async t => t.sendText(TypeMoq.It.isAny()),
                        TypeMoq.Times.never()
                    );
                }
                ...
                async function ensureWeDoNotSetCurrentDirectoryBeforeExecutingFileNotInSameDirectory(
                    isWindows: boolean
                ): Promise<void> {
                    const file = Uri.file(
                        path.join(
                ...
                test("Ensure we do not set current directory before executing file if file is not in a workspace (non windows)", async () => {
                    await ensureWeDoNotSetCurrentDirectoryBeforeExecutingFileNotInSameDirectory(
                        false
                    );
                });
                ...
                async function testFileExecution(
                    ...
                    terminalSettings
                        .setup(t => t.launchArgs)
                        .returns(() => terminalArgs);
                    terminalSettings
                        .setup(t => t.executeInFileDir)
                        .returns(() => false);
                    workspace
                    ...
                    const expectedPythonPath = isWindows
                        ...
                        : pythonPath;
                    const expectedArgs = terminalArgs.concat(
                ...
                function testReplCommandArguments(
                    isWindows: boolean,
                    pythonPath: string,
                    ...
                    terminalSettings
                        .setup(t => t.launchArgs)
                        .returns(() => terminalArgs);
                    const expectedTerminalArgs = isDjangoRepl
                        ? terminalArgs.concat(["manage.py", "shell"])
                        : terminalArgs;

                    const replCommandArgs = (executor as TerminalCodeExecutionProvider).getReplCommandArgs();
                    expect(replCommandArgs).not.to.be.an(
                        "undefined",
                        "Command args is undefined"
                    );
                    expect(replCommandArgs.command).to.be.equal(
                        expectedPythonPath,
                        "Incorrect python path"
                    );
                    expect(replCommandArgs.args).to.be.deep.equal(
                    ...
                    terminalService.verify(
                        async t =>
                            t.sendCommand(
                                ...
                                TypeMoq.It.isValue(expectedTerminalArgs)
                            ),
                        TypeMoq.Times.once()
                    );
                });
                ...
                test("Ensure repl is re-initialized when terminal is closed", async () => {
                    ...
                    expect(closeTerminalCallback).not.to.be.an(
                        "undefined",
                ...
                test("Ensure code is sent to terminal", async () => {
                    ...
                    await executor.execute("cmd1");
                    terminalService.verify(
                        async t => t.sendText("cmd1"),
                        TypeMoq.Times.once()
                    );

                    await executor.execute("cmd2");
                    terminalService.verify(
                        async t => t.sendText("cmd2"),
                        TypeMoq.Times.once()
                    );
                });
            });
