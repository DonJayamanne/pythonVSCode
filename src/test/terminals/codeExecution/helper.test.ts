...
const TEST_FILES_PATH = path.join(
    EXTENSION_ROOT_DIR,
    "src",
    "test",
    "pythonFiles",
...
suite("Terminal - Code Execution Helper", () => {
    ...
    setup(() => {
        ...
        serviceContainer
            .setup(c =>
                c.get(TypeMoq.It.isValue(IDocumentManager), TypeMoq.It.isAny())
            )
        ...
        serviceContainer
            .setup(c =>
                c.get(
                    TypeMoq.It.isValue(IEnvironmentVariablesProvider),
                    TypeMoq.It.isAny()
                )
            )
    ...
    async function ensureBlankLinesAreRemoved(
        source: string,
        ...
        processService
            ...
            .returns((file, args, options) => {
                return actualProcessService.exec.apply(actualProcessService, [
                    file,
                    args,
                    options
                ]);
    ...
    ["", "1", "2", "3", "4", "5", "6", "7"].forEach(fileNameSuffix => {
        test(`Ensure blank lines are removed (Sample${fileNameSuffix})`, async function() {
            ...
            const code = await fs.readFile(
                path.join(TEST_FILES_PATH, `sample${fileNameSuffix}_raw.py`),
                "utf8"
            );
            const expectedCode = await fs.readFile(
                path.join(
                    TEST_FILES_PATH,
                    `sample${fileNameSuffix}_normalized.py`
                ),
                "utf8"
            );
        ...
        test(`Ensure last two blank lines are preserved even if we have more than 2 trailing blank lines (Sample${fileNameSuffix})`, async function() {
            ...
            await ensureBlankLinesAreRemoved(
                code + EOL + EOL + EOL + EOL,
                expectedCode + EOL
            );
        });
    });
    test("Display message if theres no active file", async () => {
        documentManager
            .setup(doc => doc.activeTextEditor)
            .returns(() => undefined);

        const uri = await helper.getFileToExecute();
        expect(uri).to.be.an("undefined");
        applicationShell.verify(
            a => a.showErrorMessage(TypeMoq.It.isAnyString()),
            TypeMoq.Times.once()
        );
    });
    ...
    test("Returns current line if nothing is selected", async () => {
        ...
        document
            .setup(d => d.lineAt(TypeMoq.It.isAny()))
    ...
    test("Returns selected text", async () => {
        ...
        editor
            .setup(e => e.selection)
            .returns(() => new Selection(3, 0, 10, 5));
        const textLine = TypeMoq.Mock.ofType<TextLine>();
        textLine.setup(t => t.text).returns(() => lineContents);
        document
            ...
            .returns(
                (r: Range) =>
                    `${r.start.line}.${r.start.character}.${r.end.line}.${
                        r.end.character
                    }`
            );
