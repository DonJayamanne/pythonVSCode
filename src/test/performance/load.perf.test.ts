...
suite("Activation Times", () => {
    if (process.env.ACTIVATION_TIMES_LOG_FILE_PATH) {
        ...
        test(`Capture Extension Activation Times (Version: ${
            process.env.ACTIVATION_TIMES_EXT_VERSION
        }, sample: ${sampleCounter})`, async () => {
            ...
            if (elapsedTime > 10) {
                ...
                await fs.appendFile(logFile, `${elapsedTime}${EOL}`, {
                    encoding: "utf8"
                });
    ...
    if (
        ...
        test("Test activation times of Dev vs Release Extension", async () => {
            ...
            const devActivationTimes = getActivationTimes(
                JSON.parse(process.env.ACTIVATION_TIMES_DEV_LOG_FILE_PATHS!)
            );
            ...
            const devActivationAvgTime =
                devActivationTimes.reduce((sum, item) => sum + item, 0) /
                devActivationTimes.length;
            const releaseActivationAvgTime =
                releaseActivationTimes.reduce((sum, item) => sum + item, 0) /
                releaseActivationTimes.length;
            const languageServerActivationAvgTime =
                languageServerActivationTimes.reduce(
                    (sum, item) => sum + item,
                    0
            ...
            console.log(
                `Release version loaded in ${releaseActivationAvgTime}ms`
            );
            console.log(
                `Language Server loaded in ${languageServerActivationAvgTime}ms`
            );

            expect(
                devActivationAvgTime - releaseActivationAvgTime
            ).to.be.lessThan(
