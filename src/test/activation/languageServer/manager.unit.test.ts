...
suite("Language Server - Manager", () => {
    ...
    [undefined, Uri.file(__filename)].forEach(resource => {
        async function startLanguageServer() {
            when(
                commandManager.registerCommand(loadExtensionCommand, anything())
            ...
            when(analysisOptions.getAnalysisOptions()).thenResolve(
                languageClientOptions
            );
            when(analysisOptions.onDidChange).thenReturn(changeFn as any);
            when(
                serviceContainer.get<ILanaguageServer>(ILanaguageServer)
            ).thenReturn(instance(languageServer));
            when(
                languageServer.start(resource, languageClientOptions)
            ).thenResolve();
            ...
            commandRegistrationDisposable.verify(
                d => d.dispose(),
                typemoq.Times.never()
            );
        }
        ...
        test("Attempting to start LS will throw an exception", async () => {
            ...
            await expect(manager.start(resource)).to.eventually.be.rejectedWith(
