...
import {
    ...
    INugetService,
    NugetPackage
} from "../../../client/common/nuget/types";
...
suite("Languagex", () => {
    ...
    setup(() => {
        ...
        lsPackageService = new LanguageServerPackageService(
    ...
    [true, false].forEach(is64Bit => {
        ...
        test(`Get Package name for Mac (${bitness})`, async () => {
            ...
            const expectedName = `${downloadBaseFileName}-${
    ...
    test("Get latest nuget package version", async () => {
        ...
        serviceContainer
            .setup(c =>
                c.get(typeMoq.It.isValue(INugetRepository), typeMoq.It.isAny())
            )
            .returns(() => repo.object);
        serviceContainer
            .setup(c => c.get(typeMoq.It.isValue(INugetService)))
            .returns(() => nuget.object);

        repo.setup(n => n.getPackages(typeMoq.It.isValue(packageName)))
    ...
    test("Ensure minimum version of package is used", async () => {
        ...
        const expectedPackage: NugetPackage = {
            ...
            }/${packageName}.${minimumVersion}.nupkg`
        };
        expect(info).to.deep.equal(expectedPackage);
    });
});
