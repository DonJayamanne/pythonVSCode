...
import { InterpreterDataService } from "../../../client/activation/interpreterDataService";
...
suite("Language Server - Analysis Options", () => {
    class TestClass extends LanguageServerAnalysisOptions {
        ...
        public async notifyIfValuesHaveChanged(
            oldArray: string[],
            newArray: string[]
        ): Promise<void> {
    ...
    setup(() => {
        ...
        analysisOptions = new TestClass(
            ...
            instance(interpreterDataService),
            outputChannel,
            instance(pathUtils),
    ...
    test("Initialize will add event handlers and will dispose them when running dispose", async () => {
        ...
        when(workspace.onDidChangeConfiguration).thenReturn(
            () => disposable1.object
        );
    ...
    test("Changes to settings or interpreter will be debounced", async () => {
        ...
        when(interpreterService.onDidChangeInterpreter).thenReturn(cb => {
    ...
    test("If there are no changes then no events will be fired", async () => {
        when(interpreterDataService.getInterpreterData(undefined)).thenResolve({
            hash: ""
        } as any);
    ...
    test("Event must be fired if excluded files are different", async () => {
        when(
            interpreterDataService.getInterpreterData(undefined)
        ).thenResolve();
    ...
    test("Event must be fired if interpreter info is different", async () => {
        when(
            interpreterDataService.getInterpreterData({ hash: "1234" } as any)
        ).thenResolve();
