...
suite("Language Server Package Service", () => {
    ...
    test("Ensure new Major versions of Language Server is accounted for (azure blob)", async () => {
        ...
        serviceContainer
            .setup(c => c.get(typeMoq.It.isValue(INugetRepository)))
            .returns(() => nugetRepo);
        ...
        const lsPackageService = new LanguageServerPackageService(
            serviceContainer.object,
            appEnv.object,
            platform.object
        );
        ...
        expect(latestReleases).to.be.length.greaterThan(
            0,
            "No release packages returned."
        );
