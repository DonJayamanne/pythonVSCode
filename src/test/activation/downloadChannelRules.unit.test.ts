...
} from "../../client/activation/downloadChannelRules";
import {
    IPersistentState,
...
suite("Language Server Download Channel Rules", () => {
    [undefined, path.join("a", "b")].forEach(currentFolderPath => {
        const currentFolder = currentFolderPath
            ? { path: currentFolderPath, version: new SemVer("0.0.0") }
            : undefined;
        const testSuffix = ` (${
            currentFolderPath ? "with" : "without"
        } an existing Language Server Folder`;

        test(`Daily channel should always download ${testSuffix}`, async () => {
            const rule = new DownloadDailyChannelRule();
            expect(
                await rule.shouldLookForNewLanguageServer(currentFolder)
            ).to.be.equal(true, "invalid value");
        });
        ...
        test(`Stable channel should be download only if folder doesn't exist ${testSuffix}`, async () => {
            ...
            expect(
                await rule.shouldLookForNewLanguageServer(currentFolder)
            ).to.be.equal(hasExistingLSFolder, "invalid value");
        });
        ...
        suite("Betal channel", () => {
            ...
            test(`Should be download only if not checked previously ${testSuffix}`, async () => {
                const rule = new DownloadBetaChannelRule(
            ...
            test(`Should be download only if checked previously ${testSuffix}`, async () => {
                ...
                expect(
                    await rule.shouldLookForNewLanguageServer(currentFolder)
                ).to.be.equal(shouldDownload, "invalid value");
            });
        });
    });
});
