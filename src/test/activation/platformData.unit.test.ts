...
import {
    PlatformData,
    PlatformLSExecutables
} from "../../client/activation/platformData";
...
const testDataModuleName = [
    {
        ...
        isMac: false,
        isLinux: false,
        expectedName: PlatformLSExecutables.Windows
    },
    {
        isWindows: false,
        isMac: true,
        isLinux: false,
        expectedName: PlatformLSExecutables.MacOS
    },
    {
        isWindows: false,
        isMac: false,
        isLinux: true,
        expectedName: PlatformLSExecutables.Linux
    }
];
    ...
    test("Name and hash (Windows/Mac)", async () => {
        for (const t of testDataWinMac) {
            ...
            assert.equal(
                actual,
                t.expectedName,
                `${actual} does not match ${t.expectedName}`
            );
        }
    });
