...
suite("History output tests", () => {
    ...
    setup(() => {
        ...
        ioc.serviceManager.addSingletonInstance<IWebPanelProvider>(
            IWebPanelProvider,
            webPanelProvider.object
        );
        ...
        historyProvider = ioc.serviceManager.get<IHistoryProvider>(
            IHistoryProvider
        );
    ...
    function addContinuousMockData(
        code: string,
        resultGenerator: (
            c: CancellationToken
    ...
    function verifyHtmlOnLastCell(
        wrapper: ReactWrapper<any, Readonly<{}>, React.Component>,
        html: string
    ) {
        const foundResult = wrapper.find("Cell");
        assert.ok(
            foundResult.length >= 1,
            "Didn't find any cells being rendered"
        );
        ...
        assert.ok(
            outHtml.includes(sliced),
            `${outHtml} does not contain ${sliced}`
        );
    }

    function verifyLastCellInputState(
        wrapper: ReactWrapper<any, Readonly<{}>, React.Component>,
        ...
        switch (state) {
            ...
            case cellInputState.Expanded:
                assert.ok(
                    toggleButton.html().includes("collapse-input-svg-rotate"),
                    "Cell input toggle not expanded"
                );
                break;
            ...
            case cellInputState.Collapsed:
                assert.ok(
                    !toggleButton.html().includes("collapse-input-svg-rotate"),
                    "Cell input toggle not collapsed"
                );
                break;
    ...
    async function getCellResults(
        ...
        const renderPromise = waitForUpdate(
            wrapper,
            MainPanel,
            expectedRenders
        );
    ...
    runMountedTest("Hide / show cell", async wrapper => {
        ...
        updateDataScienceSettings(wrapper, {
            ...defaultDataScienceSettings(),
            showCellInputCode: false
        });
    ...
    function updateDataScienceSettings(
...
df.head()`;
        ...
        const matPlotLibResults = await fs.readFile(
            path.join(srcDirectory(), "matplotlib.txt"),
            "utf8"
        );
...
for _ in range(50):
    ...
    runMountedTest("Undo/redo commands", async wrapper => {
        ...
        assert.equal(
            afterUndo.length,
            0,
            `Undo should remove cells + ${afterUndo.debug()}`
        );
    ...
    runMountedTest("Click buttons", async wrapper => {
        ...
        visibleEditor
            .setup(v => v.show())
            .returns(() => showedEditor.resolve());
        visibleEditor
            .setup(v => v.revealRange(TypeMoq.It.isAny()))
            .returns(noop);
        visibleEditor
            .setup(v => v.document)
            .returns(() => dummyDocument.object);
        textEditors.push(visibleEditor.object);
        docManager.setup(a => a.visibleTextEditors).returns(() => textEditors);
        ioc.serviceManager.rebindInstance<IDocumentManager>(
            IDocumentManager,
            docManager.object
        );
        ...
        assert.ok(
            showedEditor.resolved,
            "Goto source is not jumping to editor"
        );
    ...
    runMountedTest("Export", async wrapper => {
        ...
        ioc.serviceManager.rebindInstance<IApplicationShell>(
            IApplicationShell,
            appShell.object
        );
        ...
        assert.equal(
            exportCalled,
            true,
            "Export is not being called during export"
        );
        ...
        exportCalled = false;
        const response = waitForMessageResponse(() =>
        ...
        assert.equal(
            exportCalled,
            false,
            "Export should not be called when no cells visible"
        );
    });
    ...
    runMountedTest("Editor Context", async wrapper => {
        ...
        assert.equal(
            ioc.getContext(EditorContexts.HaveRedoableCells),
            true,
            "Should have redoable after undo"
        );
