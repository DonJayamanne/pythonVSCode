...
suite("Jupyter notebook tests", () => {
    ...
    setup(() => {
        ...
        processFactory = ioc.serviceManager.get<IProcessServiceFactory>(
            IProcessServiceFactory
        );
        if (ioc.mockJupyter) {
            ioc.mockJupyter.addInterpreter(
                workingPython,
                SupportedCommands.all
            );
        }
    });
    ...
    async function verifySimple(
        jupyterServer: INotebookServer | undefined,
        code: string,
        expectedValue: any
    ): Promise<void> {
        const cells = await jupyterServer!.execute(
            code,
        ...
        if (data) {
            // For linter
            assert.ok(
                data.hasOwnProperty("text/plain"),
                `Cell mime type not correct`
            );
    ...
    async function verifyCell(
        ...
        assert.equal(
            cells.length,
            1,
            `${index}: Wrong number of cells returned`
        );
        if (cellType === "code") {
            assert.equal(
                cells[0].data.cell_type,
                cellType,
                `${index}: Wrong type of cell returned`
            );
            const cell = cells[0].data as nbformat.ICodeCell;
            assert.equal(
                cell.outputs.length,
                1,
                `${index}: Cell length not correct`
            );
            const error = cell.outputs[0].evalue;
            if (error) {
            ...
            if (data) {
                ...
                assert.ok(
                    ...
                    `${index}: Cell mime type not correct`
                );
                assert.ok(
                    data[mimeType],
                    `${index}: Cell mime type not correct`
    ...
    async function createNotebookServer(
        ...
        try {
            ...
            if (!expectFailure) {
                assert.ok(
                    false,
                    `Expected server to be created, but got ${exc}`
                );
            }
        }
    }
    ...
    function addInterruptableMockData(
        ...
        resultGenerator: (
            c: CancellationToken
        ) => Promise<{ result: string; haveMore: boolean }>
    ) {
        if (ioc.mockJupyter) {
    ...
    runTest("Failure", async () => {
        ...
        ioc.serviceManager.rebind<IJupyterExecution>(
            IJupyterExecution,
            FailedProcess
        );
        jupyterExecution = ioc.serviceManager.get<IJupyterExecution>(
    ...
    test("Not installed", async () => {
        ...
        class EmptyInterpreterService implements IInterpreterService {
            ...
            public getActiveInterpreter(
                resource?: Uri
            ): Promise<PythonInterpreter | undefined> {
                return Promise.resolve(undefined);
            }
            public getInterpreterDetails(
                pythonPath: string,
                resoure?: Uri
            ): Promise<PythonInterpreter> {
                throw new Error("Method not implemented");
            ...
            public getDisplayName(
                interpreter: Partial<PythonInterpreter>
            ): Promise<string> {
                throw new Error("Method not implemented");
            }
        ...
        ioc.serviceManager.rebind<IInterpreterService>(
            IInterpreterService,
            EmptyInterpreterService
        );
    ...
    runTest("Export/Import", async () => {
        ...
        const testState = generateTestState(id => {
            return;
        }, testFolderPath);
        const cells = testState.cellVMs.map(
        ...
        const exporter = ioc.serviceManager.get<INotebookExporter>(
            INotebookExporter
        );
        ...
        const notebook = await exporter.translateToNotebook(
            cells,
            newFolderPath
        );
        ...
        const importer = ioc.serviceManager.get<INotebookImporter>(
            INotebookImporter
        );
    ...
    async function testCancelableCall<T>(
        ...
        const tokenSource = new TaggedCancellationTokenSource(
            messageFormat.format(timeout.toString())
        );
        ...
        try {
            ...
            assert.ok(
                exc instanceof CancellationError,
                `Non cancellation error found : ${exc.stack}`
            );
        } finally {
            clearTimeout(disp);
            tokenSource.dispose();
    ...
    runTest("Cancel execution", async function() {
        ...
        assert.ok(
            await testCancelableMethod(
                (t: CancellationToken) =>
                    jupyterExecution.connectToNotebookServer(
                        undefined,
                        true,
                        t
                    ),
        ...
        const server = await jupyterExecution.connectToNotebookServer(
            undefined,
            true,
            nonCancelSource.token
        );
        assert.ok(
            server,
            "Server not found with a cancel token that does not cancel"
        );
        ...
        assert.ok(
            await testCancelableMethod(
                (t: CancellationToken) =>
                    jupyterExecution.getUsableJupyterPython(t),
        ...
        assert.ok(
            await testCancelableMethod(
                (t: CancellationToken) =>
                    jupyterExecution.isNotebookSupported(t),
        ...
        assert.ok(
            await testCancelableMethod(
                (t: CancellationToken) =>
                    jupyterExecution.isKernelCreateSupported(t),
                "Cancel did not cancel isKernel after {0}ms",
                true
            )
        );
        assert.ok(
    ...
    async function interruptExecute(
        server: INotebookServer | undefined,
        code: string,
        interruptMs: number,
        sleepMs: number
...
while keep_going:
  print(".")
  time.sleep(.1)`;
        const fourSecondSleep = `import time${os.EOL}time.sleep(4)${
            os.EOL
        }print("foo")`;
...
while keep_going:
  ...
  time.sleep(.1)`;
        ...
        addInterruptableMockData(
            returnable,
            async (cancelToken: CancellationToken) => {
        ...
        let interruptResult = await interruptExecute(
            server,
            returnable,
            1000,
            1000
        ...
        interruptResult = await interruptExecute(
            server,
            fourSecondSleep,
            7000,
            7000
...
df = pd.read("${escapePath(
                path.join(srcDirectory(), "DefaultSalesReport.csv")
            )}")
df.head()`,
            mimeType: "text/html",
            ...
            verifyValue: d =>
                assert.ok(
                    (d as string).includes("has no attribute 'read'"),
                    "Unexpected error result"
                )
        },
        {
            code: `import pandas as pd
df = pd.read_csv("${escapePath(
                path.join(srcDirectory(), "DefaultSalesReport.csv")
            )}")
df.head()`,
            mimeType: "text/html",
            ...
            cellType: "code",
            verifyValue: d =>
                assert.ok(d.toString().includes("</td>"), "Table not found")
        },
        {
            code: `#%% [markdown]#
# #HEADER`,
...
plt.show()`,
    ...
    async function getNotebookCapableInterpreter(): Promise<
        PythonInterpreter | undefined
    > {
        const is = ioc.serviceContainer.get<IInterpreterService>(
            IInterpreterService
        );
        ...
        if (procService) {
            for (let i = 0; i < list.length; i += 1) {
                const result = await procService.exec(
                    list[i].path,
                    ["-m", "jupyter", "notebook", "--version"],
                    { env: process.env }
                );
                if (!result.stderr) {
                    return list[i];
    ...
    async function generateNonDefaultConfig() {
        ...
        const match = /^.*\s+(.*jupyter_notebook_config.py)\s+.*$/m.exec(
            results.stdout
        );
        assert.ok(
    ...
    runTest("Non default config fails", async () => {
        if (!ioc.mockJupyter) {
            ...
            try {
                ...
                assert.fail(
                    "Should not be able to connect to notebook server with bad config"
                );
            } catch {
                noop();
            }
    ...
    runTest("Non default config does not mess up default config", async () => {
        if (!ioc.mockJupyter) {
            ...
            assert.ok(
                server,
                "Never connected to a default server with a bad default config"
            );
