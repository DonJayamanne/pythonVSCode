...
export class MockJupyterManager implements IJupyterSessionManager {
    private pythonExecutionFactory = this.createTypeMoq<
        IPythonExecutionFactory
    >("Python Exec Factory");
    private processServiceFactory = this.createTypeMoq<IProcessServiceFactory>(
        "Process Exec Factory"
    );
    private processService: MockProcessService = new MockProcessService();
    private interpreterService = this.createTypeMoq<IInterpreterService>(
        "Interpreter Service"
    );
    private asyncRegistry: IAsyncDisposableRegistry;
    private changedInterpreterEvent: EventEmitter<void> = new EventEmitter<
        void
    >();
    ...
    constructor(serviceManager: IServiceManager) {
        ...
        this.asyncRegistry = serviceManager.get<IAsyncDisposableRegistry>(
            IAsyncDisposableRegistry
        );
        ...
        this.processServiceFactory
            .setup(p => p.create())
            .returns(() => Promise.resolve(this.processService));
        ...
        this.interpreterService
            .setup(i => i.getActiveInterpreter())
        ...
        this.interpreterService
            ...
            .returns(p => {
                const found = this.installedInterpreters.find(
                    i => i.path === p
                );
                if (found) {
        ...
        const configService = serviceManager.get<IConfigurationService>(
            IConfigurationService
        );
        if (configService && configService !== null) {
            (configService.getSettings() as PythonSettings).addListener(
                "change",
        ...
        serviceManager.addSingletonInstance<IJupyterSessionManager>(
            IJupyterSessionManager,
            this
        );
        serviceManager.addSingletonInstance<IInterpreterService>(
            IInterpreterService,
            this.interpreterService.object
        );
    ...
    public addInterpreter(
        ...
        this.pythonExecutionFactory
            .setup(f =>
                f.create(
                    TypeMoq.It.is(o => {
                        return o && o.pythonPath
                            ? o.pythonPath === interpreter.path
                            : false;
                    })
                )
            )
            .returns(() => Promise.resolve(pythonService));
        this.setupSupportedPythonService(
            pythonService,
            interpreter,
            supportedCommands,
            notebookStdErr
        ...
        this.setupSupportedProcessService(
            interpreter,
            supportedCommands,
            notebookStdErr
        );
    ...
    public addContinuousOutputCell(
        ...
        cells.forEach(c => {
            const key = concatMultilineString(c.data.source).replace(
                LineFeedRegEx,
                ""
            );
    ...
    private setupSupportedPythonService(
        ...
        if (
            (supportedCommands & SupportedCommands.ipykernel) ===
            SupportedCommands.ipykernel
        ) {
            ...
            this.setupPythonServiceExec(
                ...
                () => {
                    ...
                    return Promise.resolve({
                        stdout: `somename ${path.dirname(spec)}`
                    });
                }
            );
        }
        if (
            (supportedCommands & SupportedCommands.nbconvert) ===
            SupportedCommands.nbconvert
        ) {
            this.setupPythonServiceExec(
                service,
                "jupyter",
        ...
        if (
            (supportedCommands & SupportedCommands.notebook) ===
            SupportedCommands.notebook
        ) {
            this.setupPythonServiceExec(
                service,
                "jupyter",
        ...
        if (
            (supportedCommands & SupportedCommands.kernelspec) ===
            SupportedCommands.kernelspec
        ) {
            this.setupPythonServiceExec(
                service,
                "jupyter",
            ...
            this.setupPythonServiceExec(
                ...
                ["kernelspec", "list"],
                () => {
                    const results = this.kernelSpecs
                        .map(k => {
                            return `  ${k.name}  ${k.dir}`;
                        })
                        .join(os.EOL);
    ...
    private addKernelSpec(pythonPath: string): string {
        ...
        this.kernelSpecs.push({
            name: `${this.kernelSpecs.length}Spec`,
            dir: `${path.dirname(spec)}`
        });
        return spec;
    }
    ...
    private setupPathProcessService(
        jupyterPath: string,
        service: MockProcessService,
        supportedCommands: SupportedCommands,
        notebookStdErr?: string[]
        ...
        this.setupProcessServiceExec(service, jupyterPath, ["--version"], () =>
            Promise.resolve({ stdout: "1.1.1.1" })
        );
        this.setupProcessServiceExec(service, "jupyter", ["--version"], () =>
            Promise.resolve({ stdout: "1.1.1.1" })
        );
