            ...
            if (resultGenerator) {
                ...
                return {
                    message: this.generateMessage(
                        streamResult.result.output_type,
                        streamResult.result
                    ),
...
export class MockJupyterRequest implements Kernel.IFuture {
    ...
    private deferred: Deferred<KernelMessage.IShellMessage> = createDeferred<
    ...
    private executeRequest(delay: number) {
        ...
        const outputProducers = outputs.map(
            o => new OutputMessageProducer(o, this.cancelToken)
        );
