...
suite("DataScience Code Watcher Unit Tests", () => {
    ...
    setup(() => {
        ...
        historyProvider
            ...
            .returns(() => activeHistory.object);

        // Setup our active text editor
        documentManager
            .setup(dm => dm.activeTextEditor)
    ...
    test("Add a file with just a #%% mark to a code watcher", () => {
        ...
        // Verify meta data
        expect(codeWatcher.getFileName()).to.be.equal(
            fileName,
        ...
        if (codeLenses[0].command) {
            expect(codeLenses[0].command.command).to.be.equal(
                Commands.RunCell,
                "Run Cell code lens command incorrect"
            );
        }
        expect(codeLenses[0].range).to.be.deep.equal(
            new Range(0, 0, 0, 3),
            "Run Cell code lens range incorrect"
        );
        if (codeLenses[1].command) {
            expect(codeLenses[1].command.command).to.be.equal(
                Commands.RunAllCells,
                "Run All Cells code lens command incorrect"
            );
        }
        expect(codeLenses[1].range).to.be.deep.equal(
            new Range(0, 0, 0, 3),
            "Run All Cells code lens range incorrect"
        );
...
fourth line`;
        ...
        if (codeLenses[2].command) {
            expect(codeLenses[2].command.command).to.be.equal(
                Commands.RunCell,
                "Run Cell code lens command incorrect"
            );
        }
        expect(codeLenses[2].range).to.be.deep.equal(
            new Range(6, 0, 7, 11),
            "Run Cell code lens range incorrect"
        );
        if (codeLenses[3].command) {
            expect(codeLenses[3].command.command).to.be.equal(
                Commands.RunAllCells,
                "Run All Cells code lens command incorrect"
            );
        }
        expect(codeLenses[3].range).to.be.deep.equal(
            new Range(6, 0, 7, 11),
            "Run All Cells code lens range incorrect"
        );
    ...
    test("Test the RunCell command", async () => {
        ...
        document
            .setup(doc => doc.getText(testRange))
            .returns(() => testString)
...
testing2`; // Command tests override getText, so just need the ranges here
        ...
        document
            .setup(doc => doc.getText(testRange1))
            .returns(() => testString1)
            .verifiable(TypeMoq.Times.once());
        const testRange2 = new Range(2, 0, 3, 8);
        const testString2 = "testing2";
        document
            .setup(doc => doc.getText(testRange2))
            .returns(() => testString2)
            .verifiable(TypeMoq.Times.once());
        ...
        activeHistory
            .setup(h =>
                h.addCode(
                    TypeMoq.It.isValue(testString1),
        ...
        activeHistory
            .setup(h =>
                h.addCode(
                    TypeMoq.It.isValue(testString2),
                    TypeMoq.It.isValue("test.py"),
...
testing2`;
        ...
        document
            .setup(d => d.getText(new Range(2, 0, 3, 8)))
            .returns(() => "testing2")
        ...
        textEditor
            .setup(te => te.selection)
            .returns(() => new Selection(2, 0, 2, 0));
