...
suite("Interpreters - Locators Helper", () => {
            ...
            interpreterServiceHelper
                .setup(i =>
                    i.isMacDefaultPythonPath(
                        TypeMoq.It.isValue(interpreter.path)
                    )
                )
                .returns(() => name === "mac")
                .verifiable(TypeMoq.Times.never());
    ...
    getNamesAndValues<OS>(OS).forEach(os => {
        test(`Ensure duplicates are removed (same version and same interpreter directory on ${
            os.name
        })`, async () => {
            interpreterServiceHelper
            ...
            ["3.6", "3.6", "2.7", "2.7"].forEach((name, index) => {
                const interpreter = {
                    ...
                    type: InterpreterType.Unknown,
                    version: new SemVer(
                        `3.${parseInt(name.substr(-1), 10)}.0-final`
                    )
                };
                interpreters.push(interpreter);
    ...
    getNamesAndValues<OS>(OS).forEach(os => {
        test(`Ensure interpreter types are identified from other locators (${
            os.name
        })`, async () => {
            interpreterServiceHelper
                .setup(i => i.isMacDefaultPythonPath(TypeMoq.It.isAny()))
            ...
            fs.setup(f =>
                ...
                .returns(
                    (a, b) =>
                        a === b && a === path.join("users", "python", "bin")
                )
                .verifiable(TypeMoq.Times.atLeastOnce());
