...
suite("Interpreters - Workspace VirtualEnv Service", function() {
    ...
    async function waitForInterpreterToBeDetected(envNameToLookFor: string) {
        ...
        await waitForCondition(
            predicate,
            timeoutMs,
            `${envNameToLookFor}, Environment not detected in the workspace ${
                workspaceUri.fsPath
            }`
        );
    }
    async function createVirtualEnvironment(envSuffix: string) {
        ...
        const envName = `${venvPrefix}${envSuffix}${new Date()
            .getTime()
            .toString()}`;
            ...
            exec(
                ...
                (ex, _, stderr) => {
                    ...
                    if (stderr && stderr.length > 0) {
                        reject(
                            new Error(
                                `Failed to create Env ${envName}, ${PYTHON_PATH}, Error: ${stderr}`
                            )
                        );
                    } else {
                        resolve(envName);
    ...
    suiteTeardown(async () =>
        deleteFiles(path.join(workspaceUri.fsPath, `${venvPrefix}*`))
    );
    teardown(async () =>
        deleteFiles(path.join(workspaceUri.fsPath, `${venvPrefix}*`))
    );
