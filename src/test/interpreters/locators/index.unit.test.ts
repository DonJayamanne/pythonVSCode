...
suite("Interpreters - Locators Index", () => {
    ...
    [undefined, Uri.file("Something")].forEach(resource => {
        getNamesAndValues<OSType>(OSType).forEach(osType => {
            ...
            const testSuffix = `(on ${osType.name}, with${
                resource ? "" : "out"
            } a resource)`;
            test(`All Interpreter Sources are used ${testSuffix}`, async () => {
                ...
                platformSvc
                    .setup(p => p.isWindows)
                    .returns(() => osType.value === OSType.Windows);
                platformSvc
                    .setup(p => p.isLinux)
                    .returns(() => osType.value === OSType.Linux);
                platformSvc
                    .setup(p => p.isMac)
                    .returns(() => osType.value === OSType.OSX);
                ...
                const locatorsWithInterpreters = locatorsTypes.map(typeName => {
                    ...
                    typeLocator
                        .setup(l =>
                            l.getInterpreters(TypeMoq.It.isValue(resource))
                        )
                    ...
                    serviceContainer
                        .setup(c =>
                            c.get(
                                TypeMoq.It.isValue(IInterpreterLocatorService),
                                TypeMoq.It.isValue(typeName)
                            )
                        )
                ...
                helper
                    ...
                    .returns(() =>
                        locatorsWithInterpreters.map(
                            item => item.interpreters[0]
                        )
                    )
                    .verifiable(TypeMoq.Times.once());
            ...
            test(`Interpreter Sources are sorted correctly and merged ${testSuffix}`, async () => {
                ...
                const expectedInterpreters = locatorsWithInterpreters.map(
                    item => item.interpreters[0]
                );
                helper
                    .setup(h => h.mergeInterpreters(TypeMoq.It.isAny()))
