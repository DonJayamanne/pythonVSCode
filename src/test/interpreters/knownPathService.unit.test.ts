...
suite("Interpreters Known Paths", () => {
    ...
    setup(async () => {
        ...
        serviceContainer
            .setup(c =>
                c.get(TypeMoq.It.isValue(ICurrentProcess), TypeMoq.It.isAny())
            )
        ...
        serviceContainer
            .setup(c =>
                c.get(TypeMoq.It.isValue(IPathUtils), TypeMoq.It.isAny())
            )
            .returns(() => pathUtils.object);

        knownSearchPaths = new KnownSearchPathsForInterpreters(
    ...
    test("Ensure known list of paths are returned", async () => {
        ...
        const expectedPaths = [...pathsInPATHVar].filter(
            item => item.length > 0
        );
