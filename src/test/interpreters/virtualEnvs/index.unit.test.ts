...
suite("Virtual Environment Manager", () => {
    ...
    setup(() => {
        ...
        serviceContainer
            .setup(c => c.get(TypeMoq.It.isValue(IPipEnvService)))
    ...
    test("Get PyEnv Root from current PYENV_ROOT", async () => {
        ...
        processService
            .setup(p =>
                p.exec(
                    TypeMoq.It.isValue("pyenv"),
                    TypeMoq.It.isValue(["root"])
                )
            )
    ...
    test("Get Environment Type, detects venv", async () => {
        ...
        fs.setup(f =>
            f.fileExists(TypeMoq.It.isValue(path.join(dir, "pyvenv.cfg")))
        )
    ...
    for (const isWindows of [true, false]) {
        const testTitleSuffix = `(${
            isWindows ? "On Windows" : "Non-Windows"
        }})`;
        test(`Get Environment Type, detects virtualenv ${testTitleSuffix}`, async () => {
            ...
            terminalActivation
                .setup(t =>
                    t.getActivationCommandsForInterpreter!(
                        TypeMoq.It.isValue(pythonPath),
                        TypeMoq.It.isAny()
                    )
                )
            ...
            const isRecognized = await virtualEnvMgr.isVirtualEnvironment(
                pythonPath
            );
        ...
        test(`Get Environment Type, does not detect virtualenv incorrectly ${testTitleSuffix}`, async () => {
            ...
            let isRecognized = await virtualEnvMgr.isVirtualEnvironment(
                pythonPath
            );
