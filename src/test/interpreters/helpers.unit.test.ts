...
suite("Interpreters Display Helper", () => {
    ...
    test("getActiveWorkspaceUri should return undefined if we no active editor and have more than one workspace folder", () => {
        ...
        workspaceService
            .setup(w => w.workspaceFolders)
            .returns(() => [{ uri: folderUri } as any, undefined as any]);
    ...
    test("getBestInterpreter should return undefined for an empty list", () => {
        expect(helper.getBestInterpreter([])).to.be.equal(
            undefined,
            "should be undefined"
        );
        expect(helper.getBestInterpreter(undefined)).to.be.equal(
            undefined,
            "should be undefined"
        );
    });
    test("getBestInterpreter should return first item if there is only one", () => {
        expect(helper.getBestInterpreter(["a"] as any)).to.be.equal(
            "a",
            "should be undefined"
        );
    });
    test("getBestInterpreter should return interpreter with highest version", () => {
        const interpreter1 = {
            version: JSON.parse(JSON.stringify(new SemVer("1.0.0-alpha")))
        };
        const interpreter2 = {
            version: JSON.parse(JSON.stringify(new SemVer("3.6.0")))
        };
        const interpreter3 = {
            version: JSON.parse(JSON.stringify(new SemVer("3.7.1-alpha")))
        };
        const interpreter4 = {
            version: JSON.parse(JSON.stringify(new SemVer("3.6.0-alpha")))
        };
        const interpreters = [
            interpreter1,
            interpreter2,
        ...
        expect(helper.getBestInterpreter(interpreters)).to.be.deep.equal(
            interpreter3
        );
    });
});
