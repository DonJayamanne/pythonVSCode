...
suite("Virtual environments", () => {
    ...
    setup(() => {
        ...
        serviceManager.addSingletonInstance<ICurrentProcess>(
            ICurrentProcess,
            process.object
        );
    ...
    test("Global search paths", async () => {
        ...
        virtualEnvMgr
            .setup(v => v.getPyEnvRoot(TypeMoq.It.isAny()))
        ...
        expect(paths).to.deep.equal(
            expected,
            "Global search folder list is incorrect."
        );

        virtualEnvMgr.reset();
        virtualEnvMgr
            .setup(v => v.getPyEnvRoot(TypeMoq.It.isAny()))
            .returns(() => Promise.resolve("pyenv_path"));
        paths = await pathProvider.getSearchPaths();

        virtualEnvMgr.verifyAll();
        expected = expected.concat([
            "pyenv_path",
            path.join("pyenv_path", "versions")
        ]);
        expect(paths).to.deep.equal(
            expected,
            "pyenv path not resolved correctly."
        );
    });
    ...
    test("Workspace search paths", async () => {
        ...
        workspace
            ...
            .returns(() => wsRoot.object);
        workspace
            .setup(x => x.workspaceFolders)
            .returns(() => [wsRoot.object, folder1.object]);

        const pathProvider = new WorkspaceVirtualEnvironmentsSearchPathProvider(
            serviceContainer
        );
        ...
        const fixCase = (item: string) =>
            isWindows ? item.toUpperCase() : item;
        const expected = [
            path.join(homedir, "foo"),
            "root",
            path.join("root", ".direnv")
        ]
            .map(item => Uri.file(item).fsPath)
            .map(fixCase);
        expect(paths.map(fixCase)).to.deep.equal(
            expected,
            "Workspace venv folder search list does not match."
        );
    });
});
