...
suite("Interpreters Display", () => {
    ...
    setup(() => {
        ...
        pathUtils
            .setup(p =>
                p.getDisplayName(TypeMoq.It.isAny(), TypeMoq.It.isAny())
            )
            .returns(p => p);
    ...
    test("Display name and tooltip must come from interpreter info", async () => {
        ...
        interpreterService
            .setup(i =>
                i.getActiveInterpreter(TypeMoq.It.isValue(workspaceFolder))
            )
    ...
    test("If interpreter is not identified then tooltip should point to python Path", async () => {
        ...
        statusBar.verify(
            s => (s.tooltip = TypeMoq.It.isValue(pythonPath)),
            TypeMoq.Times.once()
        );
        statusBar.verify(
            s => (s.text = TypeMoq.It.isValue(displayName)),
            TypeMoq.Times.once()
        );
    });
    test("If interpreter file does not exist then update status bar accordingly", async () => {
        ...
        interpreterHelper
            .setup(v =>
                v.getInterpreterInformation(TypeMoq.It.isValue(pythonPath))
            )
            .returns(() => Promise.resolve(undefined));
        virtualEnvMgr
        ...
        statusBar.verify(
            s => (s.color = TypeMoq.It.isValue("yellow")),
            TypeMoq.Times.once()
        );
        statusBar.verify(
            s =>
                (s.text = TypeMoq.It.isValue(
                    "$(alert) Select Python Interpreter"
                )),
            TypeMoq.Times.once()
        );
    });
