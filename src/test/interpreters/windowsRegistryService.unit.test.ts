...
suite("Interpreters from Windows Registry (unit)", () => {
    ...
    setup(() => {
        ...
        serviceContainer
            .setup(c => c.get(TypeMoq.It.isValue(IPathUtils)))
    ...
    test("Must return a single entry", async () => {
        ...
        assert.equal(
            interpreters[0].architecture,
            Architecture.x86,
            "Incorrect arhictecture"
        );
        assert.equal(
            interpreters[0].companyDisplayName,
            "Display Name for Company One",
    ...
    test("Must default names for PythonCore and exe", async () => {
        ...
        assert.equal(
            interpreters[0].path,
            path.join(environmentsPath, "path1", "python.exe"),
            "Incorrect path"
        );
        assert.equal(
            interpreters[0].version!.raw,
    ...
    test("Must return a single entry and when registry contains only the InstallPath", async () => {
        ...
        const registryValues = [
            {
                key:
                    "\\Software\\Python\\Company One\\9.9.9-final\\InstallPath",
                hive: RegistryHive.HKCU,
                arch: Architecture.x86,
                value: path.join(environmentsPath, "path1")
            }
    ...
    test("Must return multiple entries", async () => {
        const registryKeys = [
            ...
            },
            {
                ...
                arch: Architecture.x86,
                values: [
                    "\\Software\\Python\\Company One\\1.0.0",
                    "\\Software\\Python\\Company One\\2.0.0"
                ]
            },
            {
        ...
        assert.equal(
            interpreters[1].path,
            path.join(environmentsPath, "path2", "python.exe"),
            "Incorrect path"
        );
        assert.equal(
            interpreters[1].version!.raw,
            "2.0.0",
            "Incorrect version"
        );

        assert.equal(
            interpreters[2].architecture,
            Architecture.x86,
            "Incorrect arhictecture"
        );
        assert.equal(
            interpreters[2].companyDisplayName,
            "Company Two",
            "Incorrect company name"
        );
        assert.equal(
        ...
        assert.equal(
            interpreters[2].version!.raw,
            "4.0.0",
            "Incorrect version"
        );

        assert.equal(
            interpreters[3].architecture,
            Architecture.x86,
            "Incorrect arhictecture"
        );
        assert.equal(
            interpreters[3].companyDisplayName,
            "Company Two",
            "Incorrect company name"
        );
        assert.equal(
    ...
    test("Must return multiple entries excluding the invalid registry items and nonexistent paths", async () => {
        const registryKeys = [
            ...
            },
            {
                ...
                values: [
                    "\\Software\\Python\\Company One\\1.0.0",
                    "\\Software\\Python\\Company One\\Tag2"
                ]
            },
            {
                key: "\\Software\\Python\\Company Two",
