...
suite("Interpreters - PipEnv", () => {
    const rootWorkspace = Uri.file(path.join("usr", "desktop", "wkspc1"))
        .fsPath;
    getNamesAndValues(OS).forEach(os => {
        [undefined, Uri.file(path.join(rootWorkspace, "one.py"))].forEach(
            resource => {
                ...
                let persistentStateFactory: TypeMoq.IMock<
                    IPersistentStateFactory
                >;
                let envVarsProvider: TypeMoq.IMock<
                ...
                setup(() => {
                    ...
                    persistentStateFactory
                        .setup(p =>
                            p.createWorkspacePersistentState(
                                TypeMoq.It.isAny(),
                                TypeMoq.It.isAny()
                            )
                        )
                    ...
                    serviceContainer
                        .setup(c =>
                            c.get(TypeMoq.It.isValue(IWorkspaceService))
                        )
                        .returns(() => workspaceService.object);
                    serviceContainer
                        .setup(c =>
                            c.get(TypeMoq.It.isValue(IInterpreterHelper))
                        )
                    ...
                    serviceContainer
                        .setup(c =>
                            c.get(TypeMoq.It.isValue(IApplicationShell))
                        )
                        .returns(() => appShell.object);
                    serviceContainer
                        .setup(c =>
                            c.get(TypeMoq.It.isValue(IPersistentStateFactory))
                        )
                    ...
                    config
                        .setup(c =>
                            c.getSettings(TypeMoq.It.isValue(undefined))
                        )
                        .returns(() => settings.object);
                    settings
                        .setup(p => p.pipenvPath)
                        .returns(() => pipenvPathSetting);
                ...
                test(`Should return an empty list'${testSuffix}`, () => {
                    const environments = pipEnvService.getInterpreters(
                        resource
                    );
                ...
                test(`Should return an empty list if there is no \'PipFile\'${testSuffix}`, async () => {
                    ...
                    envVarsProvider
                        .setup(e =>
                            e.getEnvironmentVariables(TypeMoq.It.isAny())
                        )
                        .returns(() => Promise.resolve({}))
                    ...
                    fileSystem
                        .setup(fs =>
                            fs.fileExists(
                                TypeMoq.It.isValue(
                                    path.join(rootWorkspace, "Pipfile")
                                )
                            )
                        )
                        .returns(() => Promise.resolve(false))
                        .verifiable(TypeMoq.Times.once());
                    const environments = await pipEnvService.getInterpreters(
                        resource
                    );
                ...
                test(`Should display warning message if there is a \'PipFile\' but \'pipenv --venv\' failes ${testSuffix}`, async () => {
                    ...
                    logger
                        .setup(l =>
                            l.logWarning(TypeMoq.It.isAny(), TypeMoq.It.isAny())
                        )
                ...
                test(`Should return interpreter information${testSuffix}`, async () => {
                    ...
                    interpreterHelper
                        .setup(v =>
                            v.getInterpreterInformation(TypeMoq.It.isAny())
                        )
                        .returns(() =>
                            Promise.resolve({ version: new SemVer("1.0.0") })
                        );
                    fileSystem
                        .setup(fs =>
                            fs.fileExists(
                    ...
                    fileSystem
                        .setup(fs =>
                            fs.fileExists(TypeMoq.It.isValue(pythonPath))
                        )
                        .returns(() => Promise.resolve(true))
                        .verifiable();
                    const environments = await pipEnvService.getInterpreters(
                ...
                test(`Should return interpreter information using PipFile defined in Env variable${testSuffix}`, async () => {
                    ...
                    fileSystem
                        .setup(fs =>
                            fs.fileExists(
                                TypeMoq.It.isValue(
                                    path.join(rootWorkspace, envPipFile)
                                )
                            )
                        )
                        .returns(() => Promise.resolve(true))
                ...
                test("Must use 'python.pipenvPath' setting", async () => {
                    ...
                    assert.equal(
                        pipenvExe,
                        "spam-spam-pipenv-spam-spam",
                        "Failed to identify pipenv.exe"
                    );
                });
            }
        );
