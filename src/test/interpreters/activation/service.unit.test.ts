...
suite("Interprters Activation - Python Environment Variables", () => {
    ...
    function initSetup() {
        ...
        when(envVarsService.onDidEnvironmentVariablesChange).thenReturn(
            noop as any
        );
    ...
    [undefined, Uri.parse("a")].forEach(resource => {
        suite(resource ? "With a resource" : "Without a resource", () => {
            ...
            test("Unknown os will return empty variables", async () => {
                ...
                const env = await service.getActivatedEnvironmentVariables(
                    resource
                );
                ...
                expect(env).to.equal(
                    undefined,
                    "Should not have any variables"
                );
            });

            const osTypes = getNamesAndValues<OSType>(OSType).filter(
            ...
            osTypes.forEach(osType => {
                suite(osType.name, () => {
                    ...
                    test("getEnvironmentActivationShellCommands will be invoked", async () => {
                        ...
                        when(
                            helper.getEnvironmentActivationShellCommands(
                                resource
                            )
                        ).thenResolve();
                    ...
                    test("Validate command used to activation and printing env vars", async () => {
                        ...
                        when(
                            processServiceFactory.create(resource)
                        ).thenResolve(instance(processService));
                        when(
                            envVarsService.getEnvironmentVariables(resource)
                        ).thenResolve(envVars);
                        ...
                        verify(
                            processService.shellExec(anything(), anything())
                        ).once();

                        const shellCmd = capture(
                            processService.shellExec
                        ).first()[0];
                    ...
                    test("Return parsed variables", async () => {
                        ...
                        const stdout = `${getEnvironmentPrefix}${EOL}${JSON.stringify(
                            varsFromEnv
                        )}`;
                        when(platform.osType).thenReturn(osType.value);
                        when(
