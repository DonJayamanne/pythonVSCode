    ...
    public async getKeys(
        ...
        const items = this.keys.find(item => {
            if (typeof item.arch === "number") {
                return (
                    item.key === key && item.hive === hive && item.arch === arch
                );
            }
...
@injectable()
export class MockInterpreterVersionProvider
    ...
    public async getVersion(
        ...
        return this.useDefaultDisplayName
            ? Promise.resolve(defaultDisplayName)
