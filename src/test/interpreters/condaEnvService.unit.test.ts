...
import {
    CondaEnvService,
    parseCondaInfo
} from "../../client/interpreter/locators/services/condaEnvService";
...
suite("Interpreters from Conda Environments", () => {
    ...
    async function extractDisplayNameFromVersionInfo(isWindows: boolean) {
        ...
        info.envs.forEach(validPath => {
            const pythonPath = isWindows
                ? path.join(validPath, "python.exe")
                : path.join(validPath, "bin", "python");
            fileSystem
        ...
        assert.equal(
            ...
            "Environment not detected as a conda environment"
        );
        ...
        assert.equal(
            interpreters[1].companyDisplayName,
            AnacondaCompanyName,
            "Incorrect company display name for first env"
        );
        assert.equal(
    ...
    async function extractDisplayNameFromVersionInfoSuffixedWithEnvironmentName(
        isWindows: boolean
    ) {
        const info = {
            envs: [
    ...
    test("Must extract display name from version info suffixed with the environment name (oxs/linux)", async () => {
        await extractDisplayNameFromVersionInfoSuffixedWithEnvironmentName(
            false
        );
    });
    ...
    async function useDefaultNameIfSysVersionIsValidAndSuffixWithEnvironmentName(
        isWindows: boolean
    ) {
        const info = {
            envs: [path.join(environmentsPath, "conda", "envs", "numpy")],
    ...
    test("Must use the default display name if sys.version is invalid and suffixed with environment name (non windows)", async () => {
        await useDefaultNameIfSysVersionIsValidAndSuffixWithEnvironmentName(
            false
        );
    });
    ...
    async function useDefaultNameIfSysVersionIsEmptyAndSuffixWithEnvironmentName(
        isWindows: boolean
    ) {
        const info = {
            envs: [path.join(environmentsPath, "conda", "envs", "numpy")]
    ...
    test("Must use the default display name if sys.version is empty and suffixed with environment name (non windows)", async () => {
        await useDefaultNameIfSysVersionIsEmptyAndSuffixWithEnvironmentName(
            false
        );
    });
    ...
    async function includeDefaultPrefixIntoListOfInterpreters(
        isWindows: boolean
    ) {
        const info = {
            default_prefix: path.join(
    ...
    async function excludeInterpretersThatDoNotExistOnFileSystem(
        isWindows: boolean
    ) {
        const info = {
            envs: [
