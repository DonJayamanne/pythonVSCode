...
suite("Virtual environment manager", () => {
    ...
    test("Plain Python environment suffix", async () =>
        testSuffix(virtualEnvFolderName));
    test("Plain Python environment suffix with workspace Uri", async () =>
        testSuffix(
            virtualEnvFolderName,
            false,
            Uri.file(path.join("1", "2", "3", "4"))
        ));
    test("Plain Python environment suffix with PipEnv", async () =>
        testSuffix(
            "workspaceName",
            true,
            Uri.file(path.join("1", "2", "3", "workspaceName"))
        ));
    ...
    test("Use environment folder as env name", async () => {
        ...
        const venvManager = new VirtualEnvironmentManager(
    ...
    test("Use workspacec name as env name", async () => {
        ...
        serviceContainer
            ...
            .returns(
                () => TypeMoq.Mock.ofType<IProcessServiceFactory>().object
            );
        serviceContainer
            .setup(s => s.get(TypeMoq.It.isValue(IPipEnvService)))
        ...
        const workspaceUri = Uri.file(
            path.join("root", "sub", "wkspace folder")
        );
    ...
    async function testSuffix(
        ...
        isPipEnvironment: boolean = false,
        resource?: Uri
    ) {
        const serviceContainer = TypeMoq.Mock.ofType<IServiceContainer>();
        serviceContainer
        ...
        expect(name).to.be.equal(
            expectedEnvName,
            "Virtual envrironment name suffix is incorrect."
        );
    }
});
