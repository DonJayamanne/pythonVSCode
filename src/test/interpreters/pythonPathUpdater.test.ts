...
suite("Python Path Settings Updater", () => {
    ...
    function setupMocks() {
        ...
        updaterServiceFactory = new PythonPathUpdaterServiceFactory(
            serviceContainer.object
        );
    }
    function setupConfigProvider(
        resource?: Uri
    ): TypeMoq.IMock<WorkspaceConfiguration> {
    ...
    suite("WorkspaceFolder", () => {
        ...
        test("Python Path should not be updated when current pythonPath is the same", async () => {
            ...
            const updater = updaterServiceFactory.getWorkspaceFolderPythonPathConfigurationService(
                workspaceFolder
            );
        ...
        test("Python Path should be truncated for worspace-relative paths", async () => {
            ...
            const pythonPath = Uri.file(
                path.join(workspaceFolderPath, "env", "bin", "python")
            ).fsPath;
    ...
    suite("Workspace (multiroot scenario)", () => {
        ...
        test("Python Path should not be updated when current pythonPath is the same", async () => {
            ...
            const workspaceFolder = Uri.file(workspaceFolderPath);
            const updater = updaterServiceFactory.getWorkspacePythonPathConfigurationService(
                workspaceFolder
            );
            const pythonPath = `xWorkspaceFolderPythonPath${new Date().getMilliseconds()}`;
            const workspaceConfig = setupConfigProvider(workspaceFolder);
