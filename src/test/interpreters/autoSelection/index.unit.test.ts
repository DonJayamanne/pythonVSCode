...
suite("Interpreters - Auto Selection", () => {
    ...
    class InterpreterAutoSelectionServiceTest extends InterpreterAutoSelectionService {
        ...
        public storeAutoSelectedInterperter(
            resource: Resource,
    ...
    test("Rules are chained in order of preference", () => {
        verify(
            userDefinedInterpreter.setNextRule(instance(workspaceInterpreter))
        ).once();
        verify(workspaceInterpreter.setNextRule(instance(cachedPaths))).once();
        verify(
            cachedPaths.setNextRule(instance(currentPathInterpreter))
        ).once();
        verify(
            currentPathInterpreter.setNextRule(instance(winRegInterpreter))
        ).once();
        verify(
            winRegInterpreter.setNextRule(instance(systemInterpreter))
        ).once();
    ...
    test("Run rules in background", async () => {
        ...
        verify(
            userDefinedInterpreter.autoSelectInterpreter(
                anything(),
                autoSelectionService
            )
        ).once();
    ...
    test("Store interpreter info in state store when resource is undefined", async () => {
        ...
        await autoSelectionService.initializeStore();
        await autoSelectionService.storeAutoSelectedInterperter(
            undefined,
            interpreterInfo
        );
        const selectedInterpreter = autoSelectionService.getAutoSelectedInterpreter(
            undefined
        );
    ...
    test("Store interpreter info in state store when resource is defined", async () => {
        ...
        when(workspaceService.getWorkspaceFolder(resource)).thenReturn({
            name: "",
            index: 0,
            uri: resource
        });
    ...
    test("Store workspace interpreter info in state store", async () => {
        ...
        await autoSelectionService.initializeStore();
        await autoSelectionService.setWorkspaceInterpreter(
            resource,
            interpreterInfo
        );
        const selectedInterpreter = autoSelectionService.getAutoSelectedInterpreter(
    ...
    test("Return undefined when we do not have a global value", async () => {
        ...
        verify(state.updateValue(interpreterInfo)).never();
        expect(
            selectedInterpreter === null || selectedInterpreter === undefined
        ).to.equal(true, "Should be undefined");
    });
    test("Return global value if we do not have a matching value for the resource", async () => {
        ...
        const anotherResourceOfAnotherWorkspace = Uri.parse(
            "Some other workspace"
        );
        const selectedInterpreter = autoSelectionService.getAutoSelectedInterpreter(
            anotherResourceOfAnotherWorkspace
        );

        verify(workspaceService.getWorkspaceFolder(resource)).once();
        verify(
            workspaceService.getWorkspaceFolder(
                anotherResourceOfAnotherWorkspace
            )
        ).once();
        verify(state.updateValue(interpreterInfo)).never();
    ...
    test("setWorkspaceInterpreter will invoke storeAutoSelectedInterperter with same args", async () => {
        ...
        const interpreterInfo = { path: pythonPath } as any;
        const resource = Uri.parse("one");
        const moq = typemoq.Mock.ofInstance(
            autoSelectionService,
            typemoq.MockBehavior.Loose,
            false
        );
        moq.setup(m =>
            m.storeAutoSelectedInterperter(
                typemoq.It.isValue(resource),
                typemoq.It.isValue(interpreterInfo)
            )
        )
