...
import {
    BaseRuleService,
    NextAction
} from "../../../../client/interpreter/autoSelection/rules/baseRule";
...
suite("Interpreters - Auto Selection - Base Rule", () => {
    ...
    test("Get undefined if there's nothing in state store", async () => {
        ...
        expect(
            rule.getPreviouslyAutoSelectedInterpreter(Uri.parse("x"))
        ).to.be.equal(undefined, "Must be undefined");
    ...
    test("setGlobalInterpreter should do nothing if interprter is undefined or version is empty", async () => {
        ...
        const result1 = await rule.setGlobalInterpreter(
            undefined,
            instance(manager)
        );
        const result2 = await rule.setGlobalInterpreter(
            interpreterInfo,
            instance(manager)
        );
    ...
    test("setGlobalInterpreter should not update manager if interpreter is not better than one stored in manager", async () => {
        ...
        const interpreterInfoInManager = {
            path: "2",
            version: new SemVer("2.0.0")
        } as any;
        when(manager.getAutoSelectedInterpreter(undefined)).thenReturn(
            interpreterInfoInManager
        );

        const result = await rule.setGlobalInterpreter(
            interpreterInfo,
            instance(manager)
        );
