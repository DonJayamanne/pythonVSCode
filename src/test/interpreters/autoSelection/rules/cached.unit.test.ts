...
suite("Interpreters - Auto Selection - Cached Rule", () => {
    ...
    test("Invoke next rule if there are no cached intepreters", async () => {
        ...
        when(
            systemInterpreter.getPreviouslyAutoSelectedInterpreter(resource)
        ).thenReturn(undefined);
        when(
            currentPathInterpreter.getPreviouslyAutoSelectedInterpreter(
                resource
            )
        ).thenReturn(undefined);
        when(
            winRegInterpreter.getPreviouslyAutoSelectedInterpreter(resource)
        ).thenReturn(undefined);

        const nextAction = await rule.onAutoSelectInterpreter(
            resource,
            manager
        );
    ...
    test("Invoke next rule if fails to update global state", async () => {
        ...
        when(helper.getBestInterpreter(deepEqual(anything()))).thenReturn(
            winRegInterpreterInfo
        );
        when(
            systemInterpreter.getPreviouslyAutoSelectedInterpreter(anything())
        ).thenReturn(undefined);
        when(
            currentPathInterpreter.getPreviouslyAutoSelectedInterpreter(
                anything()
        ...
        when(
            winRegInterpreter.getPreviouslyAutoSelectedInterpreter(anything())
        ).thenReturn(winRegInterpreterInfo);
        ...
        moq.callBase = true;
        moq.setup(m =>
            m.setGlobalInterpreter(typemoq.It.isAny(), typemoq.It.isAny())
        )
            .returns(() => Promise.resolve(false))
            .verifiable(typemoq.Times.once());

        const nextAction = await moq.object.onAutoSelectInterpreter(
            resource,
            manager
        );
