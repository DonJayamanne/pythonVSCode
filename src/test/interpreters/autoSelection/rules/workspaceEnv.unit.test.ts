...
suite("Interpreters - Auto Selection - Workspace Virtual Envs Rule", () => {
    ...
    class WorkspaceVirtualEnvInterpretersAutoSelectionRuleTest extends WorkspaceVirtualEnvInterpretersAutoSelectionRule {
        ...
        public async cacheSelectedInterpreter(
            resource: Resource,
    ...
    test("Invoke next rule if user has defined a python path in settings", async () => {
        ...
        when(workspaceService.getConfiguration("python", folderUri)).thenReturn(
            pythonPath as any
        );
    ...
    test("Update settings", async () => {
        const resource = Uri.file("x");
        const workspacePythonPath: WorkspacePythonPath = {
            configTarget: "xyz" as any,
            folderUri: Uri.parse("folder")
        };
        const pythonPath = "python Path to store in settings";
        when(helper.getActiveWorkspaceUri(resource)).thenReturn(
            workspacePythonPath
        );

        await rule.cacheSelectedInterpreter(resource, {
            path: pythonPath
        } as any);
    ...
    test("getWorkspaceVirtualEnvInterpreters will not return any interpreters if interpreters are not in workspace folder (windows)", async () => {
        ...
        const interpreter1 = {
            path: path.join("one", "two", "bin", "python.exe")
        };
    ...
    test("getWorkspaceVirtualEnvInterpreters will return workspace related virtual interpreters (windows)", async () => {
        ...
        const interpreter2 = {
            path: path.join(folderPath, "venv", "bin", "python.exe")
        };
        const interpreter3 = {
            path: path.join(
    ...
    [OSType.OSX, OSType.Linux].forEach(osType => {
        test(`getWorkspaceVirtualEnvInterpreters will not return any interpreters if interpreters are not in workspace folder (${osType})`, async () => {
            ...
            const envs = await rule.getWorkspaceVirtualEnvInterpreters(
                resource
            );
            expect(envs || []).to.be.lengthOf(0);
        });
        test(`getWorkspaceVirtualEnvInterpreters will return workspace related virtual interpreters (${osType})`, async () => {
            ...
            when(virtualEnvLocator.getInterpreters(resource, true)).thenResolve(
                [interpreter1, interpreter2, interpreter3] as any
            );
            when(workspaceService.getWorkspaceFolder(resource)).thenReturn(
                workspaceFolder
            );
    ...
    test("Use pipEnv if that completes first with results", async () => {
        ...
        rule.getWorkspaceVirtualEnvInterpreters = () =>
            virtualEnvPromise.promise;
        when(pipEnvLocator.getInterpreters(folderUri)).thenResolve([
            interpreterInfo
        ]);
        when(
            helper.getBestInterpreter(deepEqual([interpreterInfo]))
        ).thenReturn(interpreterInfo);
        ...
        expect(nextInvoked.completed).to.be.equal(
            true,
            "Next rule not invoked"
        );
        verify(helper.getActiveWorkspaceUri(resource)).atLeast(1);
        verify(
            manager.setWorkspaceInterpreter(folderUri, interpreterInfo)
        ).once();
    });
    test("Use virtualEnv if that completes first with results", async () => {
        ...
        rule.next = () => Promise.resolve(nextInvoked.resolve());
        rule.getWorkspaceVirtualEnvInterpreters = () =>
            Promise.resolve([interpreterInfo]);
        when(pipEnvLocator.getInterpreters(folderUri)).thenResolve([
            interpreterInfo
        ]);
        when(
    ...
    test("Wait for virtualEnv if pipEnv completes without any intepreters", async () => {
        ...
        rule.getWorkspaceVirtualEnvInterpreters = () =>
            virtualEnvPromise.promise;
        when(pipEnvLocator.getInterpreters(folderUri)).thenResolve([]);
        when(helper.getBestInterpreter(deepEqual(anything()))).thenReturn(
            interpreterInfo
        );
