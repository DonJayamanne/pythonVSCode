...
import {
    CondaGetEnvironmentPrefix,
    CondaService
} from "../../client/interpreter/locators/services/condaService";
...
suite("Interpreters Conda Service", () => {
    ...
    let registryInterpreterLocatorService: TypeMoq.IMock<
        IInterpreterLocatorService
    >;
    ...
    setup(async () => {
        ...
        interpreterService
            .setup(x => x.onDidChangeInterpreter)
            .returns(() => e.event);
        resetMockState(undefined);
        persistentStateFactory
            .setup(s =>
                s.createGlobalPersistentState(
                    TypeMoq.It.isAny(),
                    TypeMoq.It.isAny()
                )
            )
        ...
        >();
        terminalProvider
            .setup(p => p.isShellSupported(TypeMoq.It.isAny()))
            .returns(() => true);
        terminalProvider
            .setup(p =>
                p.getActivationCommands(TypeMoq.It.isAny(), TypeMoq.It.isAny())
            )
            .returns(() => Promise.resolve(["activate"]));
        terminalProvider
            .setup(p =>
                p.getActivationCommandsForInterpreter!(
                    TypeMoq.It.isAny(),
                    TypeMoq.It.isAny()
                )
            )
        ...
        serviceContainer
            .setup(c =>
                c.get(
                    TypeMoq.It.isValue(IProcessServiceFactory),
                    TypeMoq.It.isAny()
                )
            )
        ...
        serviceContainer
            .setup(c =>
                c.get(
                    TypeMoq.It.isValue(ITerminalActivationCommandProvider),
                    TypeMoq.It.isAny()
                )
            )
        ...
        serviceContainer
            ...
            .returns(() => [terminalProvider.object]);
        config
            .setup(c => c.getSettings(TypeMoq.It.isValue(undefined)))
            .returns(() => settings.object);
        settings.setup(p => p.condaPath).returns(() => condaPathSetting);
        fileSystem
            .setup(fs =>
                fs.arePathsSame(TypeMoq.It.isAny(), TypeMoq.It.isAny())
            )
            .returns((p1, p2) => {
                return new FileSystem(platformService.object).arePathsSame(
                    p1,
                    p2
                );
            });
    ...
    async function identifyPythonPathAsCondaEnvironment(
        ...
        expect(isCondaEnv).to.be.equal(
            true,
            "Path not identified as a conda path"
        );
    }
    ...
    test("Correctly identifies a python path as a conda environment (windows)", async () => {
        ...
        fileSystem
            .setup(f =>
                f.directoryExists(
                    TypeMoq.It.isValue(
                        path.join(path.dirname(pythonPath), "conda-meta")
                    )
                )
            )
            .returns(() => Promise.resolve(true));
        await identifyPythonPathAsCondaEnvironment(
            true,
            false,
            false,
            pythonPath
    ...
    async function identifyPythonPathAsNonCondaEnvironment(
        ...
        expect(isCondaEnv).to.be.equal(
            false,
            "Path incorrectly identified as a conda path"
        );
    }
    ...
    async function checkCondaNameAndPathForCondaEnvironments(
        ...
        expect(condaEnv).deep.equal(
            expectedCondaEnv,
            "Conda environment not identified"
        );
    }
    ...
    test("Correctly retrieves conda environment (windows)", async () => {
        ...
        fileSystem
            ...
            .returns(() => Promise.resolve(true));
        await checkCondaNameAndPathForCondaEnvironments(
            true,
            false,
            false,
            pythonPath,
            condaEnvDir,
            { name: "One", path: path.dirname(pythonPath) }
        );
    });
    ...
    test("Correctly retrieves conda environment with spaces in env name (windows)", async () => {
        ...
        await checkCondaNameAndPathForCondaEnvironments(
            ...
            pythonPath,
            condaEnvDir,
            { name: "Eight", path: path.dirname(pythonPath) }
        );
    });
    ...
    test("Correctly retrieves conda environment (osx)", async () => {
        ...
        await checkCondaNameAndPathForCondaEnvironments(
            ...
            pythonPath,
            condaEnvDir,
            { name: "One", path: path.join(path.dirname(pythonPath), "..") }
        );
    });
    ...
    test("Correctly retrieves conda environment with spaces in env name (osx)", async () => {
        ...
        await checkCondaNameAndPathForCondaEnvironments(
            ...
            pythonPath,
            condaEnvDir,
            { name: "Eight", path: path.join(path.dirname(pythonPath), "..") }
        );
    });
    ...
    test("Ignore cache if environment is not found in the cache (conda env is detected second time round)", async () => {
        ...
        processService
            .setup(p =>
                p.exec(
                    TypeMoq.It.isValue("conda"),
                    TypeMoq.It.isValue(["--version"]),
                    TypeMoq.It.isAny()
                )
            )
        ...
        expect(mockState.data.data).lengthOf(
            7,
            "Incorrect number of items in the cache"
        );
    });
    ...
    test("Ignore cache if environment is not found in the cache (cond env is not detected in conda env list)", async () => {
        ...
        expect(condaEnv).deep.equal(
            undefined,
            "Conda environment incorrectly identified after ignoring cache"
        );
        expect(mockState.data.data).lengthOf(
            6,
            "Incorrect number of items in the cache"
    ...
    test("Must use Conda env from Registry to locate conda.exe", async () => {
        ...
        const condaInterpreterIndex = registryInterpreters.findIndex(
            i => i.displayName === "Anaconda"
        );
        ...
        fileSystem
            ...
            .returns((file: string) =>
                Promise.resolve(file === expectedCodnaPath)
            );
    ...
    test("Must use Conda env from Registry to latest version of locate conda.exe", async () => {
        ...
        const expectedCodnaPath = path.join(
            path.dirname(registryInterpreters[indexOfLatestVersion].path),
            "conda.exe"
        );
        platformService.setup(p => p.isWindows).returns(() => true);
        processService
    ...
    test("Must use 'conda' if conda.exe cannot be located using registry entries", async () => {
        ...
        registryInterpreterLocatorService
            ...
            .returns(() => Promise.resolve(registryInterpreters));
        fileSystem
            .setup(fs => fs.search(TypeMoq.It.isAnyString()))
            .returns(async () => []);
        fileSystem
            .setup(fs => fs.fileExists(TypeMoq.It.isAny()))
            .returns((file: string) => Promise.resolve(false));
    ...
    test("Get conda file from default/known locations", async () => {
        ...
        fileSystem
            .setup(f => f.search(TypeMoq.It.isAnyString()))
    ...
    test("Must use 'python.condaPath' setting if set", async () => {
        ...
        assert.equal(
            condaExe,
            "spam-spam-conda-spam-spam",
            "Failed to identify conda.exe"
        );

        // We should not try to call other unwanted methods.
        processService.verifyAll();
        registryInterpreterLocatorService.verify(
            r => r.getInterpreters(TypeMoq.It.isAny()),
            TypeMoq.Times.never()
        );
    });
    ...
    [
        ...
        test(`Must return conda path from known location '${knownLocation}' (non windows)`, async () => {
            ...
            fileSystem
                .setup(fs =>
                    fs.fileExists(TypeMoq.It.isValue(expectedCondaLocation))
                )
                .returns(() => Promise.resolve(true));
    ...
    test("Returns condaInfo when conda exists", async () => {
        ...
        processService
            .setup(p =>
                p.exec(
                    TypeMoq.It.isValue("conda"),
                    TypeMoq.It.isValue(["info", "--json"]),
                    TypeMoq.It.isAny()
                )
            )
            .returns(() =>
                Promise.resolve({ stdout: JSON.stringify(expectedInfo) })
            );
    ...
    test("Returns conda environments when conda exists", async () => {
        ...
        assert.equal(
            environments,
            undefined,
            "Conda environments do not match"
        );
    });
    ...
    test("Logs information message when conda does not exist", async () => {
        ...
        logger
            .setup(l =>
                l.logInformation(TypeMoq.It.isAny(), TypeMoq.It.isAny())
            )
    ...
    test("Subsequent list of environments will be retrieved from cache", async () => {
        ...
        expect(mockState.data.data).lengthOf(
            6,
            "Incorrect number of environments in cache"
        );

        mockState.data.data = [];
        const environmentsFetchedAgain = await condaService.getCondaEnvironments(
            false
        );
        expect(environmentsFetchedAgain).lengthOf(
            0,
            "Incorrect number of environments fetched from cache"
        );
    });
    ...
    test("Must use Conda env from Registry to locate conda.exe", async () => {
        ...
        const expectedCodaExe = path.join(
            path.dirname(condaPythonExePath),
            "conda.exe"
        );
    ...
    test("Fails to identify an environment as a conda env (windows)", async () => {
        ...
        fileSystem
            ...
            .returns(() => Promise.resolve(true));
        await testFailureOfGettingCondaEnvironments(
            true,
            false,
            false,
            pythonPath
    ...
    test("Create activated conda environment for Windows", async () => {
        ...
        platformService.setup(p => p.isWindows).returns(() => true);
        fileSystem
            .setup(f => f.fileExists(TypeMoq.It.isAny()))
            .returns(() => Promise.resolve(true));

        const newEnvironment = await condaService.getActivatedCondaEnvironment(
            pythonInterpreter,
            environment
        );
        ...
        const expectedPath = path.join(
            pythonInterpreter.envPath ? pythonInterpreter.envPath : "",
            "Scripts"
        );
        expect(newEnvironment.Path).to.be.equal(
            expectedPath.concat(";", environment.Path),
            "Incorrect Windows Path Value"
        );
        expect(newEnvironment.CONDA_PREFIX).to.be.equal(
            pythonInterpreter.envPath,
            "Incorrect Windows CONDA_PREFIX Value"
        );
        expect(newEnvironment.CONDA_DEFAULT_ENV).to.be.equal(
            pythonInterpreter.envName,
            "Incorrect Windows CONDA_DEFAULT_ENV Value"
        );
    });
    test("Create activated conda environment for Non-Windows", async () => {
        ...
        expect(newEnvironment.PATH).to.be.equal(
            expectedPath.concat(":", environment.PATH),
            "Incorrect Non-Windows Path Value"
        );
        expect(newEnvironment.CONDA_PREFIX).to.be.equal(
            pythonInterpreter.envPath,
            "Incorrect Non-Windows CONDA_PREFIX Value"
        );
        expect(newEnvironment.CONDA_DEFAULT_ENV).to.be.equal(
            pythonInterpreter.envName,
            "Incorrect Non-Windows CONDA_DEFAULT_ENV Value"
        );
    });
    test("Create activated conda environment for using shell", async () => {
        const pythonInterpreter: PythonInterpreter = {
            ...info,
            type: InterpreterType.Conda,
            envPath: "C:\\Anaconda\\Foo\\envs\\test",
            envName: "Anaconda"
        };
        const environment: any = { Path: "C:\\test" };
        ...
        processService
            ...
            .returns(() =>
                Promise.resolve({
                    stdout: `${CondaGetEnvironmentPrefix}\r\nCONDA_PREFIX=TEST_PREFIX`
                })
            );
        ...
        expect(newEnvironment.CONDA_PREFIX).to.be.equal(
            "TEST_PREFIX",
            "Incorrect shell exec CONDA_PREFIX Value"
        );
    });
