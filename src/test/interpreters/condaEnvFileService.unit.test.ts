...
suite("Interpreters from Conda Environments Text File", () => {
    ...
    test("Must return an empty list for an empty file", async () => {
        condaService
            .setup(c => c.condaEnvironmentsFile)
            .returns(() => environmentsFilePath);
        fileSystem
            .setup(fs =>
                fs.fileExists(TypeMoq.It.isValue(environmentsFilePath))
            )
    ...
    async function filterFilesInEnvironmentsFileAndReturnValidItems(
        isWindows: boolean
    ) {
        const validPaths = [
