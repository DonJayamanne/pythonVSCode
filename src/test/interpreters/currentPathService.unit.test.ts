...
suite("Interpreters CurrentPath Service", () => {
    ...
    setup(async () => {
        ...
        persistentState = TypeMoq.Mock.ofType<
            IPersistentState<PythonInterpreter[]>
        >();
        ...
        serviceContainer
            .setup(c =>
                c.get(
                    TypeMoq.It.isValue(IVirtualEnvironmentManager),
                    TypeMoq.It.isAny()
                )
            )
        ...
        serviceContainer
            .setup(c =>
                c.get(
                    TypeMoq.It.isValue(IInterpreterVersionService),
                    TypeMoq.It.isAny()
                )
            )
        ...
        pythonInPathCommandProvider = new PythonInPathCommandProvider(
    ...
    [true, false].forEach(isWindows => {
        test(`Interpreters that do not exist on the file system are not excluded from the list (${
            ...
            platformService
                .setup(p => p.osType)
                .returns(() => (isWindows ? OSType.Windows : OSType.Linux));
            interpreterHelper
            ...
            processService
                .setup(p =>
                    p.exec(
                        TypeMoq.It.isValue("root:Python"),
                        TypeMoq.It.isValue(execArgs),
                        TypeMoq.It.isAny()
                    )
                )
            ...
            fileSystem
                .setup(fs =>
                    fs.fileExists(TypeMoq.It.isValue("c:/root:python"))
                )
