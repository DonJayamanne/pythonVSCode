...
import {
    ...
    InterpreterSelector
} from "../../client/interpreter/configuration/interpreterSelector";
import {
...
suite("Interpreters - selector", () => {
    ...
    setup(() => {
        ...
        comparer
            .setup(c => c.compare(TypeMoq.It.isAny(), TypeMoq.It.isAny()))
            .returns(() => 0);
    });
    ...
    [true, false].forEach(isWindows => {
        test(`Suggestions (${
            ...
            assert.equal(
                actual.length,
                expected.length,
                "Suggestion lengths are different."
            );
            for (let i = 0; i < expected.length; i += 1) {
                assert.equal(
                    actual[i].label,
                    expected[i].label,
                    `Suggestion label is different at ${i}: exected '${
                        expected[i].label
                    }', found '${actual[i].label}'.`
                );
                assert.equal(
                    actual[i].path,
                    expected[i].path,
                    `Suggestion path is different at ${i}: exected '${
                        expected[i].path
                    }', found '${actual[i].path}'.`
                );
            }
    ...
    test("Update Global settings when there are no workspaces", async () => {
        ...
        appShell
            .setup(s =>
                s.showQuickPick<IInterpreterQuickPickItem>(
                    TypeMoq.It.isAny(),
                    TypeMoq.It.isAny()
                )
            )
