...
class CoverageRunner {
    ...
    public setupCoverage(): void {
        ...
        // self.matchFn = (file: string) => fileMap.has(file);
    ...
    public reportCoverage(): void {
        ...
        const reportingDir = '/Users/donjayamanne/.vscode/extensions/pythonVSCode/coverage';
        const includePid = this.options.includePid;
        const pidExt = includePid ? `-${process.pid}` : '';
        const coverageFile = path.join(reportingDir, 'coverage.output' + pidExt + '.json');
