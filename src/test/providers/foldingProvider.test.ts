...
const pythonFilesPath = path.join(
    __dirname,
    "..",
    "..",
    "..",
...
suite("Provider - Folding Provider", () => {
    const docStringFileAndExpectedFoldingRanges: FileFoldingRanges[] = [
        {
            ...
            ranges: [
                ...
                new FoldingRange(95, 143),
                new FoldingRange(149, 150, FoldingRangeKind.Comment),
                new FoldingRange(305, 313),
                new FoldingRange(320, 322)
            ]
        },
        {
    ...
    docStringFileAndExpectedFoldingRanges.forEach(item => {
        test(`Test Docstring folding regions '${path.basename(
            item.file
        )}'`, async () => {
            ...
            ranges!.forEach(range => {
                const index = item.ranges.findIndex(
                    searchItem =>
                        searchItem.start === range.start &&
                        searchItem.end === range.end
                );
                expect(index).to.be.greaterThan(
                    -1,
                    `${range.start}, ${range.end} not found`
                );
            });
        });
    });
