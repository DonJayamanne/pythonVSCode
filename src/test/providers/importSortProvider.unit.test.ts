...
suite("Import Sort Provider", () => {
    ...
    test("Ensure empty line is added when line does not end with an empty line", async () => {
        ...
        sortProvider.provideDocumentSortImportsEdits = () =>
            Promise.resolve(undefined);
        await sortProvider.sortImports(uri);

        expect(editApplied).not.to.be.equal(
            undefined,
            "Applied edit is undefined"
        );
    ...
    test("Ensure temporary file is created for sorting when document is dirty", async () => {
        ...
        fs.setup(f =>
            f.writeFile(
                TypeMoq.It.isValue(tmpFile.filePath),
        ...
        expect(tmpFileDisposed).to.be.equal(
            true,
            "Temporary file not disposed"
        );
        shell.verifyAll();
        documentManager.verifyAll();
    });
