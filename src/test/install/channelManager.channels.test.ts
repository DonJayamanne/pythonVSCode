...
suite("Installation - installation channels", () => {
    ...
    test("Select installer", async () => {
        ...
        assert.notEqual(
            items![0]!.label!.indexOf("Name 1"),
            -1,
            "Incorrect first installer name"
        );
        assert.notEqual(
            items![1]!.label!.indexOf("Name 2"),
            -1,
            "Incorrect second installer name"
        );
    });
    ...
    function mockInstaller(
        ...
        name: string,
        priority?: number
    ): TypeMoq.IMock<IModuleInstaller> {
