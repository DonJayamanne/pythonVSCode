...
suite("Installation - channel messages", () => {
    ...
    setup(() => {
        ...
        serviceManager.addSingletonInstance<IModuleInstaller>(
            IModuleInstaller,
    ...
    function verifyMessage(
        ...
        for (const m of missing) {
            assert.equal(
                message.indexOf(m) < 0,
                true,
                `Message incorrectly contains ${m}.`
            );
        }
    }
    ...
    async function testInstallerMissingMessage(
        ...
        interpreters
            ...
            .returns(
                () =>
                    new Promise<PythonInterpreter>((resolve, reject) =>
                        resolve(activeInterpreter)
                    )
            );
        ...
        appShell
            ...
            .returns(
                () => new Promise<string>((resolve, reject) => resolve(search))
            );
        appShell
