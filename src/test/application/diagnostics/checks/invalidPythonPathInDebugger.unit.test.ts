...
suite("Application Diagnostics - Checks Python Path in debugger", () => {
    ...
    test("Ensure ${workspaceFolder} is expanded", async () => {
        ...
        const workspaceFolder = {
            uri: Uri.parse("full/path/to/workspace"),
            name: "",
            index: 0
        };
        ...
        helper
            .setup(h =>
                h.getInterpreterInformation(typemoq.It.isValue(expectedPath))
            )
            .returns(() => Promise.resolve({}))
            .verifiable(typemoq.Times.once());
    ...
    test("Ensure we do not get python path from config when path is provided", async () => {
        ...
        helper
            .setup(h =>
                h.getInterpreterInformation(typemoq.It.isValue(pythonPath))
            )
            .returns(() => Promise.resolve({}))
            .verifiable(typemoq.Times.once());
