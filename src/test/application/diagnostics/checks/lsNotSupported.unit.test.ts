...
suite("Application Diagnostics - Checks LS not supported", () => {
    ...
    setup(() => {
        ...
        messageHandler = TypeMoq.Mock.ofType<
            IDiagnosticHandlerService<MessageCommandPrompt>
        >();
        lsCompatibility = TypeMoq.Mock.ofType<
        ...
        diagnosticService = new LSNotSupportedDiagnosticService(
            serviceContainer.object,
            lsCompatibility.object,
        ...
        filterService
            .setup(f =>
                f.shouldIgnoreDiagnostic(
                    TypeMoq.It.isValue(DiagnosticCodes.LSNotSupportedDiagnostic)
                )
            )
