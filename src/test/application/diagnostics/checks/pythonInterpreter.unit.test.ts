...
suite("Application Diagnostics - Checks Python Interpreter", () => {
    ...
    function createContainer() {
        ...
        serviceContainer
            .setup(s => s.get(typemoq.It.isValue(IInterpreterHelper)))
    ...
    suite("Diagnostics", () => {
        ...
        test("Should return diagnostic if there are no other interpreters, platform is mac and selected interpreter is default mac interpreter", async () => {
            ...
            helper
                .setup(i =>
                    i.isMacDefaultPythonPath(typemoq.It.isValue(pythonPath))
                )
                .returns(() => true)
                .verifiable(typemoq.Times.atLeastOnce());
        ...
        test("Should return diagnostic if there are other interpreters, platform is mac and selected interpreter is default mac interpreter", async () => {
            ...
            helper
                .setup(i =>
                    i.isMacDefaultPythonPath(
                        typemoq.It.isValue(nonMacStandardInterpreter)
                    )
                )
                .returns(() => false)
                .verifiable(typemoq.Times.atLeastOnce());
        ...
        test("Handling no interpreters diagnostic should return download link", async () => {
            ...
            messageHandler
                .setup(i =>
                    i.handle(typemoq.It.isValue(diagnostic), typemoq.It.isAny())
                )
            ...
            expect(messagePrompt).not.be.equal(
                undefined,
                "Message prompt not set"
            );
            expect(messagePrompt!.commandPrompts).to.be.deep.equal([
                { prompt: "Download", command: cmd }
            ]);
        });
            ...
            commandFactory
                .setup(f =>
                    f.createCommand(
                        typemoq.It.isAny(),
                        typemoq.It.isObjectWith<
                            CommandOption<"executeVSCCommand", string>
    ...
    suite("Change Handlers.", () => {
        ...
        test("Diagnostics are checked when path changes", async () => {
            ...
            const diagnosticSvc = new class extends InvalidPythonInterpreterService {
                ...
                public onDidChangeConfigurationEx = e =>
            ...
            event
                .setup(e =>
                    e.affectsConfiguration(
                        typemoq.It.isValue("python.pythonPath"),
                        typemoq.It.isAny()
                    )
                )
                .returns(() => true)
