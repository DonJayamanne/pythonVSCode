...
suite("Application Diagnostics - ApplicationDiagnostics", () => {
        ...
        serviceContainer
            .setup(d =>
                d.get(
                    ...
                    typemoq.It.isValue(STANDARD_OUTPUT_CHANNEL)
                )
            )
            .returns(() => outputChannel.object);
        serviceContainer
            .setup(d => d.get(typemoq.It.isValue(ILogger)))
            .returns(() => logger.object);

        appDiagnostics = new ApplicationDiagnostics(
    ...
    test("Register should register source maps", () => {
        const sourceMapService = typemoq.Mock.ofType<
            ISourceMapSupportService
        >();
        sourceMapService
            .setup(s => s.register())
        ...
        serviceContainer
            .setup(d =>
                d.get(
                    typemoq.It.isValue(ISourceMapSupportService),
                    typemoq.It.isAny()
                )
            )
    ...
    test("Diagnostics Returned by Per Startup Health Checks must be logged", async () => {
        ...
        for (const diagnostic of diagnostics) {
            const message = `Diagnostic Code: ${diagnostic.code}, Message: ${
                diagnostic.message
            }`;
            switch (diagnostic.severity) {
                case DiagnosticSeverity.Error: {
                    logger
                        .setup(l => l.logError(message))
                        .verifiable(typemoq.Times.once());
                    outputChannel
                        .setup(o => o.appendLine(message))
                        .verifiable(typemoq.Times.once());
                    break;
                }
                case DiagnosticSeverity.Warning: {
                    logger
                        .setup(l => l.logWarning(message))
                        .verifiable(typemoq.Times.once());
                    outputChannel
                        .setup(o => o.appendLine(message))
                        .verifiable(typemoq.Times.once());
                ...
                default: {
                    logger
                        .setup(l => l.logInformation(message))
                        .verifiable(typemoq.Times.once());
                    break;
                }
            }
