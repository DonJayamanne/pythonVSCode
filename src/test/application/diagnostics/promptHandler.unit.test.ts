...
suite("Application Diagnostics - PromptHandler", () => {
    ...
    setup(() => {
        ...
        promptHandler = new DiagnosticCommandPromptHandlerService(
            serviceContainer.object
        );
    });

    getNamesAndValues<DiagnosticSeverity>(DiagnosticSeverity).forEach(
        severity => {
            test(`Handling a diagnositic of severity '${
                severity.name
            }' should display a message without any buttons`, async () => {
            ...
            test(`Handling a diagnositic of severity '${
                severity.name
            }' should display a custom message with buttons`, async () => {
                const diagnostic: IDiagnostic = {
                    code: "1",
                    message: "one",
                    scope: DiagnosticScope.Global,
            ...
            test(`Handling a diagnositic of severity '${
                severity.name
            }' should display a custom message with buttons and invoke selected command`, async () => {
                const diagnostic: IDiagnostic = {
                    code: "1",
                    message: "one",
                    scope: DiagnosticScope.Global,
