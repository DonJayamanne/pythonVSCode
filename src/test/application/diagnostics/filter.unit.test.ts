...
import {
    ...
    FilterKeys
} from "../../../client/application/diagnostics/filter";
import {
    DiagnosticScope,
...
suite("Application Diagnostics - Filter", () => {
    ...
    [
        ...
        setup(() => {
            ...
            filterService = new DiagnosticFilterService(
                serviceContainer.object
            );
        });

        test(`ignoreDiagnostic must save codes in ${
            item.name
        } Persistent State`, async () => {
            const code = "xyz";
            item.state()
                .setup(g => g.value)
                .returns(() => [])
        ...
        test(`shouldIgnoreDiagnostic should return \'true\' when code exist in ${
            item.name
        } State`, async () => {
            const code = "xyz";
            item.state()
        ...
        test(`ignoreDiagnostic must append codes in ${
            item.name
        } Persistent State`, async () => {
            const code = "xyz";
