...
suite("Language.CharacterStream", () => {
    ...
    test("Characters", async () => {
        ...
        for (let i = 0; i < content.length; i += 1) {
            ...
            assert.equal(
                cs.nextChar,
                i < content.length - 1 ? content.charCodeAt(i + 1) : 0
            );
            assert.equal(cs.prevChar, i > 0 ? content.charCodeAt(i - 1) : 0);

            assert.equal(
                cs.lookAhead(2),
                i < content.length - 2 ? content.charCodeAt(i + 2) : 0
            );
            assert.equal(
                cs.lookAhead(-2),
                i > 1 ? content.charCodeAt(i - 2) : 0
            );

            const ch = content.charCodeAt(i);
            const isLineBreak =
                ch === Char.LineFeed || ch === Char.CarriageReturn;
            assert.equal(
                cs.isAtWhiteSpace(),
                ch === Char.Tab || ch === Char.Space || isLineBreak
            );
            assert.equal(cs.isAtLineBreak(), isLineBreak);
            assert.equal(
                cs.isAtString(),
                ch === Char.SingleQuote || ch === Char.DoubleQuote
            );
