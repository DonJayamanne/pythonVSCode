...
const debugFilesPath = path.join(
    __dirname,
    "..",
    "..",
    "..",
...
const EXPERIMENTAL_DEBUG_ADAPTER = path.join(
    __dirname,
    "..",
    "..",
    "client",
...
suite(`Standard Debugging of ports and hosts: ${debuggerType}`, () => {
    ...
    async function testDebuggingWithProvidedPort(
        ...
        await Promise.all([
            ...
            debugClient.launch(
                buildLaunchArgs("startAndWait.py", false, port, host)
            ),
        ...
        if (typeof port === "number" && port > 0) {
            ...
            const portBasedOnDebuggerPort = await getFreePort({
                host: "localhost",
                port
            });
            expect(portBasedOnDebuggerPort).is.not.equal(
                port,
                "Port assigned to debugger not used by the debugger"
            );
        }
    }
    ...
    test("Confirm debuggig fails when an invalid host is provided", async () => {
        ...
        expect(exception!.message).contains(
            "ENOTFOUND",
            "Debugging failed for some other reason"
        );
    });
