...
suite("Debug - Attach to Child Process", () => {
    test("Message is not displayed if debugger is launched", async () => {
        ...
        const service = new ChildProcessAttachService(
            instance(shell),
        ...
        when(debugService.startDebugging(anything(), anything())).thenResolve(
            true as any
        );
    ...
    test("Use correct workspace folder", async () => {
        ...
        const rightWorkspaceFolder: WorkspaceFolder = {
            name: "1",
            index: 1,
            uri: Uri.file("a")
        };
        const wkspace1: WorkspaceFolder = {
            name: "0",
            index: 0,
            uri: Uri.file("0")
        };
        const wkspace2: WorkspaceFolder = {
            name: "2",
            index: 2,
            uri: Uri.file("2")
        };
        ...
        when(workspaceService.workspaceFolders).thenReturn([
            wkspace1,
            rightWorkspaceFolder,
            wkspace2
        ]);
        when(
            debugService.startDebugging(rightWorkspaceFolder, anything())
        ).thenResolve(true as any);
    ...
    test("Use empty workspace folder if right one is not found", async () => {
        ...
        when(debugService.startDebugging(undefined, anything())).thenResolve(
            true as any
        );
