...
suite("Debugging - Configuration Service", () => {
    let attachResolver: typemoq.IMock<
        IDebugConfigurationResolver<AttachRequestArguments>
    >;
    let launchResolver: typemoq.IMock<
    ...
    setup(() => {
        attachResolver = typemoq.Mock.ofType<
            IDebugConfigurationResolver<AttachRequestArguments>
        >();
    ...
    test("Should use attach resolver when passing attach config", async () => {
        ...
        const resolvedConfig = await configService.resolveDebugConfiguration(
            folder,
            config as any
        );
    ...
    [{ request: "launch" }, { request: undefined }].forEach(config => {
        test(`Should use launch resolver when passing launch config with request=${
            config.request
        }`, async () => {
            const folder = { name: "1", index: 0, uri: Uri.parse("1234") };
            const expectedConfig = { yay: 1 };
            ...
            launchResolver
                .setup(a =>
                    a.resolveDebugConfiguration(
                        typemoq.It.isValue(folder),
                        typemoq.It.isValue(
                            (config as any) as LaunchRequestArguments
                        ),
                        typemoq.It.isAny()
                    )
                )
    ...
    test("Picker should be displayed", async () => {
        ...
        const state = ({
            ...
            folder: {},
            token: undefined
        } as any) as DebugConfigurationState;
        ...
        await configService.pickDebugConfiguration(
    ...
    test("Ensure generated config is returned", async function() {
        ...
        const config = await configService.provideDebugConfigurations!(
            {} as any
        );
