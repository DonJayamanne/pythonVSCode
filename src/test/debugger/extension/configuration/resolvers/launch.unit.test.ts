...
suite("Debugging - Config Resolver Launch", () => {
    ...
    function setupIoc(
        ...
        diagnosticsService
            .setup(h =>
                h.validatePythonPath(TypeMoq.It.isAny(), TypeMoq.It.isAny())
            )
            .returns(() => Promise.resolve(true));

        const configProviderUtils = new ConfigurationProviderUtils(
            factory.object,
            fileSystem.object,
            appShell.object
        );
        ...
        serviceContainer
            .setup(c => c.get(TypeMoq.It.isValue(IConfigurationProviderUtils)))
            .returns(() => configProviderUtils);
        serviceContainer
            .setup(c => c.get(TypeMoq.It.isValue(ILogger)))
        ...
        serviceContainer
            .setup(c =>
                c.get(
                    ...
                    TypeMoq.It.isValue(InvalidPythonPathInDebuggerServiceId)
                )
            )
            .returns(() => diagnosticsService.object);
    ...
    test("Defaults should be returned when an empty object is passed with a Workspace Folder and active file", async () => {
        ...
        expect(debugConfig!.cwd!.toLowerCase()).to.be.equal(
            __dirname.toLowerCase()
        );
        expect(debugConfig).to.have.property("envFile");
        expect(debugConfig!.envFile!.toLowerCase()).to.be.equal(
            path.join(__dirname, ".env").toLowerCase()
        );
    ...
    test("Defaults should be returned when an object with 'noDebug' property is passed with a Workspace Folder and active file", async () => {
        ...
        const debugConfig = await debugProvider.resolveDebugConfiguration!(
            workspaceFolder,
            ({ noDebug: true } as any) as DebugConfiguration
        );
    ...
    test("Defaults should be returned when an empty object is passed without Workspace Folder, no workspaces and active file", async () => {
        ...
        expect(debugConfig).to.have.property("envFile");
        expect(debugConfig!.envFile!.toLowerCase()).to.be.equal(
            path.join(filePath, ".env").toLowerCase()
        );
        expect(debugConfig).to.have.property("env");
        // tslint:disable-next-line:no-any
        expect(Object.keys((debugConfig as any).env)).to.have.lengthOf(0);
    ...
    test("Ensure hardcoded pythonPath is left unaltered", async () => {
        ...
        const debugConfig = await debugProvider.resolveDebugConfiguration!(
            workspaceFolder,
            ({ pythonPath: debugPythonPath } as any) as DebugConfiguration
        );
    ...
    test("Test overriding defaults of debugger", async () => {
        ...
        const debugConfig = await debugProvider.resolveDebugConfiguration!(
            workspaceFolder,
            { redirectOutput: false } as LaunchRequestArguments
        );
    ...
    async function testFixFilePathCase(
        isWindows: boolean,
        isMac: boolean,
        isLinux: boolean
    ) {
    ...
    async function testPyramidConfiguration(
        ...
        const pserveFilePath = path.join(
            path.dirname(pyramidFilePath),
            "scripts",
            "pserve.py"
        );
        ...
        if (pyramidExists) {
            pythonExecutionService
                .setup(e =>
                    e.exec(TypeMoq.It.isValue(args), TypeMoq.It.isAny())
                )
                .returns(() => Promise.resolve({ stdout: pyramidFilePath }))
                .verifiable(
                    TypeMoq.Times.exactly(addPyramidDebugOption ? 1 : 0)
                );
        } else {
            pythonExecutionService
                .setup(e =>
        ...
        fileSystem
            .setup(f => f.fileExists(TypeMoq.It.isValue(pserveFilePath)))
            .returns(() => Promise.resolve(pyramidExists))
            .verifiable(
                TypeMoq.Times.exactly(
                    pyramidExists && addPyramidDebugOption ? 1 : 0
                )
            );
        appShell
            .setup(a => a.showErrorMessage(TypeMoq.It.isAny()))
            .returns(() => Promise.resolve(undefined))
            .verifiable(
                TypeMoq.Times.exactly(
                    pyramidExists || !addPyramidDebugOption ? 0 : 1
                )
            );
        const options = addPyramidDebugOption
            ? { debugOptions: [DebugOptions.Pyramid], pyramid: true }
            : {};
        ...
        if (shouldWork) {
            ...
            expect(debugConfig).to.have.property("debugOptions");
            expect((debugConfig as any).debugOptions).contains(
                DebugOptions.Jinja
            );
        } else {
    ...
    test("Auto detect flask debugging", async () => {
        ...
        const debugConfig = await debugProvider.resolveDebugConfiguration!(
            workspaceFolder,
            ({ module: "flask" } as any) as DebugConfiguration
        );
    ...
    test("Test validation of Python Path when launching debugger (with invalid python path)", async () => {
        ...
        const debugConfig = await debugProvider.resolveDebugConfiguration!(
            workspaceFolder,
            { redirectOutput: false, pythonPath } as LaunchRequestArguments
        );
    ...
    async function testSetting(
        ...
        if (mustHaveDebugOption) {
            ...
            expect((debugConfig as any).debugOptions).not.contains(
                debugOptionName
            );
        }
    }
