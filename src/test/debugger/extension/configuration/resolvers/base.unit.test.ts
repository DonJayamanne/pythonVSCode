...
suite("Debugging - Config Resolver", () => {
    class BaseResolver extends BaseConfigurationResolver<
    ...
    > {
        public resolveDebugConfiguration(
            ...
            AttachRequestArguments | LaunchRequestArguments | undefined
        > {
        ...
        public resolveAndUpdatePythonPath(
            ...
            return super.resolveAndUpdatePythonPath(
                workspaceFolder,
                debugConfiguration
            );
        }
    ...
    [
        {
            title:
                "Should get directory of active program when there are not workspace folders",
            workspaceFolders: undefined
        },
        {
            title:
                "Should get directory of active program when there are 0 workspace folders",
            workspaceFolders: []
        }
    ].forEach(item => {
        test(item.title, () => {
            ...
            expect(uri!.fsPath).to.be.deep.equal(
                Uri.file(path.dirname(programPath)).fsPath
            );
        });
    });
