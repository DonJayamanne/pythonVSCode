...
getNamesAndValues(OSType).forEach(os => {
    ...
    suite(`Debugging - Config Resolver attach, OS = ${os.name}`, () => {
        ...
        setup(() => {
            ...
            platformService
                .setup(p => p.isWindows)
                .returns(() => os.value === OSType.Windows);
            platformService
                .setup(p => p.isMac)
                .returns(() => os.value === OSType.OSX);
            platformService
                .setup(p => p.isLinux)
                .returns(() => os.value === OSType.Linux);
        ...
        test("Defaults should be returned when an empty object is passed with a Workspace Folder and active file", async () => {
            ...
            const debugConfig = await debugProvider.resolveDebugConfiguration!(
                workspaceFolder,
                { request: "attach" } as DebugConfiguration
            );
        ...
        test("Ensure 'localRoot' is left unaltered", async () => {
            ...
            const debugConfig = await debugProvider.resolveDebugConfiguration!(
                workspaceFolder,
                ({ localRoot, request: "attach" } as any) as DebugConfiguration
            );
        ...
        ["localhost", "127.0.0.1", "::1"].forEach(host => {
            test(`Ensure path mappings are automatically added when host is '${host}'`, async () => {
                ...
                expect(pathMappings![0].localRoot).to.be.equal(
                    workspaceFolder.uri.fsPath
                );
                expect(pathMappings![0].remoteRoot).to.be.equal(
                    workspaceFolder.uri.fsPath
                );
            });
        });
        ...
        test("Ensure 'localRoot' and 'remoteRoot' is used", async () => {
            ...
            expect(debugConfig!.pathMappings).to.deep.include({
                localRoot,
                remoteRoot
            });
        });
        ...
        test("Ensure 'remoteRoot' is left unaltered", async () => {
            ...
            const debugConfig = await debugProvider.resolveDebugConfiguration!(
                workspaceFolder,
                ({ remoteRoot, request: "attach" } as any) as DebugConfiguration
            );
        ...
        test("Ensure 'port' is left unaltered", async () => {
            ...
            const debugConfig = await debugProvider.resolveDebugConfiguration!(
                workspaceFolder,
                ({ port, request: "attach" } as any) as DebugConfiguration
            );
        ...
        test("Ensure 'debugOptions' are left unaltered", async () => {
            ...
            const debugOptions = debugOptionsAvailable
                .slice()
                .concat(DebugOptions.Jinja, DebugOptions.Sudo);
