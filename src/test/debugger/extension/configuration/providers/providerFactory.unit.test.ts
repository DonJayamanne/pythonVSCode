...
suite("Debugging - Configuration Provider Factory", () => {
    ...
    setup(() => {
        mappedProviders = new Map<
            ...
            IDebugConfigurationProvider
        >();
        getNamesAndValues<DebugConfigurationType>(
            DebugConfigurationType
        ).forEach(item => {
            mappedProviders.set(
                item.value,
                (item.value as any) as IDebugConfigurationProvider
            );
        });
    ...
    getNamesAndValues<DebugConfigurationType>(DebugConfigurationType).forEach(
        item => {
