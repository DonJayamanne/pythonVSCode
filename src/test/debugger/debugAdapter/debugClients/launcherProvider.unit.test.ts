...
import {
    ...
    RemoteDebuggerLauncherScriptProvider
...
suite("Debugger - Launcher Script Provider", () => {
    test("Ensure launcher script exists", async () => {
        expect(await fs.pathExists(expectedPath)).to.be.deep.equal(
            true,
            "Debugger launcher script does not exist"
        );
    });
    test("Test debug launcher args", async () => {
        const args = new DebuggerLauncherScriptProvider().getLauncherArgs({
            host: "something",
            port: 1234
        });
        const expectedArgs = [
    ...
    test("Test non-debug launcher args", async () => {
        const args = new NoDebugLauncherScriptProvider().getLauncherArgs({
            host: "something",
            port: 1234
        });
        const expectedArgs = [
    ...
    test("Test remote debug launcher args (and do not wait for debugger to attach)", async () => {
        const args = new RemoteDebuggerLauncherScriptProvider().getLauncherArgs(
            { host: "something", port: 1234, waitUntilDebuggerAttaches: false }
        );
        const expectedArgs = [
            expectedPath,
            "--default",
    ...
    test("Test remote debug launcher args (and wait for debugger to attach)", async () => {
        const args = new RemoteDebuggerLauncherScriptProvider().getLauncherArgs(
            { host: "something", port: 1234, waitUntilDebuggerAttaches: true }
        );
        const expectedArgs = [
            expectedPath,
            "--default",
