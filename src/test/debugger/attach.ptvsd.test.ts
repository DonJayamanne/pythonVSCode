...
suite("Debugging - Attach Debugger", () => {
    ...
    async function testAttachingToRemoteProcess(
        localRoot: string,
        remoteRoot: string,
        isLocalHostWindows: boolean
    ) {
        ...
        proc = spawn(PYTHON_PATH, pythonArgs, {
            env: env,
        ...
        const providerFactory = TypeMoq.Mock.ofType<
            IDebugConfigurationProviderFactory
        >().object;
        const multiStepIput = TypeMoq.Mock.ofType<IMultiStepInputFactory>()
            .object;
        ...
        await configProvider.resolveDebugConfiguration(
            { index: 0, name: "root", uri: Uri.file(localRoot) },
            options
        );
        ...
        const stdOutPromise = debugClient.assertOutput(
            "stdout",
            "this is stdout"
        );
        const stdErrPromise = debugClient.assertOutput(
            "stderr",
            "this is stderr"
        );

        // Don't use path utils, as we're building the paths manually (mimic windows paths on unix test servers and vice versa).
        const localFileName = `${localRoot}${localHostPathSeparator}${path.basename(
            fileToDebug
        )}`;
        ...
        const breakpointStoppedPromise = debugClient.assertStoppedLocation(
            "breakpoint",
            breakpointLocation
        );
    ...
    test("Confirm we are able to attach to a running program", async () => {
        await testAttachingToRemoteProcess(
            path.dirname(fileToDebug),
            path.dirname(fileToDebug),
            IS_WINDOWS
        );
    });
