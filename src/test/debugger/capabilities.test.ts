...
suite("Debugging - Capabilities", function() {
    ...
    function createRequest(cmd: string, requestArgs: any) {
        return new class extends Message
            implements DebugProtocol.InitializeRequest {
    ...
    test("Compare capabilities", async () => {
        ...
        const initializeRequest: DebugProtocol.InitializeRequest = createRequest(
            "initialize",
            { pathFormat: "path" }
        );
        ...
        proc = spawn(
            ...
            { cwd: path.dirname(fileToDebug), env }
        );
        await sleep(3000);
        ...
        const actualResponsePromise = new Promise<
            DebugProtocol.InitializeResponse
        >(resolve => protocolParser.once("response_initialize", resolve));
        protocolWriter.write(socket, initializeRequest);
        const actualResponse = await actualResponsePromise;

        const attachRequest: DebugProtocol.AttachRequest = createRequest(
            "attach",
            {
                name: "attach",
                request: "attach",
        ...
        const attached = new Promise(resolve =>
            protocolParser.once("response_attach", resolve)
        );
        protocolWriter.write(socket, attachRequest);
        await attached;

        const configRequest: DebugProtocol.ConfigurationDoneRequest = createRequest(
            "configurationDone",
            {}
        );
        const configured = new Promise(resolve =>
            protocolParser.once("response_configurationDone", resolve)
        );
