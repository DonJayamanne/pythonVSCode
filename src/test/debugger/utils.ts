...
const testAdapterFilePath = path.join(
    EXTENSION_ROOT_DIR,
    "out",
    "client",
    "debugger",
 ...
 * @returns {DebugClient}
 */
export async function createDebugAdapter(
...
export async function validateVariablesInFrame(
    ...
    const variables = await debugClient.variablesRequest({
        variablesReference
    });

    for (const expectedVariable of expectedVariables) {
        const variable = variables.body.variables.find(
            item => item.name === expectedVariable.name
        )!;
        expect(variable).to.be.not.equal(
            "undefined",
            `variable '${expectedVariable.name}' is undefined`
        );
...
export async function hitHttpBreakpoint(
    ...
    await debugClient.setBreakpointsRequest({
        ...
        breakpoints: [
            { line: breakpointLocation.line, column: breakpointLocation.column }
        ],
        source: { path: breakpointLocation.path }
    });

    // Make the request, we want the breakpoint to be hit.
    const breakpointPromise = debugClient.assertStoppedLocation(
        "breakpoint",
        breakpointLocation
    );
