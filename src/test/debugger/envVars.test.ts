...
suite("Resolving Environment Variables when Debugging", () => {
    ...
    setup(async () => {
        ...
        const envParser = ioc.serviceContainer.get<
            IEnvironmentVariablesService
        >(IEnvironmentVariablesService);
        const pathUtils = ioc.serviceContainer.get<IPathUtils>(IPathUtils);
        mockProcess = ioc.serviceContainer.get<ICurrentProcess>(
            ICurrentProcess
        );
    ...
    async function testBasicProperties(
        console: "externalTerminal" | "integratedTerminal" | "none",
        ...
        expect(Object.keys(envVars)).lengthOf(
    ...
    async function testJsonEnvVariables(
        console: "externalTerminal" | "integratedTerminal" | "none",
        expectedNumberOfVariables: number
    ) {
        ...
        if (console === "none") {
            ...
            expect(envVars).not.to.have.property(
                prop3,
                prop3,
                "Property not found"
            );
    ...
    async function testAppendingOfPaths(
        ...
        const expectedPath =
            customPathToAppend +
            path.delimiter +
            mockProcess.env[pathVariableName];
        expect(envVars).to.have.property(
            pathVariableName,
            expectedPath,
            "PATH is not correct"
        ...
        if (
            typeof mockProcess.env.PYTHONPATH === "string" &&
            mockProcess.env.PYTHONPATH.length > 0
        ) {
            expectedPythonPath =
                customPythonPathToAppend +
                path.delimiter +
                mockProcess.env.PYTHONPATH;
        }
        expect(envVars).to.have.property(
            "PYTHONPATH",
            expectedPythonPath,
            "PYTHONPATH is not correct"
        );
    ...
    test("Confirm paths get appended correctly when using json variables and launched in external terminal", () =>
        testAppendingOfPaths("externalTerminal", 6, false));

    test("Confirm paths get appended correctly when using json variables and launched in integrated terminal", () =>
