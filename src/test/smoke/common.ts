...
async function getLanaguageServerFolders(): Promise<string[]> {
    return new Promise<string[]>((resolve, reject) => {
        glob(
            "languageServer.*",
            { cwd: SMOKE_TEST_EXTENSIONS_DIR },
            (ex, matches) => {
                ex
                    ? reject(ex)
                    : resolve(
                          matches.map(item =>
                              path.join(SMOKE_TEST_EXTENSIONS_DIR, item)
                          )
                      );
            }
        );
...
export async function openFileAndWaitForLS(
    file: string
    ...
    await waitForCondition(
        isLanguageServerDownloaded,
        30_000,
        "Language Server not downloaded"
    );
...
async function isLanguageServerDownloaded() {
    ...
    const downloaded = await getLanaguageServerFolders().then(
