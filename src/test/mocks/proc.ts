...
type ExecObservableCallback = (
    result: Observable<Output<string>> | Output<string>
) => void;
type ExecCallback = (result: ExecutionResult<string>) => void;

export const IOriginalProcessService = Symbol("IProcessService");

export class MockProcessService extends EventEmitter
    ...
    public onExecObservable(
        handler: (
            ...
            callback: ExecObservableCallback
        ) => void
    ) {
    ...
    public async exec(
        ...
        return valueReturned
            ? value!
            : this.procService.exec(file, args, options);
    }

    public async shellExec(
        command: string,
        ...
        return valueReturned
            ? value!
            : this.procService.shellExec(command, options);
    }
}
