...
suite("Autocomplete PEP 526", () => {
    ...
    test("variable (abc:str)", async () => {
        ...
        assert.notEqual(
            list!.items.filter(item => item.label === "capitalize").length,
            0,
            "capitalize not found"
        );
        assert.notEqual(
            list!.items.filter(item => item.label === "upper").length,
            0,
            "upper not found"
        );
        assert.notEqual(
            list!.items.filter(item => item.label === "lower").length,
            0,
            "lower not found"
        );
    });
    ...
    test("class methods", async () => {
        ...
        assert.notEqual(
            list!.items.filter(item => item.label === "a").length,
            0,
            "method a not found"
        );
        ...
        assert.notEqual(
            list!.items.filter(item => item.label === "b").length,
            0,
            "method b not found"
        );
    });
    ...
    test("class method types", async () => {
        ...
        >("vscode.executeCompletionItemProvider", textDocument.uri, position);
        assert.notEqual(
            list!.items.filter(item => item.label === "bit_length").length,
            0,
            "bit_length not found"
        );
    });
});
