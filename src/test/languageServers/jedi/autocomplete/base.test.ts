...
suite("Autocomplete Base Tests", function() {
    ...
    test('For "sys."', done => {
        ...
        vscode.workspace
            ...
            .then(list => {
                assert.equal(
                    list!.items.filter(item => item.label === "api_version")
                        .length,
                    1,
    ...
    test('For "import *" find a specific completion for known lib [fstat]', async () => {
        ...
        >("vscode.executeCompletionItemProvider", textDocument.uri, position);

        const indexOfFstat = list!.items.findIndex(
            (val: vscode.CompletionItem) => val.label === "fstat"
        );
    ...
    test('For "lambda"', async function() {
        ...
        >("vscode.executeCompletionItemProvider", textDocument.uri, position);
        assert.notEqual(
            list!.items.filter(item => item.label === "append").length,
            0,
            "append not found"
        );
        assert.notEqual(
            list!.items.filter(item => item.label === "clear").length,
            0,
            "clear not found"
        );
        assert.notEqual(
            list!.items.filter(item => item.label === "count").length,
            0,
            "cound not found"
        );
    });
    ...
    test('For "abc.decorators"', async () => {
        ...
        assert.notEqual(
            list!.items.filter(item => item.label === "ABCMeta").length,
            0,
            "ABCMeta not found"
        );
        assert.notEqual(
    ...
    test("For custom class", done => {
        ...
        vscode.workspace
            ...
            .then(list => {
                assert.notEqual(
                    list!.items.filter(item => item.label === "method1").length,
                    0,
                    "method1 not found"
                );
                assert.notEqual(
                    list!.items.filter(item => item.label === "method2").length,
                    0,
                    "method2 not found"
                );
            })
    ...
    test("Suppress in strings/comments", async () => {
        ...
        for (let i = 0; i < positions.length; i += 1) {
            ...
            const result = list!.items.filter(item => item.label === "abs")
                .length;
            assert.equal(
                result > 0,
                expected[i],
                `Expected ${expected[i]} at position ${positions[i].line}:${
                    positions[i].character
                } but got ${result}`
            );
        }
    });
...
function checkDocumentation(
    ...
    assert.equal(
        isValidType,
        true,
        "Documentation is neither string nor vscode.MarkdownString"
    );
