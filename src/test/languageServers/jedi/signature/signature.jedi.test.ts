...
suite("Signatures (Jedi)", () => {
    ...
    test("For ctor", async () => {
        ...
        const document = await openDocument(
            path.join(autoCompPath, "classCtor.py")
        );
    ...
    test("For ellipsis", async function() {
        ...
        const document = await openDocument(
            path.join(autoCompPath, "ellipsis.py")
        );
        for (let i = 0; i < expected.length; i += 1) {
            await checkSignature(expected[i], document!.uri, i);
        }
    ...
    test("For pow", async () => {
        ...
        const document = await openDocument(
            path.join(autoCompPath, "noSigPy3.py")
        );
...
async function checkSignature(
    ...
    if (expected.signaturesCount > 0) {
        ...
        if (expected.parameterName) {
            const parameter = actual!.signatures[0].parameters[
                expected.activeParameter
            ];
            assert.equal(
                parameter.label,
                expected.parameterName,
                `Parameter name is incorrect, case ${caseIndex}`
            );
        }
    }
}
