...
} from "../../../client/providers/symbolProvider";
...
suite("Jedi Symbol Provider", () => {
    ...
    test("Ensure symbols are returned", async () => {
        provider = new JediSymbolProvider(
    ...
    test("Ensure IFileSystem.arePathsSame is used", async () => {
        ...
        for (let counter = 0; counter < 3; counter += 1) {
            ...
            fileSystem
                .setup(fs =>
                    fs.arePathsSame(
                        TypeMoq.It.isValue(counter.toString()),
                        TypeMoq.It.isValue(__filename)
                    )
                )
                .returns(() => false)
    ...
    test("Ensure symbols are returned - minimal", async () => {
        ...
        const expected = [
            new SymbolInformation(
                ...
                SymbolKind.Class,
                "",
                new Location(uri, new Range(2, 6, 2, 15))
            ),
            new SymbolInformation(
                "test_all",
                SymbolKind.Function,
            ...
            new SymbolInformation(
                ...
                SymbolKind.Variable,
                "test_all",
                new Location(uri, new Range(3, 17, 3, 21))
            ),
            new SymbolInformation(
                "assertTrue",
                SymbolKind.Variable,
