...
suite("Completion Provider", () => {
    ...
    setup(() => {
        ...
        jediHandler = TypeMoq.Mock.ofType<
            JediProxyHandler<ICompletionResult>
        >();
    ...
    async function testDocumentation(source: string, addBrackets: boolean) {
        ...
        const [item] = await completionSource.getVsCodeCompletionItems(
            doc.object,
            position,
            token
        );
