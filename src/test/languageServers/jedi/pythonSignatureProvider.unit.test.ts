...
import {
    IArgumentsResult,
    JediProxyHandler
} from "../../../client/providers/jediProxy";
...
suite("Signature Provider unit tests", () => {
    ...
    setup(() => {
        ...
        jediFactory
            .setup(j => j.getJediProxyHandler(TypeMoq.It.isAny()))
    ...
    function testSignatureReturns(
        ...
        argsResult
            .setup(c => c.definitions)
            .returns(() => argResultItems[0].definitions);
        jediHandler
        ...
        return pySignatureProvider.provideSignatureHelp(
            doc.object,
            position,
            cancelToken.object
        );
    ...
    function testIsInsideStringOrComment(
        sourceLine: string,
        sourcePos: number
    ): boolean {
        const textLine: TypeMoq.IMock<TextLine> = TypeMoq.Mock.ofType<
            TextLine
        >();
        textLine.setup(t => t.text).returns(() => sourceLine);
        const doc: TypeMoq.IMock<TextDocument> = TypeMoq.Mock.ofType<
            TextDocument
        >();
    ...
    test("Ensure no signature is given within a string.", async () => {
        ...
        expect(sigHelp.signatures.length).to.equal(
            0,
            "Signature provided for symbols within a string?"
        );
    });
    test("Ensure no signature is given within a line comment.", async () => {
        ...
        expect(sigHelp.signatures.length).to.equal(
            0,
            "Signature provided for symbols within a full-line comment?"
        );
    });
    test("Ensure no signature is given within a comment tailing a command.", async () => {
        ...
        expect(sigHelp.signatures.length).to.equal(
            0,
            "Signature provided for symbols within a trailing comment?"
        );
    });
    ...
    test("Ensure isPositionInsideStringOrComment is behaving as expected.", () => {
        ...
        const isInsideStrComment: boolean = testIsInsideStringOrComment(
            sourceLine,
            sourcePos
        );
