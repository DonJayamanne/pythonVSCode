...
suite("Code, Hover Definition and Intellisense (Jedi)", () => {
    ...
    test("All three together", async () => {
        ...
        const actualPath = IS_WINDOWS
            ? codeDef![0].uri.fsPath.toUpperCase()
            : codeDef![0].uri.fsPath;
        assert.equal(actualPath, expectedPath, "Incorrect file");
        assert.equal(
            `${codeDef![0].range!.start.line},${
                codeDef![0].range!.start.character
            }`,
            "17,4",
            "Start position is incorrect"
        );
        assert.equal(
            `${codeDef![0].range!.end.line},${
                codeDef![0].range!.end.character
            }`,
            "21,11",
        ...
        assert.equal(
            `${hoverDef![0].range!.start.line},${
                hoverDef![0].range!.start.character
            }`,
            "30,4",
            "Start position is incorrect"
        );
        assert.equal(
            `${hoverDef![0].range!.end.line},${
                hoverDef![0].range!.end.character
            }`,
            "30,11",
            "End position is incorrect"
        );
        ...
        const expectedContent =
            ...
            "This is method1";
        assert.equal(
            normalizeMarkedString(hoverDef![0].contents[0]),
            expectedContent,
