...
const autoPep8FileToFormat = path.join(
    formatFilesPath,
    "autoPep8FileToFormat.py"
);
...
suite("Formatting - General", () => {
    ...
    suiteSetup(async () => {
        ...
        const yapf = pythonProcess.execModule(
            "yapf",
            [originalUnformattedFile],
            { cwd: workspaceRootPath }
        );
        const autoPep8 = pythonProcess.execModule(
            "autopep8",
            [originalUnformattedFile],
            { cwd: workspaceRootPath }
        );
        ...
        if (await formattingTestIsBlackSupported()) {
            ...
            const black = pythonProcess.execModule(
                "black",
                [blackReferenceFile],
                { cwd: workspaceRootPath }
            );
    ...
    async function formattingTestIsBlackSupported(): Promise<boolean> {
        ...
        return !(await isPythonVersionInProcess(
            processService,
            "2",
            "3.0",
            "3.1",
    ...
    async function injectFormatOutput(outputFileName: string) {
        ...
        procService.onExecObservable((file, args, options, callback) => {
            if (args.indexOf("--diff") >= 0) {
                callback({
                    out: fs.readFileSync(
                        path.join(formatFilesPath, outputFileName),
                        "utf8"
                    ),
                    source: "stdout"
                });
    ...
    test("Yapf", async () =>
        testFormatting(
            new YapfFormatter(ioc.serviceContainer),
            formattedYapf,
            yapfFileToFormat,
            "yapf.output"
        ));
