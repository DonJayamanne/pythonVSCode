...
suite("Formatting - Dispatcher", () => {
    ...
    test("No providers", async () => {
        ...
        assert.deepStrictEqual(
            result,
            [],
            "Did not return an empty list of edits"
        );
    });
    ...
    test("Two providers", () => {
        const colonProvider = setupProvider(
            doc.object,
            pos.object,
            ":",
            opt.object,
        ...
        assert.equal(
            result,
            edits.object,
            "Did not return correct editsfor colon provider"
        );
        ...
        assert.equal(
            result2,
            edits2.object,
            "Did not return correct edits for newline provider"
        );
