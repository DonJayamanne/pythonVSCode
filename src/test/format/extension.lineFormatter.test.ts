...
suite("Formatting - line formatter", () => {
    ...
    test("Brace after keyword", () => {
        ...
        testFormatLine(
            "if (True|False)and(False/True)not (! x )",
            "if (True | False) and (False / True) not (!x)"
        );
    ...
    test("Grammar file", () => {
        ...
        for (let i = 0; i < lines.length; i += 1) {
            ...
            assert.equal(
                actual,
                line,
                `Line ${i + 1} changed: '${line.trim()}' to '${actual.trim()}'`
            );
        }
    });
    ...
    function formatMultiline(content: string, lineNumber: number): string {
        ...
        document.setup(x => x.getText(TypeMoq.It.isAny())).returns(o => {
            ...
            if (r.start.line === r.end.line) {
                return lines[r.start.line].substring(
                    r.start.character,
                    r.end.character
                );
            }
