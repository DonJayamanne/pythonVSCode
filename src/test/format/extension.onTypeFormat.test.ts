...
const srcPythoFilesPath = path.join(
    __dirname,
    "..",
    "..",
    "..",
...
const outPythoFilesPath = path.join(
    __dirname,
    "pythonFiles",
    "typeFormatFiles"
);
...
const elseBlockTabOutFilePath = path.join(
    outPythoFilesPath,
    "elseBlocksTab.py"
);

const elseBlockFirstLine2OutFilePath = path.join(
    outPythoFilesPath,
    "elseBlocksFirstLine2.py"
);
const elseBlockFirstLine4OutFilePath = path.join(
    outPythoFilesPath,
    "elseBlocksFirstLine4.py"
);
const elseBlockFirstLineTabOutFilePath = path.join(
    outPythoFilesPath,
    "elseBlocksFirstLineTab.py"
);
        ...
        .then(
            edits => {
                ...
                edits.forEach((edit, index) => {
                    ...
                    const providedRange = `${edit.range.start.line},${
                        edit.range.start.character
                    },${edit.range.end.line},${edit.range.end.character}`;
                    const expectedRange = `${expectedEdit.range.start.line},${
                        expectedEdit.range.start.character
                    },${expectedEdit.range.end.line},${
                        expectedEdit.range.end.character
                    }`;
                    assert.ok(
...
suite("Try blocks with indentation of 2 spaces", () => {
    ...
    const testCases: ITestCase[] = [
        ...
        {
            title:
                "except off by one inside a for loop should not be formatted",
            line: 47,
            column: 12,
            expectedEdits: []
        },
        ...
        {
            title:
                "except ValueError as err: off by one inside a function should not be formatted",
            line: 157,
            column: 25,
            expectedEdits: []
        },
