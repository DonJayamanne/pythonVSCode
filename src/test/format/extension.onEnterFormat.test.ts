...
const formatFilesPath = path.join(
    EXTENSION_ROOT_DIR,
    "src",
    "test",
    "pythonFiles",
...
suite("Formatting - OnEnter provider", () => {
    ...
    test("Formatting line ending in comment", () =>
        testFormattingAtPosition(6, 0, "x + 1  # "));

    test("Formatting line with @", () => doesNotFormat(7, 0));

    test("Formatting line with @", () => doesNotFormat(8, 0));

    test("Formatting line with unknown neighboring tokens", () =>
        testFormattingAtPosition(9, 0, "if x <= 1:"));

    test("Formatting line with unknown neighboring tokens", () =>
        testFormattingAtPosition(10, 0, "if 1 <= x:"));

    test("Formatting method definition with arguments", () =>
        testFormattingAtPosition(11, 0, "def __init__(self, age=23)"));

    test("Formatting space after open brace", () =>
        testFormattingAtPosition(12, 0, "while (1)"));

    test("Formatting line ending in string", () =>
        testFormattingAtPosition(13, 0, 'x + """'));

    function testFormattingAtPosition(
        line: number,
        character: number,
