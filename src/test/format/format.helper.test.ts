...
suite("Formatting - Helper", () => {
    ...
    setup(() => {
        ...
        config
            ...
            .returns(() => getExtensionSettings(undefined));

        ioc.serviceManager.addSingletonInstance<IConfigurationService>(
            IConfigurationService,
            config.object
        );
    ...
    test("Ensure product is set in Execution Info", async () => {
        [Product.autopep8, Product.black, Product.yapf].forEach(formatter => {
            ...
            assert.equal(
                info.product,
                formatter,
                `Incorrect products for ${formatHelper.translateToId(
                    formatter
                )}`
            );
        });
    ...
    test("Ensure executable is set in Execution Info", async () => {
        ...
        [Product.autopep8, Product.black, Product.yapf].forEach(formatter => {
            ...
            assert.equal(
                info.execPath,
                execPath,
                `Incorrect executable paths for product ${formatHelper.translateToId(
                    formatter
                )}`
            );
        });
    ...
    test("Ensure arguments are set in Execution Info", async () => {
        ...
        [Product.autopep8, Product.black, Product.yapf].forEach(formatter => {
            ...
            assert.equal(
                ...
                true,
                `Incorrect custom arguments for product ${formatHelper.translateToId(
                    formatter
                )}`
            );
        });
    ...
    test("Ensure correct setting names are returned", async () => {
        [Product.autopep8, Product.black, Product.yapf].forEach(formatter => {
            ...
            assert.deepEqual(
                ...
                settings,
                `Incorrect settings for product ${formatHelper.translateToId(
                    formatter
                )}`
            );
        });
    ...
    test("Ensure translation of ids works", async () => {
        ...
        [Product.autopep8, Product.black, Product.yapf].forEach(formatter => {
            ...
            assert.equal(
                ...
                `Incorrect translation for product ${formatHelper.translateToId(
                    formatter
                )}`
            );
        });
