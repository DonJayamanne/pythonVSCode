...
suite("SocketStream", () => {
    ...
    test("Read Ascii String", done => {
        ...
        let buffer = Buffer.concat([
            new Buffer("A"),
            uint64be.encode(message.length),
            new Buffer(message)
        ]);
        const stream = new SocketStream((socket as any) as net.Socket, buffer);
    ...
    test("Read RollBackTransaction", done => {
        ...
        const partOfSecondMessage = Buffer.concat([
            new Buffer("A"),
            uint64be.encode(message.length)
        ]);
        ...
        assert.equal(
            stream.ReadString(),
            message,
            "First message not read properly"
        );
        const secondMessage = stream.ReadString();
        assert.equal(
            stream.HasInsufficientDataForReading,
            true,
            "Should not have sufficient data for reading"
        );
    ...
    test("Read EndTransaction", done => {
        ...
        assert.notEqual(
            stream.ReadString(),
            message,
            "First message cannot be read after commit transaction"
        );
        done();
    });
