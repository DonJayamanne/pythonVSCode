...
suite("FileSystem", () => {
    ...
    const fileToAppendTo = path.join(
        __dirname,
        "created_for_testing_dummy.txt"
    );
    setup(() => {
    ...
    test("ReadFile returns contents of a file", async () => {
        ...
        const expectedContents = await fs
            .readFile(file)
    ...
    function caseSensitivityFileCheck(
        isWindows: boolean,
        isOsx: boolean,
        isLinux: boolean
    ) {
        ...
        if (isWindows) {
            expect(fileSystem.arePathsSame(path1, path2)).to.be.equal(
                true,
                "file paths do not match (windows)"
            );
        } else {
            expect(fileSystem.arePathsSame(path1, path2)).to.be.equal(
                false,
                "file match (non windows)"
            );
        }

        expect(fileSystem.arePathsSame(path1, path1)).to.be.equal(
            true,
            "1. file paths do not match"
        );
        expect(fileSystem.arePathsSame(path2, path2)).to.be.equal(
            true,
            "2. file paths do not match"
        );
        expect(fileSystem.arePathsSame(path1, path3)).to.be.equal(
            false,
            "2. file paths do not match"
        );
    }
    ...
    test("Check existence of files synchronously", async () => {
        expect(fileSystem.fileExistsSync(__filename)).to.be.equal(
            true,
            "file not found"
        );
    });
    ...
    test("Ensure creating a temporary file results in a unique temp file path", async () => {
        ...
        expect(tempFile.filePath).to.not.equal(
            tempFile2.filePath,
            "Temp files must be unique, implementation of createTemporaryFile is off."
        );
    });
    test("Ensure writing to a temp file is supported via file stream", async () => {
        await fileSystem.createTemporaryFile(".tmp").then(
            (tf: TemporaryFile) => {
                expect(tf).to.not.equal(
                    undefined,
                    "Error trying to create a temporary file"
                );
                const writeStream = fileSystem.createWriteStream(tf.filePath);
                writeStream.write("hello", "utf8", err => {
                    expect(err).to.equal(
                        undefined,
                        `Failed to write to a temp file, error is ${err}`
                    );
                });
            },
            failReason => {
                expect(failReason).to.equal(
                    "No errors occured",
                    `Failed to create a temporary file with error ${failReason}`
                );
            }
        );
    });
        ...
        await fileSystem
            ...
            .then(async (fl: TemporaryFile) => {
                await fileSystem.chmod(fl.filePath, "7777").then(
                    ...
                    failReason => {
                        expect(failReason).to.equal(
                            "There was no error using chmod",
                            `Failed to perform chmod operation successfully, got error ${failReason}`
                        );
                    }
                );
            });
