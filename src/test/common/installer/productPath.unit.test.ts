...
suite("Product Path", () => {
    [undefined, Uri.file("resource")].forEach(resource => {
        getNamesAndValues<Product>(Product).forEach(product => {
            ...
            let workspaceSymnbolSettings: TypeMoq.IMock<
                IWorkspaceSymbolSettings
            >;
            ...
            setup(() => {
                ...
                pythonSettings
                    .setup(p => p.formatting)
                    .returns(() => formattingSettings.object);
                pythonSettings
                    .setup(p => p.unitTest)
                    .returns(() => unitTestSettings.object);
                pythonSettings
                    .setup(p => p.workspaceSymbols)
                    .returns(() => workspaceSymnbolSettings.object);
                configService
            ...
            const productType = new ProductService().getProductType(
                product.value
            );
            switch (productType) {
                case ProductType.Formatter: {
                    test(`Ensure path is returned for ${product.name} (${
                        ...
                        const productPathService = new FormatterProductPathService(
                            serviceContainer.object
                        );
                        ...
                        serviceContainer
                            .setup(s =>
                                s.get(
                                    TypeMoq.It.isValue(IFormatterHelper),
                                    TypeMoq.It.isAny()
                                )
                            )
                        ...
                        const value = productPathService.getExecutableNameFromSettings(
                            product.value,
                            resource
                        );
                ...
                case ProductType.Linter: {
                    test(`Ensure path is returned for ${product.name} (${
                        ...
                        const productPathService = new LinterProductPathService(
                            serviceContainer.object
                        );
                        ...
                        serviceContainer
                            .setup(s =>
                                s.get(
                                    TypeMoq.It.isValue(ILinterManager),
                                    TypeMoq.It.isAny()
                                )
                            )
                        ...
                        linterInfo
                            .setup(l =>
                                l.pathName(TypeMoq.It.isValue(resource))
                            )
                            .returns(() => expectedPath)
                            .verifiable(TypeMoq.Times.once());
                        linterManager
                ...
                case ProductType.RefactoringLibrary: {
                    test(`Ensure path is returned for ${product.name} (${
                        ...
                        const productPathService = new RefactoringLibraryProductPathService(
                            serviceContainer.object
                        );
                ...
                case ProductType.WorkspaceSymbols: {
                    test(`Ensure path is returned for ${product.name} (${
                        ...
                        const productPathService = new CTagsProductPathService(
                            serviceContainer.object
                        );
                        const expectedPath = "Some Path";
                ...
                case ProductType.TestFramework: {
                    test(`Ensure path is returned for ${product.name} (${
                        ...
                        const productPathService = new TestFrameworkProductPathService(
                            serviceContainer.object
                        );
                        ...
                        serviceContainer
                            .setup(s =>
                                s.get(
                                    TypeMoq.It.isValue(ITestsHelper),
                                    TypeMoq.It.isAny()
                                )
                            )
                ...
                default: {
                    test(`No tests for Product Path of this Product Type ${
                        product.name
                    }`, () => {
