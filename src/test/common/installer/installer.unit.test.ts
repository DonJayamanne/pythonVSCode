    ...
    [undefined, Uri.file("resource")].forEach(resource => {
        getNamesAndValues<Product>(Product).forEach(product => {
            ...
            setup(() => {
                ...
                serviceContainer
                    .setup(c =>
                        c.get(
                            TypeMoq.It.isValue(IInstallationChannelManager),
                            TypeMoq.It.isAny()
                        )
                    )
                ...
                serviceContainer
                    .setup(c =>
                        c.get(
                            TypeMoq.It.isValue(IPersistentStateFactory),
                            TypeMoq.It.isAny()
                        )
                    )
                ...
                serviceContainer
                    .setup(c =>
                        c.get(
                            TypeMoq.It.isValue(IProductPathService),
                            TypeMoq.It.isAny()
                        )
                    )
                ...
                productPathService
                    .setup(p =>
                        p.getExecutableNameFromSettings(
                            TypeMoq.It.isAny(),
                ...
                installer = new ProductInstaller(
            ...
            switch (product.value) {
                ...
                case Product.unittest: {
                    test(`Ensure resource info is passed into the module installer ${
                        product.name
                    } (${
                        resource ? "With a resource" : "without a resource"
                    })`, async () => {
                        const response = await installer.install(
                            product.value,
                            resource
                        );
                ...
                default: {
                    test(`Ensure resource info is passed into the module installer ${
                        ...
                        moduleInstaller
                            ...
                            .returns(() =>
                                Promise.reject(new Error("UnitTesting"))
                            );
                    ...
                    if (product.value !== Product.unittest) {
                        test(`Ensure the prompt is displayed only once, until the prompt is closed, ${
                            ...
                            workspaceService
                                .setup(w =>
                                    w.getWorkspaceFolder(
                                        TypeMoq.It.isValue(resource!)
                                    )
                                )
                                .returns(
                                    () =>
                                        TypeMoq.Mock.ofType<WorkspaceFolder>()
                                            .object
                                )
                                .verifiable(
                                    TypeMoq.Times.exactly(resource ? 5 : 0)
                                );
                            app.setup(a =>
                                a.showErrorMessage(
                                    TypeMoq.It.isAny(),
                            ...
                            persistVal.setup(p =>
                                p.updateValue(TypeMoq.It.isValue(true))
                            );
                            persistentStore
                                .setup(ps =>
                            ...
                            installer
                                .promptToInstall(product.value, resource)
                                .ignoreErrors();
                        ...
                        if (product.value === Product.pylint) {
                            test(`Ensure the install prompt is not displayed when the user requests it not be shown again, ${
                                ...
                                workspaceService
                                    ...
                                    .verifiable(
                                        TypeMoq.Times.exactly(resource ? 2 : 0)
                                    );
                                app.setup(a =>
                                    a.showErrorMessage(
                                        TypeMoq.It.isAnyString(),
                                ...
                                // Display first prompt.
                                const initialResponse = await installer.promptToInstall(
                                    product.value,
                                    resource
                                );

                                // Display a second prompt.
                                const secondResponse = await installer.promptToInstall(
                                    product.value,
                                    resource
                                );

                                expect(initialResponse).to.be.equal(
                                    InstallerResponse.Ignore
                                );
                                expect(secondResponse).to.be.equal(
                                    InstallerResponse.Ignore
                                );
                            ...
                            productService.getProductType(product.value) ===
                            ProductType.Linter
                        ) {
                            test(`Ensure the 'do not show again' prompt isn't shown for non-pylint linters, ${
                                ...
                                // Display the prompt.
                                await installer.promptToInstall(
                                    product.value,
                                    resource
                                );
                        ...
                        test(`Ensure the prompt is displayed again when previous prompt has been closed, ${
                            ...
                            workspaceService
                                ...
                                .verifiable(
                                    TypeMoq.Times.exactly(resource ? 3 : 0)
                                );
                            app.setup(a =>
                                a.showErrorMessage(
                                    TypeMoq.It.isAny(),
        ...
        suite("Test LinterInstaller.promptToInstallImplementation", () => {
            class LinterInstallerTest extends LinterInstaller {
                ...
                public async promptToInstallImplementation(
                    product: Product,
                    uri?: Uri
            ...
            setup(() => {
                ...
                ).thenReturn(instance(appShell));
                when(
                    serviceContainer.get<IConfigurationService>(
                        IConfigurationService
                    )
                ...
                ).thenReturn(instance(productService));
                when(
                    serviceContainer.get<ICommandManager>(ICommandManager)
                ).thenReturn(instance(cmdManager));

                installer = new LinterInstallerTest(
            ...
            test("Ensure 3 options for pylint", async () => {
                ...
                await installer.promptToInstallImplementation(
                    product,
                    resource
                );
                verify(
            ...
            test("Ensure select linter command is invoked", async () => {
                ...
                when(
                    cmdManager.executeCommand(Commands.Set_Linter)
                ).thenResolve(undefined);

                const response = await installer.promptToInstallImplementation(
                    product,
                    resource
                );
