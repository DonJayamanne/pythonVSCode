...
suite("Module Installer - Invalid Paths", () => {
    [undefined, Uri.file("resource")].forEach(resource => {
        ["moduleName", path.join("users", "dev", "tool", "executable")].forEach(
            pathToExecutable => {
                const isExecutableAModule =
                    path.basename(pathToExecutable) === pathToExecutable;
                ...
                getNamesAndValues<Product>(Product).forEach(product => {
                    ...
                    switch (product.value) {
                        ...
                        default: {
                            test(`Ensure invalid path message is ${
                                isExecutableAModule
                                    ? "not displayed"
                                    : "displayed"
                            } ${product.name} (${
                                ...
                                productPathService
                                    ...
                                    .verifiable(
                                        TypeMoq.Times.atLeast(
                                            isExecutableAModule ? 0 : 1
                                        )
                                    );
                                productPathService
                                    .setup(p =>
                                ...
                                const anyParams = [0, 1, 2, 3, 4, 5].map(() =>
                                    TypeMoq.It.isAny()
                                );
                                app.setup(a =>
                                    a.showErrorMessage(
                                ...
                                persistValue.setup(pv =>
                                    pv.updateValue(TypeMoq.It.isValue(true))
                                );
                                persistentState
                                    .setup(ps =>
