...
import {
    PipEnvInstaller,
    pipenvName
} from "../../../client/common/installer/pipEnvInstaller";
...
suite("Module Installer", () => {
    ...
    [CondaInstaller, PipInstaller, PipEnvInstaller].forEach(installerClass => {
        ...
        const proxyServers =
            installerClass === PipInstaller ? ["", "proxy:1234"] : [""];
        proxyServers.forEach(proxyServer => {
            [undefined, Uri.file("/users/dev/xyz")].forEach(resource => {
                ...
                [undefined, ...condaEnvs].forEach(condaEnvInfo => {
                    const testProxySuffix =
                        proxyServer.length === 0
                            ? "without proxy info"
                            : "with proxy info";
                    const testCondaEnv = condaEnvInfo
                        ? condaEnvInfo.name
                            ...
                            : "with conda path"
                        : "without conda";
                    const testSuite = [testProxySuffix, testCondaEnv]
                    ...
                    suite(`${installerClass.name} (${testSuite})`, () => {
                        ...
                        setup(() => {
                            ...
                            condaService
                                ...
                                .returns(() =>
                                    Promise.resolve(condaExecutable)
                                );
                            condaService
                                .setup(c =>
                                    c.getCondaEnvironment(TypeMoq.It.isAny())
                                )
                        ...
                        function setActiveInterpreter(
                            ...
                            interpreterService
                                ...
                                .returns(() =>
                                    Promise.resolve(activeInterpreter)
                                )
                        ...
                        getModuleNamesForTesting().forEach(product => {
                            ...
                            if (product.value === Product.pylint) {
                                ...
                                generatePythonInterpreterVersions().forEach(
                                    interpreterInfo => {
                                            ...
                                            if (
                                                ...
                                                test(testTitle, async () => {
                                                    ...
                                                    if (
                                                        ...
                                                        expectedArgs.push(
                                                            condaEnvInfo.name.toCommandArgument()
                                                        );
                                                    } else if (
                                                        condaEnvInfo &&
                                                        condaEnvInfo.path
                                                        ...
                                                        expectedArgs.push(
                                                            condaEnvInfo.path.fileToCommandArgument()
                                                        );
                                                    }
                                                    expectedArgs.push(
                                                        '"pylint<2.0.0"'
                            ...
                            if (installerClass === PipInstaller) {
                                test(`Ensure getActiveInterperter is used in PipInstaller (${
                                    product.name
                                })`, async () => {
                                    setActiveInterpreter();
                                    try {
...
function getModuleNamesForTesting(): {
    ...
    return getNamesAndValues<Product>(Product)
        .map(product => {
            ...
            try {
                const prodInstaller = new ProductInstaller(
                    mockSvc,
                    mockOutChnl
                );
                moduleName = prodInstaller.translateProductToModuleName(
                    product.value,
