...
suite("Terminal Service Factory", () => {
    ...
    setup(() => {
        ...
        serviceContainer
            .setup(c =>
                c.get(
                    TypeMoq.It.isValue(IInterpreterService),
                    TypeMoq.It.isAny()
                )
            )
        ...
        serviceContainer
            .setup(c =>
                c.get(TypeMoq.It.isValue(ITerminalHelper), TypeMoq.It.isAny())
            )
            .returns(() => terminalHelper.object);
        const terminalManager = TypeMoq.Mock.ofType<ITerminalManager>();
        serviceContainer
            .setup(c =>
                c.get(TypeMoq.It.isValue(ITerminalManager), TypeMoq.It.isAny())
            )
    ...
    test("Ensure same instance of terminal service is returned", () => {
        ...
        const differentInstance = factory.getTerminalService(
            undefined,
            "New Title"
        );
    ...
    test("Ensure different instance of terminal service is returned when title is provided", () => {
        ...
        expect(defaultInstance instanceof TerminalService).to.equal(
            true,
            "Not an instance of Terminal service"
        );

        const notSameAsDefaultInstance =
            factory.getTerminalService(undefined, "New Title") ===
            defaultInstance;
        expect(notSameAsDefaultInstance).to.not.equal(
            true,
            "Instances are the same as default instance"
        );

        const instance = factory.getTerminalService(undefined, "New Title");
        const sameInstance =
            factory.getTerminalService(undefined, "New Title") === instance;
        expect(sameInstance).to.equal(true, "Instances are not the same");
    ...
    test("Ensure different instance of terminal services are created", () => {
        ...
        expect(instance1 instanceof TerminalService).to.equal(
            true,
            "Not an instance of Terminal service"
        );

        const notSameAsFirstInstance =
            factory.createTerminalService() === instance1;
        expect(notSameAsFirstInstance).to.not.equal(
            true,
        ...
        expect(notSameAsSecondInstance).to.not.equal(
            true,
            "Instances are the same"
        );
        ...
        expect(notSameAsThirdInstance).to.not.equal(
            true,
            "Instances are the same"
        );
    });
    ...
    test("Ensure same terminal is returned when using resources from the same workspace", () => {
        ...
        const terminalsAreSameForWorkspaceA =
            terminalForFile1A === terminalForFile2A;
        expect(terminalsAreSameForWorkspaceA).to.equal(
            true,
            "Instances are not the same for Workspace A"
        );

        const terminalsForWorkspaceABAreDifferent =
            terminalForFile1A === terminalForFileB;
