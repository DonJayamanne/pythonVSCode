...
suite("Terminal Service", () => {
    ...
    setup(() => {
        ...
        mockServiceContainer
            .setup(c => c.get(ITerminalManager))
            .returns(() => terminalManager.object);
        mockServiceContainer
            .setup(c => c.get(ITerminalHelper))
            .returns(() => terminalHelper.object);
        mockServiceContainer
            .setup(c => c.get(IPlatformService))
            .returns(() => platformService.object);
        mockServiceContainer
            .setup(c => c.get(IDisposableRegistry))
            .returns(() => disposables);
        mockServiceContainer
            .setup(c => c.get(IWorkspaceService))
            .returns(() => workspaceService.object);
        mockServiceContainer
            .setup(c => c.get(ITerminalActivator))
    ...
    test("Ensure terminal is disposed", async () => {
        terminalHelper
            .setup(helper =>
                helper.getEnvironmentActivationCommands(
                    TypeMoq.It.isAny(),
                    TypeMoq.It.isAny()
                )
            )
        ...
        workspaceService
            .setup(w =>
                w.getConfiguration(
                    TypeMoq.It.isValue("terminal.integrated.shell")
                )
            )
            .returns(() => {
        ...
        terminal.verify(
            t => t.show(TypeMoq.It.isValue(true)),
    ...
    test("Ensure command is sent to terminal and it is shown", async () => {
        ...
        terminalHelper
            .setup(h => h.identifyTerminalShell(TypeMoq.It.isAny()))
            .returns(() => TerminalShellType.bash);
        terminalManager
            .setup(t => t.createTerminal(TypeMoq.It.isAny()))
            .returns(() => terminal.object);
        ...
        terminal.verify(
            t =>
                t.sendText(
                    TypeMoq.It.isValue(commandToExpect),
    ...
    test("Ensure text is sent to terminal and it is shown", async () => {
        ...
        terminal.verify(
            t => t.sendText(TypeMoq.It.isValue(textToSend)),
            TypeMoq.Times.exactly(1)
        );
    });
    ...
    test("Ensure terminal shown and focus is set to the Terminal", async () => {
        ...
        terminal.verify(
            t => t.show(TypeMoq.It.isValue(false)),
            TypeMoq.Times.exactly(2)
        );
    });
    ...
    test("Ensure terminal is activated once after creation", async () => {
        ...
        terminalActivator
            .setup(h =>
                h.activateEnvironmentInTerminal(
                    TypeMoq.It.isAny(),
                    TypeMoq.It.isAny(),
                    TypeMoq.It.isAny()
                )
    ...
    test("Ensure close event is not fired when another terminal is closed", async () => {
        ...
        expect(eventHandler).not.to.be.an(
            "undefined",
            "event handler not initialized"
        );
