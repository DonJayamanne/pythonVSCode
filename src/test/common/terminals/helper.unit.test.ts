...
suite("Terminal Service helpers", () => {
    ...
    suite("Misc", () => {
        ...
        test("Create terminal without a title", () => {
            ...
            when(terminalManager.createTerminal(anything())).thenReturn(
                terminal as any
            );
            ...
            expect(args.name).to.be.deep.equal(
                undefined,
        ...
        test("Create terminal with a title", () => {
            ...
            const term = helper.createTerminal(title);

            verify(terminalManager.createTerminal(anything())).once();
            const args = capture(terminalManager.createTerminal).first()[0];
            expect(term).to.be.deep.equal(terminal);
            expect(args.name).to.be.deep.equal(title);
        });
        test("Test identification of Terminal Shells", async () => {
            ...
            shellPathsAndIdentification.set(
                "c:\\windows\\system32\\gitbash.exe",
            ...
            shellPathsAndIdentification.set(
                "/usr/microsoft/xxx/powershell/pwsh",
                TerminalShellType.powershellCore
            );
            ...
            shellPathsAndIdentification.forEach((shellType, shellPath) => {
                expect(helper.identifyTerminalShell(shellPath)).to.equal(
                    shellType,
                    `Incorrect Shell Type for path '${shellPath}'`
                );
            });
        });
        async function ensurePathForShellIsCorrectlyRetrievedFromSettings(
            osType: OSType,
            expectedShellPath: string
        ) {
            ...
            when(
                workspaceService.getConfiguration("terminal.integrated.shell")
            ).thenReturn(workspaceConfig.object);

            const shellPath = helper.getTerminalShellPath();

            workspaceConfig.verifyAll();
            expect(shellPath).to.equal(
                expectedShellPath,
                "Incorrect path for Osx"
            );
        }
        test("Ensure path for shell is correctly retrieved from settings (osx)", async () => {
            await ensurePathForShellIsCorrectlyRetrievedFromSettings(
                OSType.OSX,
                "abcd"
            );
        });
        ...
        test("Ensure spaces in command is quoted", async () => {
            getNamesAndValues<TerminalShellType>(TerminalShellType).forEach(
                item => {
                    ...
                    const args = ["1", "2"];
                    const commandPrefix =
                        item.value === TerminalShellType.powershell ||
                        item.value === TerminalShellType.powershellCore
                            ? "& "
                            : "";
                    const expectedTerminalCommand = `${commandPrefix}${command.fileToCommandArgument()} 1 2`;

                    const terminalCommand = helper.buildCommandForTerminal(
                        item.value,
                        command,
                        args
                    );
                    expect(terminalCommand).to.equal(
                        expectedTerminalCommand,
                        `Incorrect command for Shell ${item.name}`
                    );
                }
            );
        });
    ...
    suite("Activation", () => {
        [undefined, Uri.parse("x")].forEach(resource => {
            suite(resource ? "With a resource" : "Without a resource", () => {
                setup(() => {
                    doSetup();
                    when(configurationService.getSettings(resource)).thenReturn(
                        instance(pythonSettings)
                    );
                });
                test("Activation command must be empty if activation of terminals is disabled", async () => {
                    when(pythonSettings.terminal).thenReturn({
                ...
                function ensureCondaIsSupported(
                    ...
                    when(
                        condaService.isCondaEnvironment(pythonPath)
                    ).thenResolve(isSupported);
                    when(
                        condaActivationProvider.getActivationCommands(
                            resource,
                            anything()
                        )
                    ).thenResolve(condaActivationCommands);
                }
                ...
                test("Activation command must return undefined if none of the proivders support the shell", async () => {
                    ...
                    when(
                        bashActivationProvider.isShellSupported(anything())
                    ).thenReturn(false);
                    when(
                        cmdActivationProvider.isShellSupported(anything())
                    ).thenReturn(false);
                    when(
                        pyenvActivationProvider.isShellSupported(anything())
                    ).thenReturn(false);
                    when(
                        pipenvActivationProvider.isShellSupported(anything())
                    ).thenReturn(false);
                ...
                test("Activation command must return command from bash if that is supported and others are not", async () => {
                    ...
                    when(
                        bashActivationProvider.getActivationCommands(
                            resource,
                            anything()
                        )
                    ).thenResolve(expectCommand);
                ...
                test("Activation command must return command from pipenv if that is supported and even if others are supported", async () => {
                    ...
                    when(
                        pipenvActivationProvider.getActivationCommands(
                            resource,
                            anything()
                        )
                    ).thenResolve(expectCommand);
                    ...
                    [
                        ...
                        when(
                            provider.getActivationCommands(resource, anything())
                        ).thenResolve(["Something"]);
                        when(provider.isShellSupported(anything())).thenReturn(
                            true
                        );
                    });
                ...
                test("Activation command must return command from Command Prompt if that is supported and others are not", async () => {
                    ...
                    when(
                        cmdActivationProvider.getActivationCommands(
                            resource,
                            anything()
                        )
                    ).thenResolve(expectCommand);
                ...
                [OSType.Linux, OSType.OSX, OSType.Windows].forEach(osType => {
                    test(`Activation command for Shell must never use pipenv nor pyenv (${osType})`, async () => {
                        ...
                        when(platformService.osType).thenReturn(osType);
                        when(
                            bashActivationProvider.isShellSupported(
                                shellToExpect
                            )
                        ).thenReturn(false);
                        when(
                            cmdActivationProvider.isShellSupported(
                                shellToExpect
                            )
                        ).thenReturn(false);
                        ...
                        verify(
                            pipenvActivationProvider.isShellSupported(
                                anything()
                            )
                        ).never();
                        verify(
