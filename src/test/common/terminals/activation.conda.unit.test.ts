...
suite("Terminal Environment Activation conda", () => {
    ...
    setup(() => {
        ...
        serviceContainer
            .setup(c =>
                c.get(TypeMoq.It.isValue(ICondaService), TypeMoq.It.isAny())
            )
        ...
        pythonSettings
            .setup(s => s.terminal)
            .returns(() => terminalSettings.object);
    ...
    test("Ensure no activation commands are returned if the feature is disabled", async () => {
        ...
        expect(activationCommands).to.equal(
            undefined,
            "Activation commands should be undefined"
        );
    });
    ...
    async function expectNoCondaActivationCommandForPowershell(
        isWindows: boolean,
        isOsx: boolean,
        isLinux: boolean,
        pythonPath: string,
        ...
        switch (shellType) {
            ...
            case TerminalShellType.fish: {
                expectedActivationCommamnd = [
                    `conda activate ${envName.toCommandArgument()}`
                ];
                break;
            }
            default: {
                expectedActivationCommamnd = isWindows
    ...
    getNamesAndValues<TerminalShellType>(TerminalShellType).forEach(
        shellType => {
            test(`Conda activation command for shell ${
                shellType.name
            } on (windows)`, async () => {
                ...
                await expectNoCondaActivationCommandForPowershell(
                    true,
                    false,
                    false,
                    pythonPath,
    ...
    getNamesAndValues<TerminalShellType>(TerminalShellType).forEach(
        shellType => {
            test(`Conda activation command for shell ${
                shellType.name
            } on (windows), containing spaces in environment name`, async () => {
                const pythonPath = path.join(
                    "c",
                    "users",
                    "xyz",
            ...
            test(`Conda activation command for shell ${
                shellType.name
            } on (linux), containing spaces in environment name`, async () => {
                const pythonPath = path.join(
                    "users",
                    "xyz",
                    ".conda",
            ...
            test(`Conda activation command for shell ${
                shellType.name
            } on (mac), containing spaces in environment name`, async () => {
                const pythonPath = path.join(
                    "users",
                    "xyz",
                    ".conda",
    ...
    async function expectCondaActivationCommand(
        ...
        const expectedActivationCommand = isWindows
            ? ["activate EnvA"]
    ...
    test("Get activation script command if environment is not a conda environment", async () => {
        ...
        when(
            bash.getActivationCommands(anything(), TerminalShellType.bash)
        ).thenResolve(expectedActivationCommand);
    ...
    test("If environment is a conda environment and environment detection fails, ensure activatino of script is sent (windows)", async () => {
        ...
        fileSystem
            ...
            .returns(() => Promise.resolve(true));
        await expectActivationCommandIfCondaDetectionFails(
            true,
            false,
            false,
            pythonPath,
    ...
    const testsForWindowsActivation: WindowsActivationTestParams[] = [
        ...
        {
            testName:
                "Activation uses full path with spaces on windows for powershell",
        ...
        {
            testName:
                "Activation uses full path on windows under powershell, environment name has spaces",
            basePath: windowsTestPath,
        ...
        {
            testName:
                "Activation uses full path on windows for powershell-core",
            basePath: windowsTestPath,
            envName: "TesterEnv",
            expectedResult: undefined,
            expectedRawCmd: `${path.join(windowsTestPath, "activate")}`,
        ...
        {
            testName:
                "Activation uses full path with spaces on windows for powershell-core",
            basePath: windowsTestPathSpaces,
            envName: "TesterEnv",
            expectedResult: undefined,
            expectedRawCmd: `"${path.join(windowsTestPathSpaces, "activate")}"`,
        ...
        {
            testName:
                "Activation uses full path on windows for powershell-core, environment name has spaces",
            basePath: windowsTestPath,
            envName: "The Tester Environment",
            expectedResult: undefined,
            expectedRawCmd: `${path.join(windowsTestPath, "activate")}`,
        ...
        },
        {
            ...
            envName: "TesterEnv",
            expectedResult: [
                `${path.join(windowsTestPath, "activate")} TesterEnv`
            ],
            expectedRawCmd: `${path.join(windowsTestPath, "activate")}`,
        ...
        {
            testName:
                "Activation uses full path with spaces on windows for cmd.exe",
            basePath: windowsTestPathSpaces,
            envName: "TesterEnv",
            expectedResult: [
                `"${path.join(windowsTestPathSpaces, "activate")}" TesterEnv`
            ],
            expectedRawCmd: `"${path.join(windowsTestPathSpaces, "activate")}"`,
            terminalKind: TerminalShellType.commandPrompt
        },
        {
            testName:
                "Activation uses full path on windows for cmd.exe, environment name has spaces",
            basePath: windowsTestPath,
            envName: "The Tester Environment",
            expectedResult: [
                `${path.join(
    ...
    testsForWindowsActivation.forEach(
        (testParams: WindowsActivationTestParams) => {
            test(testParams.testName, async () => {
                ...
                condaSrv.setup(c => c.getCondaFile()).returns(async () => {
                    return path.join(testParams.basePath, "conda.exe");
                });
                servCnt
                    .setup(s =>
                        s.get(
                            TypeMoq.It.isValue(ICondaService),
                            TypeMoq.It.isAny()
                        )
                    )
                ...
                if (
                    testParams.terminalKind === TerminalShellType.commandPrompt
                ) {
                    result = await tstCmdProvider.getWindowsCommands(
                        testParams.envName
                    );
                } else {
                    result = await tstCmdProvider.getPowershellCommands(
                        testParams.envName,
                        testParams.terminalKind
                    );
                }
                expect(result).to.deep.equal(
                    testParams.expectedResult,
                    "Specific terminal command is incorrect."
                );
            });
        }
    );
