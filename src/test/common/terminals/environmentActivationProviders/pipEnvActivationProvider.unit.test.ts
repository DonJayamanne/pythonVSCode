    ...
    [undefined, Uri.parse("x")].forEach(resource => {
        suite(resource ? "With a resource" : "Without a resource", () => {
            ...
            setup(() => {
                ...
                pipenvService
                    .setup(p => p.executable)
                    .returns(() => pipenvExecFile);
            });

            test("No commands for no interpreter", async () => {
                when(
                    interpreterService.getActiveInterpreter(resource)
                ).thenResolve();

                for (const shell of getNamesAndValues<TerminalShellType>(
                    TerminalShellType
                )) {
                    const cmd = await activationProvider.getActivationCommands(
                        resource,
                        shell.value
                    );
            ...
            test("No commands for an interpreter that is not Pipenv", async () => {
                const nonPipInterpreterTypes = getNamesAndValues<
                    InterpreterType
                >(InterpreterType).filter(
                    t => t.value !== InterpreterType.Pipenv
                );
                for (const interpreterType of nonPipInterpreterTypes) {
                    when(
                        interpreterService.getActiveInterpreter(resource)
                    ).thenResolve({ type: interpreterType } as any);
            ...
            test("pipenv shell is returned for pipenv interpeter", async () => {
                when(
                    interpreterService.getActiveInterpreter(resource)
                ).thenResolve({ type: InterpreterType.Pipenv } as any);
