...
suite("Activation of Environments in Terminal", () => {
    ...
    suiteSetup(async () => {
        ...
        terminalSettings = vscode.workspace.getConfiguration(
            "terminal",
            vscode.workspace.workspaceFolders[0].uri
        );
        pythonSettings = vscode.workspace.getConfiguration(
            "python",
            vscode.workspace.workspaceFolders[0].uri
        );
        defaultShell.Windows = terminalSettings.inspect(
            "integrated.shell.windows"
        ).globalValue;
        defaultShell.Linux = terminalSettings.inspect(
    ...
    async function testActivation(envPath) {
        ...
        await waitForCondition(
            () => fs.pathExists(outputFile),
            5_000,
            "'testExecInTerminal.log' file not created"
        );
