...
suite("Terminal Environment Activation pyenv", () => {
    ...
    setup(() => {
        ...
        activationProvider = new PyEnvActivationCommandProvider(
            serviceContainer.object
        );
    });

    test("All shells should be supported", async () => {
        for (const item of getNamesAndValues<TerminalShellType>(
            TerminalShellType
        )) {
            expect(activationProvider.isShellSupported(item.value)).to.equal(
                true,
                "All shells should be supported"
            );
        }
    });
    ...
    test("Ensure no activation commands are returned if intrepreter info is not found", async () => {
        ...
        const activationCommands = await activationProvider.getActivationCommands(
            undefined,
