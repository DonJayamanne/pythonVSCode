...
suite("Terminal Activator", () => {
    ...
    async function testActivationAndHandlers(activationSuccessful: boolean) {
        baseActivator
            .setup(b =>
                b.activateEnvironmentInTerminal(
                    TypeMoq.It.isAny(),
                    TypeMoq.It.isAny(),
                    TypeMoq.It.isAny()
                )
    ...
    test("Terminal is activated and handlers are invoked", () =>
        testActivationAndHandlers(true));
    test("Terminal is not activated and handlers are invoked", () =>
        testActivationAndHandlers(false));
});
