        ...
        suite(suiteTitle, () => {
            [
                ...
                suite(`and script file is ${scriptFileName}`, () => {
                    ...
                    getNamesAndValues<TerminalShellType>(
                        TerminalShellType
                    ).forEach(shellType => {
                        const isScriptFileSupported =
                            ["activate.bat", "activate.ps1"].indexOf(
                                scriptFileName
                            ) >= 0;
                        const titleTitle = isScriptFileSupported
                            ? `Ensure terminal type is supported (Shell: ${
                                  shellType.name
                              })`
                            : `Ensure terminal type is not supported (Shell: ${
                                  shellType.name
                              })`;

                        test(titleTitle, async () => {
                            const bash = new CommandPromptAndPowerShell(
                                serviceContainer.object
                            );

                            const supported = bash.isShellSupported(
                                shellType.value
                            );
                            switch (shellType.value) {
                                ...
                                case TerminalShellType.powershell: {
                                    expect(supported).to.be.equal(
                                        ...
                                        `${
                                            shellType.name
                                        } shell not supported (it should be)`
                                    );
                                    break;
                                }
                                default: {
                                    expect(supported).to.be.equal(
                                        ...
                                        `${
                                            shellType.name
                                        } incorrectly supported (should not be)`
                                    );
                                }
                            }
                        });
            ...
            suite("and script file is activate.bat", () => {
                ...
                setup(() => {
                    ...
                    serviceContainer
                        ...
                        .returns(() => fileSystem.object);
                    serviceContainer
                        .setup(c => c.get(IPlatformService))
                ...
                test("Ensure batch files are supported by command prompt", async () => {
                    ...
                    const pathToScriptFile = path.join(
                        path.dirname(pythonPath),
                        "activate.bat"
                    );
                    fileSystem
                        .setup(fs =>
                            fs.fileExists(TypeMoq.It.isValue(pathToScriptFile))
                        )
                        .returns(() => Promise.resolve(true));
                    const commands = await bash.getActivationCommands(
                        resource,
                    ...
                    expect(commands).to.be.deep.equal(
                        [pathToScriptFile.fileToCommandArgument()],
                        "Invalid command"
                    );
                });

                test("Ensure batch files are not supported by powershell (on windows)", async () => {
                    const batch = new CommandPromptAndPowerShell(
                        serviceContainer.object
                    );
                    ...
                    fileSystem
                        ...
                        .returns(() => Promise.resolve(true));
                    const command = await batch.getActivationCommands(
                        resource,
                ...
                test("Ensure batch files are not supported by powershell core (on windows)", async () => {
                    ...
                    fileSystem
                        ...
                        .returns(() => Promise.resolve(true));
                    const command = await bash.getActivationCommands(
                        resource,
