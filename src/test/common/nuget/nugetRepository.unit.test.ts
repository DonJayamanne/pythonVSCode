...
suite("Nuget on Nuget Repo", () => {
    ...
    setup(() => {
        ...
        serviceContainer
            .setup(c => c.get(typeMoq.It.isValue(IHttpClient)))
    ...
    test("Get all package versions", async () => {
        ...
        const expectedUri = `${packageBaseAddress}/${packageName
            .toLowerCase()
            .trim()}/index.json`;
        ...
        const versions = await nugetRepo.getVersions(
            packageBaseAddress,
            packageName
        );
    ...
    test("Get package uri", async () => {
        ...
        const packageUri = nugetRepo.getNugetPackageUri(
            packageBaseAddress,
            packageName,
            new SemVer(version)
        );
    ...
    test("Get packages", async () => {
        ...
        nugetRepo.getVersions = () =>
            Promise.resolve(versions.map(v => new SemVer(v)));
        ...
        expect(packages.map(item => item.version.raw)).to.be.deep.equal(
            versions
        );
        expect(packages.map(item => item.uri)).to.be.deep.equal(
            versions.map(() => "uri")
        );
        expect(packages.map(item => item.package)).to.be.deep.equal(
            versions.map(() => "packageName")
        );
    });
});
