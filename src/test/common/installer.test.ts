...
suite("Installer", () => {
    ...
    function initializeDI() {
        ...
        ioc.serviceManager.addSingleton<IWorkspaceService>(
            IWorkspaceService,
            WorkspaceService
        );
    ...
    getNamesAndValues<Product>(Product).forEach(prod => {
        test(`Ensure isInstalled for Product: '${
            prod.name
        }' executes the right command`, async () => {
    ...
    async function testInstallingProduct(product: Product) {
        ...
        const moduleInstallers = ioc.serviceContainer.getAll<
            MockModuleInstaller
        >(IModuleInstaller);
        const moduleInstallerOne = moduleInstallers.find(
            item => item.displayName === "two"
        )!;

        moduleInstallerOne.on("installModule", moduleName => {
            const installName = installer.translateProductToModuleName(
                product,
    ...
    getNamesAndValues<Product>(Product).forEach(prod => {
        test(`Ensure install for Product: '${
            prod.name
        }' executes the right command in IModuleInstaller`, async () => {
            ioc.serviceManager.addSingletonInstance<IModuleInstaller>(
                IModuleInstaller,
                new MockModuleInstaller("one", false)
            );
