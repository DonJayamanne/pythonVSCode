...
suite("Multiroot Environment Variables Provider", () => {
    ...
    const pathVariableName = IS_WINDOWS
        ? WINDOWS_PATH_VARIABLE_NAME
        : NON_WINDOWS_PATH_VARIABLE_NAME;
    ...
    setup(() => {
        ...
        const mockEnvironmentActivationService = mock(
            EnvironmentActivationService
        );
        when(
            mockEnvironmentActivationService.getActivatedEnvironmentVariables(
                anything()
            )
        ).thenResolve();
    ...
    function getVariablesProvider(
        mockVariables: EnvironmentVariables = { ...process.env }
    ) {
        ...
        const disposables = ioc.serviceContainer.get<Disposable[]>(
            IDisposableRegistry
        );
    ...
    test("Custom variables should not be undefined without an env file", async () => {
        ...
        await expect(vars).to.eventually.not.equal(
            undefined,
            "Variables is not undefiend"
        );
    });
    ...
    test("Custom variables should be parsed from env file", async () => {
        ...
        const vars = await envProvider.getEnvironmentVariables(
            workspace4PyFile
        );

        expect(vars).to.not.equal(undefined, "Variables is is undefiend");
        expect(vars).to.have.property(
            "X1234PYEXTUNITTESTVAR",
            "1234",
            "X1234PYEXTUNITTESTVAR value is invalid"
        );
        expect(vars).to.have.property(
            "PYTHONPATH",
            "../workspace5",
            "PYTHONPATH value is invalid"
        );
    });
    ...
    test("PYTHONPATH from process variables should be merged with that in env file", async () => {
        ...
        const expectedPythonPath = `../workspace5${path.delimiter}${
            processVariables.PYTHONPATH
        }`;
        expect(vars).to.not.equal(undefined, "Variables is is undefiend");
        expect(vars).to.have.property(
            "X1234PYEXTUNITTESTVAR",
    ...
    test("PATH from process variables should be included in in variables returned (mock variables)", async () => {
        ...
        expect(vars).to.have.property(
            pathVariableName,
            processVariables[pathVariableName],
    ...
    test("PYTHONPATH and PATH from process variables should be merged with that in env file", async () => {
        ...
        const expectedPath = `/usr/x:/usr/y${path.delimiter}${
            processVariables[pathVariableName]
        }`;
        expect(vars).to.not.equal(undefined, "Variables is is undefiend");
    ...
    test("PYTHONPATH and PATH from process variables should be included in variables returned", async () => {
        ...
        expect(vars).to.not.equal(undefined, "Variables is is undefiend");
        expect(vars).to.have.property(
            "X12345PYEXTUNITTESTVAR",
            "12345",
            "X12345PYEXTUNITTESTVAR value is invalid"
        );
        expect(vars).to.have.property(
            "PYTHONPATH",
            processVariables.PYTHONPATH,
    ...
    test("Custom variables should not be merged with process environment varaibles", async () => {
        ...
        expect(vars).to.not.to.have.property(
            randomEnvVariable,
            undefined,
            "Yikes process variable has leaked"
        );
    });
    ...
    test("Custom variables will be refreshed when settings points to a different env file", async () => {
        ...
        const newVars = await envProvider.getEnvironmentVariables(
            workspace4PyFile
        );
        expect(newVars).to.not.equal(undefined, "Variables is is undefiend");
        expect(newVars).to.have.property(
            "X12345PYEXTUNITTESTVAR",
            "12345",
            "X12345PYEXTUNITTESTVAR value is invalid"
        );
        expect(newVars).to.not.to.have.property(
            "PYTHONPATH",
            "../workspace5",
            "PYTHONPATH value is invalid"
        );
    ...
    test("Custom variables will be refreshed when .env file is created, modified and deleted", async function() {
        ...
        await expect(vars).to.eventually.not.equal(
            undefined,
            "Variables is is undefiend"
        );

        // Create env3.
        const contents = fs.readFileSync(
            path.join(workspace4Path.fsPath, ".env2")
        );
        ...
        expect(newVars).to.not.equal(
            undefined,
            "Variables is is undefiend after creating"
        );
        expect(newVars).to.have.property(
            "X12345PYEXTUNITTESTVAR",
            "12345",
            "X12345PYEXTUNITTESTVAR value is invalid after creating"
        );
        expect(newVars).to.not.to.have.property(
            "PYTHONPATH",
            "../workspace5",
            "PYTHONPATH value is invalid after creating"
        );

        // Modify env3.
        fs.writeFileSync(
            env3,
            `${contents}${EOL}X123456PYEXTUNITTESTVAR=123456`
        );

        // Wait for settings to get refreshed.
        await new Promise(resolve => setTimeout(resolve, 5000));

        const updatedVars = await envProvider.getEnvironmentVariables(
            workspace4PyFile
        );
        expect(updatedVars).to.not.equal(
            undefined,
            "Variables is is undefiend after modifying"
        );
        expect(updatedVars).to.have.property(
            "X12345PYEXTUNITTESTVAR",
            "12345",
            "X12345PYEXTUNITTESTVAR value is invalid after modifying"
        );
        expect(updatedVars).to.not.to.have.property(
            "PYTHONPATH",
            "../workspace5",
            "PYTHONPATH value is invalid after modifying"
        );
        expect(updatedVars).to.have.property(
            "X123456PYEXTUNITTESTVAR",
            "123456",
            "X123456PYEXTUNITTESTVAR value is invalid after modifying"
        );
        ...
        const varsAfterDeleting = await envProvider.getEnvironmentVariables(
            workspace4PyFile
        );
        expect(varsAfterDeleting).to.not.equal(
            undefined,
            "Variables is undefiend after deleting"
        );
    });
    ...
    test("Change event will be raised when when .env file is created, modified and deleted", async function() {
        ...
        envProvider.onDidEnvironmentVariablesChange(() =>
            eventRaisedPromise.resolve(true)
        );
        const vars = envProvider.getEnvironmentVariables(workspace4PyFile);
        await expect(vars).to.eventually.not.equal(
            undefined,
