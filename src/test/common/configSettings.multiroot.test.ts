...
suite("Multiroot Config Settings", () => {
    suiteSetup(async function() {
        ...
        await clearPythonPathInWorkspaceFolder(
            Uri.file(path.join(multirootPath, "workspace1"))
        );
        await initialize();
    });
    ...
    async function enableDisableLinterSetting(
        resource: Uri,
        ...
        if (
            ...
            cfgValue &&
            cfgValue.workspaceValue === enabled
        ) {
            return;
        }
        if (
            ...
            cfgValue &&
            cfgValue.workspaceFolderValue === enabled
        ) {
            return;
        }
    ...
    test("Workspace folder should inherit Python Path from workspace root", async () => {
        ...
        assert.equal(
            cfgSetting.pythonPath,
            pythonPath,
            "Python Path not inherited from workspace"
        );
    });
    ...
    test("Workspace folder should not inherit Python Path from workspace root", async () => {
        ...
        assert.equal(
            cfgSetting.pythonPath,
            privatePythonPath,
            "Python Path for workspace folder is incorrect"
        );
    });
    ...
    test("Enabling/Disabling Pylint in root should be reflected in config settings", async () => {
        const workspaceUri = Uri.file(path.join(multirootPath, "workspace1"));
        await enableDisableLinterSetting(
            workspaceUri,
            ConfigurationTarget.WorkspaceFolder,
            "pylintEnabled",
            undefined
        ...
        assert.equal(
            settings.linting.pylintEnabled,
            true,
            "Pylint not enabled when it should be"
        );
        ...
        assert.equal(
            settings.linting.pylintEnabled,
            false,
            "Pylint enabled when it should not be"
        );
    });
    ...
    test("Enabling/Disabling Pylint in root and workspace should be reflected in config settings", async () => {
        ...
        assert.equal(
            cfgSetting.linting.pylintEnabled,
            false,
            "Workspace folder pylint setting is true when it should not be"
        );
        PythonSettings.dispose();
        ...
        assert.equal(
            cfgSetting.linting.pylintEnabled,
            true,
            "Workspace folder pylint setting is false when it should not be"
        );
    });
    ...
    test("Enabling/Disabling Pylint in root should be reflected in config settings when opening a document", async () => {
        ...
        assert.equal(
            cfg.linting.pylintEnabled,
            true,
            "Pylint should be enabled in workspace"
        );
        PythonSettings.dispose();
        ...
        assert.equal(
            cfg.linting.pylintEnabled,
            false,
            "Pylint should not be enabled in workspace"
        );
    });
    ...
    test("${workspaceFolder} variable in settings should be replaced with the right value", async () => {
        ...
        let cfg = getExtensionSettings(document.uri);
        assert.equal(
            path.dirname(cfg.workspaceSymbols.tagFilePath),
            workspace2Uri.fsPath,
            "ctags file path for workspace2 is incorrect"
        );
        assert.equal(
            path.basename(cfg.workspaceSymbols.tagFilePath),
            "workspace2.tags.file",
            "ctags file name for workspace2 is incorrect"
        );
        PythonSettings.dispose();
        ...
        assert.equal(
            path.dirname(cfg.workspaceSymbols.tagFilePath),
            workspace3Uri.fsPath,
            "ctags file path for workspace3 is incorrect"
        );
        assert.equal(
            path.basename(cfg.workspaceSymbols.tagFilePath),
            "workspace3.tags.file",
            "ctags file name for workspace3 is incorrect"
        );
        PythonSettings.dispose();
    });
});
