...
suite("ProcessService Observable", () => {
    ...
    test("exec should output print statements", async () => {
        ...
        const result = await procService.exec(pythonPath, [
            "-c",
            `print("${printOutput}")`
        ]);
    ...
    test("exec should stream stdout and stderr separately", async function() {
        ...
        const stdouts = result.stdout
            ...
            .map(line => line.trim())
            .filter(line => line.length > 0);
        expect(stdouts).to.deep.equal(
            expectedStdout,
            "stdout values are incorrect"
        );
        const stderrs = result
            .stderr!.split(/\r?\n/g)
            .map(line => line.trim())
            .filter(line => line.length > 0);
        expect(stderrs).to.deep.equal(
            expectedStderr,
            "stderr values are incorrect"
        );
    });
    ...
    test("exec should throw an error when spawn file not found", async () => {
        ...
        await expect(result).to.eventually.be.rejected.and.to.have.property(
            "code",
            "ENOENT",
            "Invalid error code"
        );
    ...
    test("shellExec should be able to run python too", async () => {
        ...
        const result = await procService.shellExec(
            `"${pythonPath}" -c "print('${printOutput}')"`
        );
