        ...
        suite(resource ? "With a resource" : "Without a resource", () => {
            ...
            setup(() => {
                ...
                factory = new PythonExecutionFactory(
                    instance(mock(ServiceContainer)),
            ...
            test("Ensure PythonExecutionService is created", async () => {
                ...
                when(procecssFactory.create(resource)).thenResolve(
                    instance(mock(ProcessService))
                );
                when(
                    activationHelper.getActivatedEnvironmentVariables(resource)
                ).thenResolve({ x: "1" });
                when(pythonSettings.pythonPath).thenReturn("HELLO");
                when(configService.getSettings(resource)).thenReturn(
            ...
            test("Ensure we use an existing `create` method if there are no environment variables for the activated env", async () => {
                ...
                factory.create = async (
                    _options: ExecutionFactoryCreationOptions
                ) => {
                    createInvoked = true;
                    return Promise.resolve(
                        (mockExecService as any) as IPythonExecutionService
                    );
                };
                ...
                const service = await factory.createActivatedEnvironment(
                    resource
                );
