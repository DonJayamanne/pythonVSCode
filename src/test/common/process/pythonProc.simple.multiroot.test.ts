...
suite("PythonExecutableService", () => {
    ...
    setup(async () => {
        ...
        configService = serviceManager.get<IConfigurationService>(
            IConfigurationService
        );
    ...
    test("Importing without a valid PYTHONPATH should fail", async () => {
        ...
        const pythonExecService = await pythonExecFactory.create({
    ...
    test("Importing with a valid PYTHONPATH from .env file should succeed", async function() {
        ...
        await expect(promise).to.eventually.have.property(
            "stdout",
            `Hello${EOL}`
        );
    });
    ...
    test("Known modules such as 'os' and 'sys' should be deemed 'installed'", async () => {
        ...
        await expect(osModuleIsInstalled).to.eventually.equal(
            true,
            "os module is not installed"
        );
        await expect(sysModuleIsInstalled).to.eventually.equal(
            true,
            "sys module is not installed"
        );
    });
    ...
    test("Unknown modules such as 'xyzabc123' be deemed 'not installed'", async () => {
        ...
        const randomModuleIsInstalled = pythonExecService.isModuleInstalled(
            randomModuleName
        );
    ...
    test("Ensure correct path to executable is returned", async () => {
        ...
        expect(executablePath).to.equal(
            expectedExecutablePath,
            "Executable paths are not the same"
        );
    });
});
