...
suite("ProcessService", () => {
    ...
    test("execObservable should stream output with new lines", function(done) {
        ...
        const result = procService.execObservable(pythonPath, [
            "-c",
            pythonCode.join(";")
        ]);
        const outputs = ["1", "2", "3"];
        ...
        result.out.subscribe(
            output => {
                ...
                if (
                    expectedValue !== output.out.trim() &&
                    expectedValue === output.out
                ) {
                    done(
                        `Received value ${
                            output.out
                        } is not same as the expectd value ${expectedValue}`
                    );
                }
                if (output.source !== "stdout") {
                    done(
                        `Source is not stdout. Value received is ${
                            output.source
                        }`
                    );
                }
    ...
    test("execObservable should end when cancellationToken is cancelled", function(done) {
        ...
        const result = procService.execObservable(
            pythonPath,
            ["-c", pythonCode.join(";")],
    ...
    test("execObservable should end when process is killed", function(done) {
        ...
        result.out.subscribe(
            ...
            () => {
                const errorMsg = procKilled
                    ? undefined
                    : "Program terminated even before killing it.";
                done(errorMsg);
            }
        );
    });
    ...
    test("execObservable should stream stdout and stderr separately", function(done) {
        ...
        result.out.subscribe(
            output => {
                ...
                expect(output.source).to.be.equal(
                    expectedOutput.source,
                    "Expected sopurce is incorrect"
                );
            },
            done,
            done
    ...
    test("execObservable should throw an error with stderr output", done => {
        ...
        result.out.subscribe(
            output => {
                    ...
                    "Output received, when we're expecting an error to be thrown."
                );
            },
            (ex: Error) => {
    ...
    test("execObservable should exit without no output", done => {
        ...
        result.out.subscribe(
            output => {
                    ...
                    `Output received, when we\'re not expecting any, ${JSON.stringify(
                        output
                    )}`
                );
            },
