...
suite("PythonExecutableService", () => {
    ...
    setup(() => {
        ...
        serviceContainer
            .setup(c =>
                c.get(TypeMoq.It.isValue(IEnvironmentVariablesProvider))
            )
        ...
        envVarsProvider
            .setup(v => v.getEnvironmentVariables(TypeMoq.It.isAny()))
            .returns(() => Promise.resolve({}));

        const envActivationService = TypeMoq.Mock.ofType<
        ...
        serviceContainer
            .setup(s =>
                s.get(
                    TypeMoq.It.isValue(IEnvironmentActivationService),
                    TypeMoq.It.isAny()
                )
            )
    ...
    test("Ensure resource is used when getting configuration service settings (defined resource)", async () => {
        ...
        pythonSettings.setup(p => p.pythonPath).returns(() => pythonPath);
        configService
            .setup(c => c.getSettings(TypeMoq.It.isValue(resource)))
            .returns(() => pythonSettings.object);
        procService
            .setup(p =>
                p.exec(
