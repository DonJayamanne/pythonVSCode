...
suite("Module Installer", () => {
    [undefined, Uri.file(__filename)].forEach(resource => {
        ...
        function initializeDI() {
            ...
            ioc.serviceManager.addSingleton<IPersistentStateFactory>(
                IPersistentStateFactory,
                PersistentStateFactory
            );
            ioc.serviceManager.addSingleton<ILogger>(ILogger, Logger);
            ioc.serviceManager.addSingleton<IInstaller>(
                IInstaller,
                ProductInstaller
            );
            ...
            mockTerminalFactory
                ...
                .callback(passedInResource =>
                    expect(passedInResource).to.be.equal(resource)
                )
                .returns(() => mockTerminalService.object);
            ...
            ioc.serviceManager.addSingleton<IModuleInstaller>(
                IModuleInstaller,
                PipInstaller
            );
            ioc.serviceManager.addSingleton<IModuleInstaller>(
            ...
            ioc.serviceManager.addSingleton<ICurrentProcess>(
                ICurrentProcess,
                CurrentProcess
            );
            ioc.serviceManager.addSingleton<IFileSystem>(
                IFileSystem,
                FileSystem
            );
            ioc.serviceManager.addSingleton<IPlatformService>(
                IPlatformService,
                PlatformService
            );
            ioc.serviceManager.addSingleton<IConfigurationService>(
                IConfigurationService,
                ConfigurationService
            );

            const workspaceService = TypeMoq.Mock.ofType<IWorkspaceService>();
            ioc.serviceManager.addSingletonInstance<IWorkspaceService>(
                IWorkspaceService,
                workspaceService.object
            );
            const http = TypeMoq.Mock.ofType<WorkspaceConfiguration>();
            http.setup(h =>
                h.get(TypeMoq.It.isValue("proxy"), TypeMoq.It.isAny())
            ).returns(() => "");
            workspaceService
        ...
        async function resetSettings(): Promise<void> {
            const configService = ioc.serviceManager.get<IConfigurationService>(
                IConfigurationService
            );
        ...
        test("Ensure pip is supported and conda is not", async () => {
            ...
            ioc.serviceManager.addSingletonInstance<ITerminalHelper>(
                ITerminalHelper,
            ...
            const moduleInstallers = ioc.serviceContainer.getAll<
                IModuleInstaller
            >(IModuleInstaller);
            expect(moduleInstallers).length(
                4,
                "Incorrect number of installers"
            );

            const pipInstaller = moduleInstallers.find(
                item => item.displayName === "Pip"
            )!;
            expect(pipInstaller).not.to.be.an(
                "undefined",
                "Pip installer not found"
            );
            await expect(pipInstaller.isSupported()).to.eventually.equal(
                true,
                "Pip is not supported"
            );

            const condaInstaller = moduleInstallers.find(
                item => item.displayName === "Conda"
            )!;
            expect(condaInstaller).not.to.be.an(
                "undefined",
                "Conda installer not found"
            );
            await expect(condaInstaller.isSupported()).to.eventually.equal(
                false,
                "Conda is supported"
            );

            const mockInstaller = moduleInstallers.find(
                item => item.displayName === "mock"
            )!;
            expect(mockInstaller).not.to.be.an(
                "undefined",
                "mock installer not found"
            );
            await expect(mockInstaller.isSupported()).to.eventually.equal(
                true,
                "mock is not supported"
            );
        });
        ...
        test("Ensure conda is supported", async () => {
            ...
            serviceContainer
                .setup(c => c.get(TypeMoq.It.isValue(ICondaService)))
            ...
            condaService
                .setup(c =>
                    c.isCondaEnvironment(TypeMoq.It.isValue(pythonPath))
                )
                .returns(() => Promise.resolve(true));
        ...
        test("Ensure conda is not supported even if conda is available", async () => {
            ...
            await expect(condaInstaller.isSupported()).to.eventually.equal(
                false,
                "Conda should not be supported"
            );
        });

        const resourceTestNameSuffix = resource
            ? " with a resource"
            : " without a resource";
        test(`Validate pip install arguments ${resourceTestNameSuffix}`, async () => {
            ...
            mockTerminalService
                .setup(t =>
                    t.sendCommand(TypeMoq.It.isAnyString(), TypeMoq.It.isAny())
                )
            ...
            interpreterService
                ...
                .returns(() =>
                    Promise.resolve({ type: InterpreterType.Unknown } as any)
                );
        ...
        test(`Validate pipenv install arguments ${resourceTestNameSuffix}`, async () => {
            ...
            >(IModuleInstaller);
            const pipInstaller = moduleInstallers.find(
                item => item.displayName === "pipenv"
            )!;

            expect(pipInstaller).not.to.be.an(
                "undefined",
                "pipenv installer not found"
            );
