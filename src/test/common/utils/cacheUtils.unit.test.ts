...
} from "../../../client/common/utils/cacheUtils";
...
suite("Common Utils - CacheUtils", () => {
    ...
    ["hello", undefined, { date: new Date(), hello: 1234 }].forEach(
        dataToStore => {
            ...
            test("Data is stored in cache (with workspaces)", () => {
                ...
                vsc.workspace.workspaceFolders = [
                    { index: 0, name: "1", uri: Uri.parse("wkfolder") }
                ];
                vsc.workspace.getWorkspaceFolder = () =>
            ...
            test("Data is stored in cache and different resources do not point to same storage location (with multiple workspaces)", () => {
                ...
                expect(cache.data).to.be.deep.equal(dataToStore);
                expect(cache2.data).to.be.deep.equal(
                    undefined,
                    "Must not have any data"
                );
