...
suite("Feature Deprecation Manager Tests", () => {
    test("Ensure deprecated command Build_Workspace_Symbols registers its popup", () => {
        const persistentState: TypeMoq.IMock<
            IPersistentStateFactory
        > = TypeMoq.Mock.ofType<IPersistentStateFactory>();
        const persistentBool: TypeMoq.IMock<
            IPersistentState<boolean>
        > = TypeMoq.Mock.ofType<IPersistentState<boolean>>();
        ...
        persistentState
            .setup(a =>
                a.createGlobalPersistentState(
                    TypeMoq.It.isValue(
                        "SHOW_DEPRECATED_FEATURE_PROMPT_BUILD_WORKSPACE_SYMBOLS"
                    ),
        ...
        const popupMgr: TypeMoq.IMock<IApplicationShell> = TypeMoq.Mock.ofType<
            IApplicationShell
        >();
        popupMgr
            .setup(p =>
        ...
        const cmdDisposable: TypeMoq.IMock<Disposable> = TypeMoq.Mock.ofType<
            Disposable
        >();
        const cmdManager: TypeMoq.IMock<ICommandManager> = TypeMoq.Mock.ofType<
            ICommandManager
        >();
        cmdManager
            .setup(c =>
    ...
    test("Ensure setting is checked", () => {
        ...
        const deprecatedSetting: DeprecatedSettingAndValue = {
        ...
        let isUsed = featureDepMgr.isDeprecatedSettingAndValueUsed(
            pythonConfig.object,
            deprecatedSetting
        );
        ...
        for (const config of testConfigs) {
            ...
            pythonConfig
                .setup(p =>
                    p.has(TypeMoq.It.isValue(deprecatedSetting.setting))
                )
                .returns(() => true)
                .verifiable(TypeMoq.Times.atLeastOnce());
            pythonConfig
                .setup(p =>
                    p.get(TypeMoq.It.isValue(deprecatedSetting.setting))
                )
                .returns(() => config.valueInSetting);

            isUsed = featureDepMgr.isDeprecatedSettingAndValueUsed(
                pythonConfig.object,
                deprecatedSetting
            );

            pythonConfig.verifyAll();
            expect(isUsed).to.be.equal(
                config.expectedValue,
                `Failed for config = ${JSON.stringify(config)}`
            );
        }
