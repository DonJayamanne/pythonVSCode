...
suite("Workspace Symbols Provider", () => {
    ...
    test("Returns 0 tags without any generators", async () => {
        ...
        const tags = await provider.provideWorkspaceSymbols(
            "",
    ...
    test("symbols should be filtered correctly", async () => {
        ...
        assert.equal(
            symbols.length >= 2,
            true,
            "Incorrect number of symbols returned"
        );
        assert.notEqual(
            symbols.findIndex(sym =>
                sym.location.uri.fsPath.endsWith("childFile.py")
            ),
            -1,
        ...
        assert.notEqual(
            symbols.findIndex(sym =>
                sym.location.uri.fsPath.endsWith("workspace2File.py")
            ),
            -1,
            "File with symbol not found in child workspace folder"
        );

        const symbolsForMeth = await provider.provideWorkspaceSymbols(
            "meth",
            new CancellationTokenSource().token
        );
        assert.equal(
            symbolsForMeth.length >= 10,
            true,
            "Incorrect number of symbols returned"
        );
        assert.notEqual(
            symbolsForMeth.findIndex(sym =>
                sym.location.uri.fsPath.endsWith("childFile.py")
            ),
            -1,
        ...
        assert.notEqual(
            symbolsForMeth.findIndex(sym =>
                sym.location.uri.fsPath.endsWith("file.py")
            ),
            -1,
