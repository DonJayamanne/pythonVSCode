...
import {
    ...
    formatStreamText
} from "../../client/datascience/common";
...
export class Cell extends React.Component<ICellProps> {
        ...
        if (this.props.cellVM.cell.data.cell_type === "sys_info") {
            return (
                <SysInfo
                    ...
                    connection={this.props.cellVM.cell.data.connection}
                    path={this.props.cellVM.cell.data.path}
                    message={this.props.cellVM.cell.data.message}
                    version={this.props.cellVM.cell.data.version}
                    notebook_version={
                        this.props.cellVM.cell.data.notebook_version
                    }
                />
            );
        } else {
    ...
    private renderNormalCell() {
        const busy =
            this.props.cellVM.cell.state === CellState.init ||
            this.props.cellVM.cell.state === CellState.executing;
        const hasNoSource =
            this.props.cellVM.cell.file === Identifiers.EmptyFileName;
        return (
                ...
                <div className="cell-outer">
                    <div className="controls-div">
                        <div className="controls-flex">
                            <ExecutionCount
                                isBusy={busy}
                                count={this.props.cellVM.cell.data.execution_count.toString()}
    ...
    private renderCodeOutputs = () => {
        if (this.isCodeCell() && this.hasOutput()) {
            // Render the outputs
            return this.getCodeCell().outputs.map(
                (output: nbformat.IOutput, index: number) => {
    ...
    private renderOutput = (output: nbformat.IOutput, index: number) => {
        ...
        let mimetype: string = richestMimetype(
            copy.data,
            displayOrder,
            transforms
        );
        ...
        const str: string = this.getUnknownMimeTypeFormatString().format(
            mimetype
        );
