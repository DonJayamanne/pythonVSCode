...
export class Input extends React.Component<IInputProps> {
    ...
    private expectedIndent(instance: CodeMirror.Editor, line: number): number {
        ...
        const lastChar =
            lineStr.length === 0 ? null : lineStr.charAt(lineStr.length - 1);
    ...
    private enter = (instance: CodeMirror.Editor) => {
        ...
        if (cursor.line === lastLine) {
            ...
            const lastChar =
                lastLineStr.length === 0
                    ? null
                    : lastLineStr.charAt(lastLineStr.length - 1);
    ...
    private arrowDown = (instance: CodeMirror.Editor) => {
        if (
            instance.getDoc().getCursor().line === 0 &&
            instance.getDoc().lineCount() <= 1
        ) {
