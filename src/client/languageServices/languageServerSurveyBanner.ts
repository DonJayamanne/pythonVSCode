...
export class LanguageServerSurveyBanner implements IPythonExtensionBanner {
    ...
    constructor(
        ...
        private lsService: ILanguageServerFolderService,
    ...
    public async launchSurvey(): Promise<void> {
        ...
        this.browserService.launch(
            `https://www.research.net/r/LJZV9BZ?n=${launchCounter}&v=${lsVersion}`
        );
    }

    private async incrementPythonLanguageServiceLaunchCounter(): Promise<
        number
    > {
        const state = this.persistentState.createGlobalPersistentState<number>(
            LSSurveyStateKeys.ShowAttemptCounter,
            0
        );
    ...
    private async getPythonLSVersion(
        fallback: string = "unknown"
    ): Promise<string> {
        const langServiceLatestFolder:
            | FolderVersionPair
            | undefined = await this.lsService.getCurrentLanguageServerDirectory();
        return langServiceLatestFolder
            ? langServiceLatestFolder.version.raw
            : fallback;
    }
    ...
    private async getPythonLSLaunchThresholdCounter(): Promise<number> {
        const state = this.persistentState.createGlobalPersistentState<
            number | undefined
        >(LSSurveyStateKeys.ShowAfterCompletionCount, undefined);
        if (state.value === undefined) {
            await state.updateValue(
                getRandomBetween(
                    this.minCompletionsBeforeShow,
                    this.maxCompletionsBeforeShow
                )
            );
        }
        return state.value!;
