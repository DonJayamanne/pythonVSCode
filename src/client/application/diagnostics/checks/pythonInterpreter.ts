...
export const InvalidPythonInterpreterServiceId =
...
export class InvalidPythonInterpreterService extends BaseDiagnosticsService {
    ...
    public async diagnose(): Promise<IDiagnostic[]> {
        ...
        const interpreterService = this.serviceContainer.get<
            IInterpreterService
        >(IInterpreterService);
        ...
        if (
            interpreters.filter(i => !helper.isMacDefaultPythonPath(i.path))
                .length === 0
        ) {
            return [
                new InvalidPythonInterpreterDiagnostic(
    ...
    protected async onDidChangeConfiguration(event: ConfigurationChangeEvent) {
        ...
        const workspacesUris: (
            | Uri
            | undefined)[] = workspaceService.hasWorkspaceFolders
        ...
        this.timeOut = setTimeout(() => {
            this.timeOut = undefined;
            this.diagnose()
                .then(dianostics => this.handle(dianostics))
                .ignoreErrors();
