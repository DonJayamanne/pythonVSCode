...
export const InvalidDebuggerTypeDiagnosticsServiceId =
    "InvalidDebuggerTypeDiagnosticsServiceId";
...
export class InvalidDebuggerTypeDiagnosticsService extends BaseDiagnosticsService {
    ...
    constructor(
        ...
        const cmdManager = serviceContainer.get<ICommandManager>(
            ICommandManager
        );
    ...
    public async diagnose(): Promise<IDiagnostic[]> {
        if (await this.isExperimentalDebuggerUsed()) {
            return [
                new InvalidDebuggerTypeDiagnostic(InvalidDebuggerTypeMessage)
            ];
        } else {
            return [];
        }
    ...
    private async isExperimentalDebuggerUsedInWorkspace(
    ...
    private async fixLaunchJson() {
        ...
        await Promise.all(
            workspaceService.workspaceFolders!.map(workspaceFolder =>
                this.fixLaunchJsonInWorkspace(workspaceFolder)
            )
        );
    }
    private async fixLaunchJsonInWorkspace(workspaceFolder: WorkspaceFolder) {
        if (
            !(await this.isExperimentalDebuggerUsedInWorkspace(workspaceFolder))
        ) {
            return;
        }
