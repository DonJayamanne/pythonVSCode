...
export class TestFileCodeLensProvider implements CodeLensProvider {
    ...
    public async provideCodeLenses(
        ...
        if (
            !testItems ||
            testItems.testFiles.length === 0 ||
            testItems.testFunctions.length === 0
        ) {
            return [];
        }
    ...
    private async getCodeLenses(
        ...
        const file = tests.testFiles.find(
            item => item.fullPath === document.uri.fsPath
        );
        if (!file) {
            return [];
        }
    ...
    private getCodeLens(
        ...
        switch (symbolKind) {
            ...
            case SymbolKind.Class: {
                const cls = allFuncsAndSuites.suites.find(
                ...
                return [
                    new CodeLens(range, {
                        title:
                            getTestStatusIcon(cls.status) +
                            constants.Text.CodeLensRunUnitTest,
                        command: constants.Commands.Tests_Run,
                        arguments: [
                            undefined,
                            CommandSource.codelens,
                            file,
                    ...
                    new CodeLens(range, {
                        title:
                            getTestStatusIcon(cls.status) +
                            constants.Text.CodeLensDebugUnitTest,
...
function getTestStatusIcons(fns: TestFunction[]): string {
    ...
    count = fns.filter(
        fn => fn.status === TestStatus.Error || fn.status === TestStatus.Fail
    ).length;
    if (count > 0) {
        statuses.push(`✘ ${count}`);
    }
...
function getFunctionCodeLens(
    ...
    if (symbolContainer.length === 0) {
        fn = functionsAndSuites.functions.find(
            func => func.name === symbolName
        );
    } else {
    ...
    const functions = functionsAndSuites.functions.filter(
        func =>
            func.name.startsWith(`${symbolName}[`) && func.name.endsWith("]")
    );
        ...
        return [
            new CodeLens(range, {
                ...
                arguments: [
                    ...
                    CommandSource.codelens,
                    file,
                    <TestsToRun>{ testFunction: functions }
                ]
            }),
            new CodeLens(range, {
    ...
    return [
        new CodeLens(range, {
            title: `${getTestStatusIcons(functions)}${
                constants.Text.CodeLensRunUnitTest
            } (Multiple)`,
...
function getAllTestSuitesAndFunctionsPerFile(
    testFile: TestFile
): FunctionsAndSuites {
...
function getAllTestSuitesAndFunctions(
    testSuite: TestSuite
): FunctionsAndSuites {
    const all: { functions: TestFunction[]; suites: TestSuite[] } = {
        functions: [],
        suites: []
    };
