...
@injectable()
export class UnitTestConfigurationService
    implements IUnitTestConfigurationService {
    ...
    public async displayTestFrameworkError(wkspace: Uri): Promise<void> {
        ...
        if (enabledCount > 1) {
            return this.promptToEnableAndConfigureTestFramework(
                wkspace,
                this.installer,
                this.outputChannel,
    ...
    public async selectTestRunner(
        ...
        return selectedTestRunner
            ? (selectedTestRunner.product as UnitTestProduct)
            : undefined;
    }
    public enableTest(wkspace: Uri, product: UnitTestProduct) {
        const factory = this.serviceContainer.get<
            ITestConfigurationManagerFactory
        >(ITestConfigurationManagerFactory);
        const configMgr = factory.create(wkspace, product);
        const pythonConfig = this.workspaceService.getConfiguration(
            "python",
            wkspace
        );
        ...
        return pythonConfig
            .update("unitTest.promptToConfigure", undefined)
            .then(
                () => {
                    return configMgr.enable();
                },
    ...
    private async promptToEnableAndConfigureTestFramework(
        wkspace: Uri,
            ...
            [Product.unittest, Product.pytest, Product.nosetest]
                ...
                .forEach(prod => {
                    factory
                        ...
                        .catch(ex =>
                            console.error(
                                "Python Extension: createTestConfigurationManager.disable",
                                ex
                            )
                        );
                });
        ...
        return configMgr
            ...
            .catch(reason => {
                return this.enableTest(wkspace, selectedTestRunner).then(() =>
