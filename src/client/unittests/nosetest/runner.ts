...
export class TestManagerRunner implements ITestManagerRunner {
    ...
    constructor(
        ...
        this.argsService = serviceContainer.get<IArgumentsService>(
            IArgumentsService,
            NOSETEST_PROVIDER
        );
        ...
        this.xUnitParser = this.serviceContainer.get<IXUnitParser>(
            IXUnitParser
        );
    ...
    public async runTest(
        ...
        if (options.testsToRun && options.testsToRun.testFolder) {
            testPaths = testPaths.concat(
                options.testsToRun.testFolder.map(f => f.nameToRun)
            );
        }
        if (options.testsToRun && options.testsToRun.testFile) {
            testPaths = testPaths.concat(
                options.testsToRun.testFile.map(f => f.nameToRun)
            );
        }
        ...
        if (options.testsToRun && options.testsToRun.testFunction) {
            testPaths = testPaths.concat(
                options.testsToRun.testFunction.map(f => f.nameToRun)
            );
        }
        ...
        try {
            ...
            const testArgs = this.argsService.filterArguments(args, [
                XUNIT_FILE
            ]);
            ...
            return options.debug
                ? options.tests
                : await this.updateResultsFromLogFiles(
                      options.tests,
                      xmlLogFile,
                      testResultsService
                  );
    ...
    private async updateResultsFromLogFiles(
        ...
        await this.xUnitParser.updateResultsFromXmlLogFile(
            tests,
            outputXmlFile,
            PassCalculationFormulae.nosetests
        );
