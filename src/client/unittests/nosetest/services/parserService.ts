...
export class TestsParser implements ITestsParser {
    ...
    public parse(content: string, options: ParserOptions): Tests {
        ...
        testFiles = testFiles.filter(
            testFile =>
                testFile.suites.length > 0 || testFile.functions.length > 0
        );
    ...
    private getTestFiles(content: string, options: ParserOptions) {
        ...
        content.split(/\r?\n/g).forEach((line, index, lines) => {
            if (
                (line.startsWith(NOSE_WANT_FILE_PREFIX) &&
                    line.endsWith(NOSE_WANT_FILE_SUFFIX)) ||
                index === lines.length - 1
            ) {
                // process the previous lines.
                this.parseNoseTestModuleCollectionResult(
                    options.cwd,
                    logOutputLines,
                    testFiles
                );
    ...
    private parseNoseTestModuleCollectionResult(
        ...
        lines.forEach(line => {
            if (
                ...
                fileName = fileName.substring(
                    0,
                    fileName.lastIndexOf(NOSE_WANT_FILE_SUFFIX_WITHOUT_EXT)
                );
                ...
                if (
                    path.extname(fileName) === ".pyc" ||
                    path.extname(fileName) === ".pyo"
                ) {
            ...
            if (line.startsWith("nose.selector: DEBUG: wantClass <class '")) {
                const name = extractBetweenDelimiters(
                    line,
                    "nose.selector: DEBUG: wantClass <class '",
                    "'>? True"
                );
            ...
            if (
                ...
                const cls = testFile.suites.find(
                    suite => suite.name === clsName
                );
                if (cls) {
            ...
            if (
                line.startsWith("nose.selector: DEBUG: wantFunction <function ")
            ) {
                const name = extractBetweenDelimiters(
                    line,
