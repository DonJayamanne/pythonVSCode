...
export class ArgumentsService implements IArgumentsService {
    ...
    public filterArguments(
                ...
                case TestFilter.discovery: {
                    optionsWithoutArgsToRemove.push(
                        ...[
                            ...
                            "--stop",
                            "--with-coverage",
                            ...OptionsWithoutArguments.filter(item =>
                                item.startsWith("--cover")
                            ),
                            ...OptionsWithoutArguments.filter(item =>
                                item.startsWith("--logging")
                            ),
                            ...OptionsWithoutArguments.filter(item =>
                                item.startsWith("--pdb")
                            ),
                            ...OptionsWithoutArguments.filter(
                                item => item.indexOf("xunit") >= 0
                            )
                        ]
                    );
                    optionsWithArgsToRemove.push(
                        ...[
                            ...
                            ...OptionsWithArguments.filter(item =>
                                item.startsWith("--logging")
                            ),
                            ...OptionsWithoutArguments.filter(
                                item => item.indexOf("xunit") >= 0
        ...
        return this.helper.filterArguments(
            filteredArgs,
            optionsWithArgsToRemove,
            optionsWithoutArgsToRemove
        );
    }
    public getTestFolders(args: string[]): string[] {
        return this.helper.getPositionalArguments(
            args,
            OptionsWithArguments,
            OptionsWithoutArguments
        );
