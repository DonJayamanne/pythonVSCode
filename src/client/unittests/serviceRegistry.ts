...
export function registerTypes(serviceManager: IServiceManager) {
    ...
    serviceManager.add<ITestResultsService>(
        ITestResultsService,
        TestResultsService
    );

    serviceManager.add<ITestVisitor>(
        ITestVisitor,
    ...
    serviceManager.add<ITestsParser>(
        ITestsParser,
        UnitTestTestsParser,
        UNITTEST_PROVIDER
    );
    ...
    serviceManager.add<ITestDiscoveryService>(
        ITestDiscoveryService,
    ...
    serviceManager.add<IArgumentsService>(
        IArgumentsService,
        UnitTestArgumentsService,
        UNITTEST_PROVIDER
    );
    serviceManager.add<ITestManagerRunner>(
        ITestManagerRunner,
        PytestManagerRunner,
        PYTEST_PROVIDER
    );
    serviceManager.add<ITestManagerRunner>(
        ITestManagerRunner,
        NoseTestManagerRunner,
        NOSETEST_PROVIDER
    );
    serviceManager.add<ITestManagerRunner>(
        ITestManagerRunner,
        UnitTestTestManagerRunner,
        UNITTEST_PROVIDER
    );

    serviceManager.addSingleton<IUnitTestConfigurationService>(
        IUnitTestConfigurationService,
        UnitTestConfigurationService
    );
    serviceManager.addSingleton<IUnitTestManagementService>(
        IUnitTestManagementService,
        UnitTestManagementService
    );
    serviceManager.addSingleton<ITestResultDisplay>(
        ITestResultDisplay,
        TestResultDisplay
    );
    ...
    serviceManager.addSingleton<IUnitTestDiagnosticService>(
        IUnitTestDiagnosticService,
        UnitTestDiagnosticService
    );
    serviceManager.addSingleton<ITestMessageService>(
        ITestMessageService,
        TestMessageService,
        PYTEST_PROVIDER
    );
    ...
    serviceManager.addFactory<ITestManager>(ITestManagerFactory, context => {
        return (
            ...
            const serviceContainer = context.container.get<IServiceContainer>(
                IServiceContainer
            );

            switch (testProvider) {
                case NOSETEST_PROVIDER: {
                    return new NoseTestManager(
                        workspaceFolder,
                        rootDirectory,
                        serviceContainer
                    );
                ...
                case UNITTEST_PROVIDER: {
                    return new UnitTestTestManager(
                        workspaceFolder,
                        rootDirectory,
                        serviceContainer
                    );
                ...
                default: {
                    throw new Error(
                        `Unrecognized test provider '${testProvider}'`
                    );
                }
            }
        };
    ...
    serviceManager.addFactory<ITestManagerService>(
        ITestManagerServiceFactory,
        context => {
            return (workspaceFolder: Uri) => {
                const serviceContainer = context.container.get<
                    IServiceContainer
                >(IServiceContainer);
                const testsHelper = context.container.get<ITestsHelper>(
                    ITestsHelper
                );
                return new TestManagerService(
                    workspaceFolder,
                    testsHelper,
                    serviceContainer
                );
