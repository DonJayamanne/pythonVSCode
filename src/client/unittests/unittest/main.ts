    ...
    public async runTestImpl(
        ...
        if (debug) {
            args = this.argsService.filterArguments(
                this.settings.unitTest.unittestArgs,
        ...
        if (runFailedTests === true) {
            ...
            testsToRun.testFunction = tests.testFunctions
                .filter(fn => {
                    return (
                        fn.testFunction.status === TestStatus.Error ||
                        fn.testFunction.status === TestStatus.Fail
                    );
                })
