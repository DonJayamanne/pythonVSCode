...
export class TestManager extends BaseTestManager {
    ...
    constructor(
        ...
        this.argsService = this.serviceContainer.get<IArgumentsService>(
            IArgumentsService,
            this.testProvider
        );
        this.helper = this.serviceContainer.get<ITestsHelper>(ITestsHelper);
        this.runner = this.serviceContainer.get<ITestManagerRunner>(
            ITestManagerRunner,
            this.testProvider
        );
        this.testMessageService = this.serviceContainer.get<
            ITestMessageService
        >(ITestMessageService, this.testProvider);
    }
    ...
    public async runTestImpl(
        ...
        if (debug) {
            args = this.argsService.filterArguments(
                this.settings.unitTest.pyTestArgs,
        ...
        const testResults = await this.runner.runTest(
            this.testResultsService,
            options,
            this
        );
        const messages: IPythonUnitTestMessage[] = await this.testMessageService.getFilteredTestMessages(
            this.rootDirectory,
            testResults
        );
