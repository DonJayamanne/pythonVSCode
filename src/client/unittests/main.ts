...
export class UnitTestManagementService
    implements IUnitTestManagementService, Disposable {
    ...
    private readonly onDidChange: vscode.EventEmitter<
        void
    ...
    constructor(
        ...
        this.disposableRegistry = serviceContainer.get<Disposable[]>(
            IDisposableRegistry
        );
        this.outputChannel = serviceContainer.get<OutputChannel>(
            IOutputChannel,
            TEST_OUTPUT_CHANNEL
        );
    ...
    public async activate(): Promise<void> {
        this.workspaceTestManagerService = this.serviceContainer.get<
            IWorkspaceTestManagerService
        >(IWorkspaceTestManagerService);

        this.registerHandlers();
        this.registerCommands();
        this.autoDiscoverTests().catch(ex =>
            this.serviceContainer
                .get<ILogger>(ILogger)
                .logError("Failed to auto discover tests upon activation", ex)
        );
    }
    public async activateCodeLenses(
        symboldProvider: vscode.DocumentSymbolProvider
    ): Promise<void> {
        const testCollectionStorage = this.serviceContainer.get<
            ITestCollectionStorageService
        >(ITestCollectionStorageService);
    ...
    public async getTestManager(
        ...
        if (resource) {
            const wkspaceFolder = this.workspaceService.getWorkspaceFolder(
                resource
            );
        ...
        const testManager = this.workspaceTestManagerService!.getTestManager(
            wkspace
        );
        if (testManager) {
            return testManager;
        ...
        if (displayTestNotConfiguredMessage) {
            const configurationService = this.serviceContainer.get<
                IUnitTestConfigurationService
            >(IUnitTestConfigurationService);
    ...
    public async configurationChangeHandler(e: ConfigurationChangeEvent) {
        ...
        if (
            ...
            !settings.unitTest.pyTestEnabled &&
    ...
    public async discoverTestsForDocument(doc: TextDocument): Promise<void> {
        ...
        if (
            !tests ||
            !Array.isArray(tests.testFiles) ||
            tests.testFiles.length === 0
        ) {
            return;
        }
        if (
            tests.testFiles.findIndex(
                (f: TestFile) => f.fullPath === doc.uri.fsPath
            ) === -1
        ) {
            return;
        }
    ...
    public async discoverTests(
        ...
        if (
            testManager.status === TestStatus.Discovering ||
            testManager.status === TestStatus.Running
        ) {
            return;
        }

        if (!this.testResultDisplay) {
            this.testResultDisplay = this.serviceContainer.get<
                ITestResultDisplay
            >(ITestResultDisplay);
        ...
        this.testResultDisplay
            ...
            .catch(ex =>
                console.error("Python Extension: displayDiscoverStatus", ex)
            );
    ...
    public async displayStopUI(message: string): Promise<void> {
        ...
        const testDisplay = this.serviceContainer.get<ITestDisplay>(
            ITestDisplay
        );
    ...
    public async selectAndRunTestMethod(
        cmdSource: CommandSource,
        resource: Uri,
        debug?: boolean
    ) {
        ...
        const selectedTestFn = await testDisplay.selectTestFunction(
            testManager.workspaceFolder.fsPath,
            tests
        );
        if (!selectedTestFn) {
            return;
    ...
    public async selectAndRunTestFile(cmdSource: CommandSource) {
        ...
        const selectedFile = await testDisplay.selectTestFile(
            testManager.workspaceFolder.fsPath,
            tests
        );
        if (!selectedFile) {
        ...
        await this.runTestsImpl(cmdSource, testManager.workspaceFolder, {
    ...
    public async runCurrentTestFile(cmdSource: CommandSource) {
        ...
        const testManager = await this.getTestManager(
            true,
        ...
        const testFiles = tests.testFiles.filter(testFile => {
            return (
                testFile.fullPath ===
                this.documentManager.activeTextEditor!.document.uri.fsPath
            );
        });
    ...
    private registerCommands(): void {
        const disposablesRegistry = this.serviceContainer.get<Disposable[]>(
            IDisposableRegistry
        );
        ...
        const disposables = [
            ...
            commandManager.registerCommand(
                ...
                (
                    ...
                    file: Uri,
                    testToRun: TestsToRun
                ) => this.runTestsImpl(cmdSource, file, testToRun, false, true)
            ),
            commandManager.registerCommand(
                constants.Commands.Tests_View_UI,
            ...
            commandManager.registerCommand(
                constants.Commands.Tests_Stop,
                (_, resource: Uri) => this.stopTests(resource)
            ),
            commandManager.registerCommand(
                constants.Commands.Tests_ViewOutput,
                (_, cmdSource: CommandSource = CommandSource.commandPalette) =>
    ...
    private registerHandlers() {
        const documentManager = this.serviceContainer.get<IDocumentManager>(
            IDocumentManager
        );

        this.disposableRegistry.push(
            documentManager.onDidSaveTextDocument(
