...
export async function run(
    ...
    const executionInfo: ExecutionInfo = {
        ...
        args: options.args,
        moduleName:
            testExecutablePath && testExecutablePath.length > 0
                ? undefined
                : moduleName,
    ...
    if (testProvider === UNITTEST_PROVIDER) {
        promise = serviceContainer
            ...
            .createActivatedEnvironment(options.workspaceFolder)
            .then(executionService =>
                executionService.execObservable(options.args, {
                    ...spawnOptions
                })
            );
    } else if (
        typeof executionInfo.moduleName === "string" &&
        executionInfo.moduleName.length > 0
    ) {
        ...
        const pythonToolsExecutionService = serviceContainer.get<
            IPythonToolExecutionService
        >(IPythonToolExecutionService);
            ...
            result.out.subscribe(
                ...
                async () => {
                    ...
                    if (
                        ...
                        const isInstalled = await pythonExecutionService.isModuleInstalled(
                            moduleName
                        );
                        if (!isInstalled) {
                            return reject(
