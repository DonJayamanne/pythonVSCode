...
export async function selectTestWorkspace(): Promise<Uri | undefined> {
    if (
        ...
        const workspaceFolder = await (window as any).showWorkspaceFolderPick({
...
export class TestsHelper implements ITestsHelper {
    ...
    constructor(
        ...
        private flatteningVisitor: TestFlatteningVisitor,
    ...
    public placeTestFilesIntoFolders(tests: Tests): void {
        ...
        folders.forEach(dir => {
            dir.split(path.sep).reduce(
                (parentPath, currentName, index, values) => {
                    ...
                    if (!folderMap.has(newPath)) {
                        ...
                        tests.testFiles
                            .filter(fl => path.dirname(fl.name) === newPath)
                            .forEach(testFile => {
