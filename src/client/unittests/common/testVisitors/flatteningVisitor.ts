...
export class TestFlatteningVisitor implements ITestVisitor {
    ...
    public visitTestFile(testFile: TestFile): void {
        ...
        testFile.functions.forEach(fn =>
            this.addTestFunction(fn, testFile, packageName)
        );
        testFile.suites.forEach(suite =>
            this.visitTestSuiteOfAFile(suite, testFile)
        );
    }
    // tslint:disable-next-line:no-empty
    public visitTestFolder(testFile: TestFolder) {}
    private visitTestSuiteOfAFile(
        testSuite: TestSuite,
        parentTestFile: TestFile
    ): void {
        testSuite.functions.forEach(fn =>
            this.visitTestFunctionOfASuite(fn, testSuite, parentTestFile)
        );
        testSuite.suites.forEach(suite =>
            this.visitTestSuiteOfAFile(suite, parentTestFile)
        );
    ...
    private visitTestFunctionOfASuite(
        ...
        const key = `Function:${testFunction.name},Suite:${
            parentTestSuite.name
        },SuiteXmlName:${parentTestSuite.xmlName},ParentFile:${
            parentTestFile.fullPath
        }`;
    ...
    private addTestSuite(testSuite: TestSuite, parentTestFile: TestFile) {
        const key = `Suite:${testSuite.name},SuiteXmlName:${
            testSuite.xmlName
        },ParentFile:${parentTestFile.fullPath}`;
        if (this._flattenedTestSuites.has(key)) {
            return;
        }
    ...
    private addTestFunction(
        testFunction: TestFunction,
        parentTestFile: TestFile,
        parentTestPackage: string
    ) {
        const key = `Function:${testFunction.name},ParentFile:${
            parentTestFile.fullPath
        }`;
