...
export abstract class BaseTestManager implements ITestManager {
    ...
    public get workingDirectory(): string {
        return this.settings.unitTest.cwd &&
            this.settings.unitTest.cwd.length > 0
            ? this.settings.unitTest.cwd
            : this.rootDirectory;
    }
    public stop() {
            ...
            .catch((reason: {}) => {
                if (isNotInstalledError(reason as Error) && !quietMode) {
                    this.installer
                        ...
                        .catch(ex =>
                            console.error(
                                "Python Extension: isNotInstalledError",
                                ex
                            )
                        );
                }
    ...
    public async updateDiagnostics(
        tests: Tests,
        messages: IPythonUnitTestMessage[]
    ): Promise<void> {
