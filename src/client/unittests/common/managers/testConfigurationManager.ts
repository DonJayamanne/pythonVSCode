...
export abstract class TestConfigurationManager
    implements ITestConfigurationManager {
    ...
    constructor(
        ...
        this.testConfigSettingsService = serviceContainer.get<
    ...
    public async enable() {
        ...
        return this.testConfigSettingsService.enable(
            this.workspace,
            this.product
        );
    }
    ...
    protected selectTestDir(
        rootDir: string,
        subDirs: string[],
        customOptions: QuickPickItem[] = []
    ): Promise<string> {
        const options = {
