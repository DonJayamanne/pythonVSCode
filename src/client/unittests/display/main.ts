...
export class TestResultDisplay implements ITestResultDisplay {
    ...
    constructor(
        ...
        this.statusBar = this.appShell.createStatusBarItem(
    ...
    public displayProgressStatus(
        testRunResult: Promise<Tests>,
        debug: boolean = false
    ) {
    ...
    public displayDiscoverStatus(
        testDiscovery: Promise<Tests>,
    ...
    private updateTestRunWithSuccess(
        tests: Tests,
        debug: boolean = false
    ): Tests {
        this.clearProgressTicker();
        ...
        if (tests.summary.passed > 0) {
            statusText.push(
                `${constants.Octicons.Test_Pass} ${tests.summary.passed}`
            );
        ...
        if (tests.summary.skipped > 0) {
            statusText.push(
                `${constants.Octicons.Test_Skip} ${tests.summary.skipped}`
            );
        ...
        if (tests.summary.failures > 0) {
            statusText.push(
                `${constants.Octicons.Test_Fail} ${tests.summary.failures}`
            );
        ...
        if (tests.summary.errors > 0) {
            statusText.push(
                `${constants.Octicons.Test_Error} ${tests.summary.errors}`
            );
            toolTip.push(
                `${tests.summary.errors} Error${
                    tests.summary.errors > 1 ? "s" : ""
                }`
            );
            foreColor = "yellow";
        }
    ...
    private updateTestRunWithFailure(reason: any): Promise<any> {
        ...
        if (reason === CANCELLATION_REASON) {
            ...
            this.testsHelper.displayTestErrorMessage(
    ...
    private displayProgress(message: string, tooltip: string, command: string) {
        ...
        this.progressTimeout = setInterval(
            () => this.updateProgressTicker(),
            150
        );
    }
    private updateProgressTicker() {
        const text = `${this.progressPrefix} ${
            this.ticker[this.discoverCounter % 7]
        }`;
    ...
    private updateWithDiscoverSuccess(
        tests: Tests,
        quietMode: boolean = false
    ) {
        this.clearProgressTicker();
            ...
            this.appShell
                ...
                .then(item => {
                    if (item === "Disable Tests") {
                        this.disableTests().catch(ex =>
                            console.error("Python Extension: disableTests", ex)
                        );
                    }
                });
        }
    ...
    private updateWithDiscoverFailure(reason: any) {
        ...
        if (reason !== CANCELLATION_REASON) {
            ...
            this.statusBar.tooltip =
                "Discovering Tests failed (view 'Python Test Log' output panel for details)";
