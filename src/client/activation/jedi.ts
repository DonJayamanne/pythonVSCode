...
export class JediExtensionActivator implements IExtensionActivator {
    ...
    public async activate(): Promise<void> {
        ...
        context.subscriptions.push(
            ...activateGoToObjectDefinitionProvider(jediFactory)
        );
        ...
        context.subscriptions.push(
            languages.registerRenameProvider(
                ...
                new PythonRenameProvider(this.serviceManager)
            )
        );
        ...
        context.subscriptions.push(
            languages.registerHoverProvider(
                this.documentSelector,
                new PythonHoverProvider(jediFactory)
            )
        );
        context.subscriptions.push(
            languages.registerReferenceProvider(
                this.documentSelector,
                new PythonReferenceProvider(jediFactory)
            )
        );
        context.subscriptions.push(
        ...
        context.subscriptions.push(
            languages.registerCodeLensProvider(
                this.documentSelector,
                this.serviceManager.get<IShebangCodeLensProvider>(
        ...
        const symbolProvider = new JediSymbolProvider(
            serviceContainer,
            jediFactory
        );
        context.subscriptions.push(
        ...
        const pythonSettings = this.serviceManager
        ...
        context.subscriptions.push(
            languages.registerRenameProvider(
                PYTHON,
                new PythonRenameProvider(serviceContainer)
            )
        );

        const testManagementService = this.serviceManager.get<
            IUnitTestManagementService
        >(IUnitTestManagementService);
        testManagementService
            .activate()
            .then(() =>
                testManagementService.activateCodeLenses(symbolProvider)
            )
            .catch(ex =>
                this.serviceManager
                    .get<ILogger>(ILogger)
                    .logError("Failed to activate Unit Tests", ex)
            );
    }
