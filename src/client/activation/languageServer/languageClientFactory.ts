...
export class BaseLanguageClientFactory implements ILanguageClientFactory {
    constructor(
        ...
        @named(LanguageClientFactory.downloaded)
        private readonly downloadedFactory: ILanguageClientFactory,
        @inject(ILanguageClientFactory)
        @named(LanguageClientFactory.simple)
        private readonly simpleFactory: ILanguageClientFactory,
...
export class SimpleLanguageClientFactory implements ILanguageClientFactory {
    ...
    public async createLanguageClient(
        ...
        const serverModule = path.join(
            EXTENSION_ROOT_DIR,
            languageServerFolder,
            this.platformData.engineDllName
        );
        const serverOptions: ServerOptions = {
            run: {
