...
import {
    ...
    INugetService,
    NugetPackage
} from "../../common/nuget/types";
import { IPlatformService } from "../../common/platform/types";
import {
    IConfigurationService,
    IExtensions,
    LanguageServerDownloadChannels
} from "../../common/types";
...
import {
    ...
    LanguageServerPackageStorageContainers
} from "./languageServerPackageRepository";

const downloadBaseFileName = "Python-Language-Server";
export const maxMajorVersion = 0;
export const PackageNames = {
    [PlatformName.Windows32Bit]: `${downloadBaseFileName}-${
        PlatformName.Windows32Bit
    }`,
    [PlatformName.Windows64Bit]: `${downloadBaseFileName}-${
        PlatformName.Windows64Bit
    }`,
    [PlatformName.Linux64Bit]: `${downloadBaseFileName}-${
        PlatformName.Linux64Bit
    }`,
    [PlatformName.Mac64Bit]: `${downloadBaseFileName}-${PlatformName.Mac64Bit}`
};

@injectable()
export class LanguageServerPackageService
    implements ILanguageServerPackageService {
    ...
    public async getLatestNugetPackageVersion(): Promise<NugetPackage> {
        ...
        const nugetRepo = this.serviceContainer.get<INugetRepository>(
            INugetRepository,
            downloadChannel
        );
        const packageName = this.getNugetPackageName();
        traceVerbose(
            `Listing packages for ${downloadChannel} for ${packageName}`
        );
    ...
    public getLanguageServerDownloadChannel(): LanguageServerDownloadChannels {
        const configService = this.serviceContainer.get<IConfigurationService>(
            IConfigurationService
        );
    ...
    protected getValidPackage(packages: NugetPackage[]): NugetPackage {
        const nugetService = this.serviceContainer.get<INugetService>(
            INugetService
        );
        ...
        const minimumVersion = this.appEnv.packageJson
            .languageServerVersion as string;
        ...
        return {
            ...
            uri: `${azureCDNBlobStorageAccount}/${
                LanguageServerPackageStorageContainers.stable
    ...
    private isAlphaVersionOfExtension() {
        ...
        return (
            version.prerelease.length > 0 && version.prerelease[0] === "alpha"
        );
    }
}
