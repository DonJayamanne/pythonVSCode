...
@injectable()
export class LanguageServerAnalysisOptions
    ...
    constructor(
        ...
        @inject(IInterpreterDataService)
        private readonly interpreterDataService: IInterpreterDataService,
    ...
    public async initialize(resource: Resource) {
        ...
        let disposable = this.workspace.onDidChangeConfiguration(
            this.onSettingsChangedHandler,
            this
        );
        this.disposables.push(disposable);

        disposable = this.interpreterService.onDidChangeInterpreter(
            () => this.onSettingsChangedHandler(),
            this
        );
        this.disposables.push(disposable);
    ...
    public async getAnalysisOptions(): Promise<LanguageClientOptions> {
        ...
        try {
            interpreterData = await this.interpreterDataService.getInterpreterData(
                this.resource
            );
        } catch (ex) {
            traceError(
                "Unable to determine path to the Python interpreter. IntelliSense will be limited.",
                ex
            );
        }
        ...
        // tslint:disable-next-line:no-string-literal
        properties["DatabasePath"] = path.join(
            this.context.extensionPath,
            this.languageServerFolder
        );

        let searchPaths = interpreterData
            ? interpreterData.searchPaths.split(path.delimiter)
            : [];
        ...
        if (vars.PYTHONPATH && vars.PYTHONPATH.length > 0) {
            const paths = vars.PYTHONPATH.split(
                this.pathUtils.delimiter
    ...
    protected getPythonExcludeSection(list: string[]): void {
        ...
        const paths =
            pythonSettings && pythonSettings.linting
                ? pythonSettings.linting.ignorePatterns
                : undefined;
    ...
    protected getTypeshedPaths(): string[] {
        ...
        return settings.analysis.typeshedPaths &&
    ...
    protected async onSettingsChangedHandler(
        e?: ConfigurationChangeEvent
    ): Promise<void> {
        if (e && !e.affectsConfiguration("python", this.resource)) {
            return;
        }
        this.onSettingsChanged().catch(ex =>
            traceError("Failed to detect changes", ex)
        );
    }
    @traceDecorators.verbose(
        "Changes in python settings detected in analysis options"
    )
    @debounce(1000)
    protected async onSettingsChanged(): Promise<void> {
        const idata = await this.interpreterDataService.getInterpreterData(
            this.resource
        );
    ...
    protected async notifyIfValuesHaveChanged(
        oldArray: string[],
        newArray: string[]
    ): Promise<void> {
