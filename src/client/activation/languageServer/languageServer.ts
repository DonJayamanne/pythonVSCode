...
import {
    ILanaguageServer as ILanguageServer,
...
export class LanguageServer implements ILanguageServer {
    ...
    public async start(
        ...
        this.languageClient = await this.factory.createLanguageClient(
            resource,
            options
        );
        ...
        this.languageClient.onTelemetry(telemetryEvent => {
            const eventName =
                telemetryEvent.EventName || PYTHON_LANGUAGE_SERVER_TELEMETRY;
            sendTelemetryEvent(
                eventName,
