...
export class InterpreterDataService implements IInterpreterDataService {
    ...
    public async getInterpreterData(
        resource: Resource
    ): Promise<InterpreterData | undefined> {
        const executionFactory = this.serviceContainer.get<
            IPythonExecutionFactory
        >(IPythonExecutionFactory);
        ...
        let interpreterData = this.context.globalState.get<InterpreterData>(
            cacheKey
        );
        ...
        if (interpreterData) {
            ...
            if (interpreterData.dataVersion !== DataVersion) {
                ...
                const currentHash = await this.getInterpreterHash(
                    interpreterPath
                );
        ...
        if (interpreterChanged || !interpreterData) {
            interpreterData = await this.getInterpreterDataFromPython(
                execService,
                interpreterPath
            );
            ...
            this.verifySearchPaths(
                interpreterData.searchPaths,
                interpreterPath,
                execService
            );
        }
        ...
        fs.lstat(pythonExecutable, (err, stats) => {
            if (err) {
                ...
                const actual = createHash("sha512")
                    .update(`${stats.ctime}-${stats.mtime}`)
                    .digest("hex");
    ...
    private async getInterpreterDataFromPython(
        execService: IPythonExecutionService,
        interpreterPath: string
    ): Promise<InterpreterData> {
        const result = await execService.exec(
        ...
        if (!result.stdout) {
            throw Error(
                "Unable to determine Python interpreter version and system prefix."
            );
        }
        ...
        if (!output || output.length < 1) {
            throw Error(
                "Unable to parse version and and system prefix from the Python interpreter output."
            );
        }
        ...
        if (
            !majorMatches ||
            majorMatches.length < 2 ||
            !minorMatches ||
            minorMatches.length < 2
        ) {
        ...
        return new InterpreterDataCls(
            DataVersion,
            interpreterPath,
            `${majorMatches[1]}.${minorMatches[1]}`,
            searchPaths,
            hash
        );
    }

    private async getSearchPaths(
        execService: IPythonExecutionService
    ): Promise<string> {
        const result = await execService.exec(
            [
                "-c",
                'import sys; import os; print(sys.path + os.getenv("PYTHONPATH", "").split(os.pathsep));'
            ],
            {}
        );
        if (!result.stdout) {
        ...
        this.getSearchPaths(execService)
            .then(async paths => {
                if (paths !== currentPaths) {
                    ...
                    const appShell = this.serviceContainer.get<
                        IApplicationShell
                    >(IApplicationShell);
                    await appShell.showWarningMessage(
