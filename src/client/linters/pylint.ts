...
const REGEX =
    "(?<line>\\d+),(?<column>-?\\d+),(?<type>\\w+),(?<code>[\\w-]+):(?<message>.*)\\r?(\\n|$)";
...
export class Pylint extends BaseLinter {
    ...
    constructor(
        ...
        this.platformService = serviceContainer.get<IPlatformService>(
            IPlatformService
        );
    }
    ...
    protected async runLinter(
        ...
        if (
            ...
            !(await Pylint.hasConfigrationFileInWorkspace(
                this.fileSystem,
        ...
        messages.forEach(msg => {
            msg.severity = this.parseMessagesSeverity(
                msg.type,
                this.pythonSettings.linting.pylintCategorySeverity
            );
        });
    ...
    // tslint:disable-next-line:member-ordering
    public static async hasConfigrationFileInWorkspace(
        fs: IFileSystem,
        folder: string,
        root: string
    ): Promise<boolean> {
        ...
        do {
            ...
            !fs.arePathsSame(current, root) &&
            !fs.arePathsSame(current, above)
        );
        return false;
    }
}
