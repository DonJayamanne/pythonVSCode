...
export class PyDocStyle extends BaseLinter {
    ...
    protected async parseMessages(
        ...
        for (
            let counter = 0;
            counter < oldOutputLines.length / 2;
            counter += 1
        ) {
            outputLines.push(
                oldOutputLines[2 * counter] + oldOutputLines[2 * counter + 1]
            );
        }
        ...
        return (
            outputLines
                .filter(
                    (value, index) =>
                        index < maxLines && value.indexOf(":") >= 0
                )
                .map(line => {
                ...
                .map(line => {
                    try {
                        ...
                        const sourceStart = sourceLine.indexOf(
                            trmmedSourceLine
                        );
                        ...
                        this.logger.logError(
                            `Failed to parse pydocstyle line '${line}'`,
                            ex
                        );
                        return;
                    }
