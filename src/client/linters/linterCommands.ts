...
export class LinterCommands implements IDisposable {
    ...
    constructor(private serviceContainer: IServiceContainer) {
        ...
        commandManager.registerCommand(
            Commands.Set_Linter,
            this.setLinterAsync.bind(this)
        );
        commandManager.registerCommand(
            Commands.Enable_Linter,
            this.enableLintingAsync.bind(this)
        );
        commandManager.registerCommand(
            Commands.Run_Linter,
            this.runLinting.bind(this)
        );
    }
    public dispose() {
    ...
    public async setLinterAsync(): Promise<void> {
        ...
        const selection = await this.appShell.showQuickPick(
            linterList,
            quickPickOptions
        );
        if (selection !== undefined) {
            if (selection === "Disable Linting") {
                ...
                sendTelemetryEvent(SELECT_LINTER, undefined, {
                    enabled: false
                });
            } else {
                ...
                await this.linterManager.setActiveLintersAsync(
                    [linters[index].product],
                    this.settingsUri
                );
                sendTelemetryEvent(SELECT_LINTER, undefined, {
                    tool: selection as LinterId,
                    enabled: true
                });
            }
        }
    ...
    public async enableLintingAsync(): Promise<void> {
        ...
        const current = (await this.linterManager.isLintingEnabled(
            true,
            this.settingsUri
        ))
            ? options[0]
        ...
        if (selection !== undefined) {
            ...
            await this.linterManager.enableLintingAsync(
                enable,
                this.settingsUri
            );
        }
