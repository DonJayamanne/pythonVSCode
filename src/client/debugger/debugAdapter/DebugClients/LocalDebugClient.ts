...
export class LocalDebugClient extends DebugClient<LaunchRequestArguments> {
    ...
    public CreateDebugServer(
        serviceContainer?: IServiceContainer
    ): BaseDebugServer {
        this.debugServer = new LocalDebugServerV2(
            this.debugSession,
            this.args,
            serviceContainer!
        );
    ...
    public async LaunchApplicationToDebug(
        ...
        const helper = new DebugClientHelper(
            environmentVariablesService,
            pathUtils,
            currentProcess
        );
        const environmentVariables = await helper.getEnvironmentVariables(
            this.args
        );
        // tslint:disable-next-line:max-func-body-length cyclomatic-complexity no-any
        return new Promise<any>((resolve, reject) => {
            const fileDir =
                this.args && this.args.program
                    ? path.dirname(this.args.program)
                    : "";
            let processCwd = fileDir;
            if (
                typeof this.args.cwd === "string" &&
                this.args.cwd.length > 0 &&
                this.args.cwd !== "null"
            ) {
            ...
            if (
                typeof this.args.pythonPath === "string" &&
                this.args.pythonPath.trim().length > 0
            ) {
            ...
            switch (this.args.console) {
                ...
                case "integratedTerminal": {
                    const isSudo =
                        Array.isArray(this.args.debugOptions) &&
                        this.args.debugOptions.some(opt => opt === "Sudo");
                ...
                default: {
                    this.pyProc = spawn(pythonPath, args, {
                        cwd: processCwd,
                    ...
                    this.debugServer!.DebugClientConnected.then(resolve).catch(
                        ex =>
                            console.error(
                                "Python Extension: debugServer.DebugClientConnected",
                                ex
                            )
                    );
                }
    ...
    protected handleProcessOutput(
        proc: ChildProcess,
        failedToLaunch: (error: Error | string | Buffer) => void
    ) {
        proc.on("error", error => {
    ...
    private buildLaunchArguments(cwd: string, debugPort: number): string[] {
        return [
            ...this.buildDebugArguments(cwd, debugPort),
            ...this.buildStandardArguments()
        ];
    }
    ...
    protected buildStandardArguments() {
        const programArgs =
            Array.isArray(this.args.args) && this.args.args.length > 0
                ? this.args.args
                : [];
        if (
            typeof this.args.module === "string" &&
            this.args.module.length > 0
        ) {
    ...
    private launchExternalTerminal(
        sudo: boolean,
        cwd: string,
        pythonPath: string,
        args: string[],
        ...
        return new Promise((resolve, reject) => {
            if (this.canLaunchTerminal) {
                ...
                const isExternalTerminal =
                    this.args.console === "externalTerminal";
                const consoleKind = isExternalTerminal
                    ? "external"
                    : "integrated";
                ...
                this.debugSession.runInTerminalRequest(
                    termArgs,
                    5000,
                    response => {
                        if (response.success) {
                ...
                open({
                    ...
                    error => {
                        if (
                            this.debugServerStatus === DebugServerStatus.Running
                        ) {
                            return;
                        }
                        reject(error);
