...
export class DebugClientHelper {
    ...
    public async getEnvironmentVariables(
        ...
        const debugLaunchEnvVars: { [key: string]: string } =
        ...
        if (typeof args.console !== "string" || args.console === "none") {
            ...
            if (
                env[pathVariableName] === undefined &&
                typeof this.process.env[pathVariableName] === "string"
            ) {
                env[pathVariableName] = this.process.env[pathVariableName];
            }
            if (
                env.PYTHONPATH === undefined &&
                typeof this.process.env.PYTHONPATH === "string"
            ) {
