...
import {
    CreateAttachDebugClient,
    CreateLaunchDebugClient
} from "./DebugClients/DebugFactory";
...
export class PythonDebugger extends DebugSession {
    ...
    protected initializeRequest(
        ...
        if (typeof args.supportsRunInTerminalRequest === "boolean") {
            this.supportsRunInTerminalRequest =
                args.supportsRunInTerminalRequest;
        }
    ...
    protected attachRequest(
        ...
        this.debugServer!.Start()
            ...
            .catch(ex => {
                ...
                const message =
                    this.getUserFriendlyAttachErrorMessage(ex) ||
                    "Attach Failed";
    ...
    protected launchRequest(
        ...
        if (
            ...
            args.program &&
            !fs.fileExistsSync(args.program)
        ) {
        ...
        this.launchPTVSD(args)
            ...
            .catch(ex => {
                const message =
                    this.getUserFriendlyLaunchErrorMessage(args, ex) ||
                    "Debug Error";
                this.sendErrorResponse(
                    response,
                    { format: message, id: 1 },
    ...
    private async launchPTVSD(args: LaunchRequestArguments) {
        const launcher = CreateLaunchDebugClient(
            args,
            this,
            this.supportsRunInTerminalRequest
        );
    ...
    private getConnectionTimeout(args: LaunchRequestArguments) {
        ...
        const connectionTimeout =
            typeof (args as any).timeout === "number"
                ? ((args as any).timeout as number)
                : DEBUGGER_CONNECT_TIMEOUT;
        return Math.max(connectionTimeout, MIN_DEBUGGER_CONNECT_TIMEOUT);
    }
    private getUserFriendlyLaunchErrorMessage(
        ...
        if (isNotInstalledError(error)) {
            return `Failed to launch the Python Process, please validate the path '${
                launchArgs.pythonPath
            }'`;
        } else {
            return errorMsg;
        }
...
class DebugManager implements Disposable {
    ...
    private readonly initializeRequestDeferred: Deferred<
    ...
    private readonly launchRequestDeferred: Deferred<
    ...
    private readonly attachRequestDeferred: Deferred<
    ...
    constructor(private readonly serviceContainer: IServiceContainer) {
        ...
        this.protocolMessageWriter = this.serviceContainer.get<
            IProtocolMessageWriter
        >(IProtocolMessageWriter);

        this.inputProtocolParser = this.serviceContainer.get<IProtocolParser>(
            IProtocolParser
        );
        this.inputProtocolParser.connect(this.throughInputStream);
        this.disposables.push(this.inputProtocolParser);
        this.outputProtocolParser = this.serviceContainer.get<IProtocolParser>(
            IProtocolParser
        );
        ...
        this.protocolLogger.connect(
            this.throughInputStream,
            this.throughOutputStream
        );
        this.disposables.push(this.protocolLogger);

        this.initializeRequestDeferred = createDeferred<
            DebugProtocol.InitializeRequest
        >();
        this.launchRequestDeferred = createDeferred<
            DebugProtocol.LaunchRequest
        >();
        this.attachRequestDeferred = createDeferred<
            DebugProtocol.AttachRequest
        >();
    }
    public dispose() {
    ...
    public async start() {
        const debugStreamProvider = this.serviceContainer.get<
            IDebugStreamProvider
        >(IDebugStreamProvider);
        const {
            input,
            output
        } = await debugStreamProvider.getInputAndOutputStreams();
    ...
    private shutdown = async () => {
        ...
        if (
            ...
            try {
                ...
                this.sendMessage(
                    new Response(this.disconnectRequest, ""),
                    this.outputStream
                );
            } catch (err) {
    ...
    private startDebugSession() {
        ...
        this.debugSession.start(
            this.debugSessionInputStream,
            this.debugSessionOutputStream
        );
    }
    private interceptProtocolMessages() {
        ...
        this.inputProtocolParser.once(
            "request_initialize",
            this.onRequestInitialize
        );
        ...
        this.outputProtocolParser.once(
            "event_terminated",
            this.onEventTerminated
        );
        this.outputProtocolParser.once(
            "response_disconnect",
            this.onResponseDisconnect
        );
    }
    /**
    ...
    private connectVSCodeToPTVSD = async (
        ...
        if (this.launchOrAttach === "launch") {
            const debugSoketProtocolParser = this.serviceContainer.get<
                IProtocolParser
            >(IProtocolParser);
            debugSoketProtocolParser.connect(this.socket);
            debugSoketProtocolParser.once(
                "event_process",
                (proc: DebugProtocol.ProcessEvent) => {
        ...
        ((this.inputStream as any) as NodeJS.ReadStream).unpipe<Writable>(
            this.debugSessionInputStream
        );
    ...
    private onRequestInitialize = (
        request: DebugProtocol.InitializeRequest
    ) => {
    ...
    private onRequestLaunch = (request: DebugProtocol.LaunchRequest) => {
        ...
        this.loggingEnabled =
            (request.arguments as LaunchRequestArguments).logToFile === true;
    ...
    private onRequestAttach = (request: DebugProtocol.AttachRequest) => {
        ...
        this.loggingEnabled =
            (request.arguments as AttachRequestArguments).logToFile === true;
...
async function startDebugger() {
    ...
    try {
        ...
        // Notify the user.
        protocolMessageWriter.write(
            process.stdout,
            new Event("error", message)
        );
        protocolMessageWriter.write(
            process.stdout,
            new OutputEvent(`${message}${EOL}${details}`, "stderr")
        );
    }
}
...
process.on("uncaughtException", (err: Error) => {
    logger.error(
        `Uncaught Exception: ${err && err.message ? err.message : ""}`
    );
