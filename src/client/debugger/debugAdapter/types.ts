...
export type LocalDebugOptions = {
    port: number;
    host: string;
    customDebugger?: boolean;
};
export type RemoteDebugOptions = LocalDebugOptions & {
...
export interface ILocalDebugLauncherScriptProvider
    extends IDebugLauncherScriptProvider<LocalDebugOptions> {
    getLauncherArgs(options: LocalDebugOptions): string[];
}

export interface IRemoteDebugLauncherScriptProvider
    extends IDebugLauncherScriptProvider<RemoteDebugOptions> {}
...
export interface IDebugStreamProvider {
    ...
    getInputAndOutputStreams(): Promise<{
