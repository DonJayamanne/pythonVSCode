...
@injectable()
export class DjangoLaunchDebugConfigurationProvider
    ...
    public async buildConfiguration(
        ...
        const defaultProgram = `${workspaceFolderToken}${
        ...
        if (!program) {
            const selectedProgram = await input.showInputBox({
                ...
                validate: value =>
                    this.validateManagePy(state.folder, defaultProgram, value)
            });
            if (selectedProgram) {
    ...
    protected async getManagePyPath(
        folder: WorkspaceFolder | undefined
    ): Promise<string | undefined> {
        if (!folder) {
            return;
