...
export abstract class BaseConfigurationResolver<T extends DebugConfiguration>
    ...
    protected resolveAndUpdatePythonPath(
        ...
        if (
            debugConfiguration.pythonPath === "${config:python.pythonPath}" ||
            !debugConfiguration.pythonPath
        ) {
            const pythonPath = this.configurationService.getSettings(
                workspaceFolder
            ).pythonPath;
    ...
    protected isDebuggingFlask(
        debugConfiguration: Partial<
            LaunchRequestArguments & AttachRequestArguments
        >
    ) {
        return debugConfiguration.module &&
            debugConfiguration.module.toUpperCase() === "FLASK"
            ? true
            : false;
    }
    protected sendTelemetry(
        ...
        const telemetryProps: DebuggerTelemetry = {
            ...
            hasEnvVars:
                typeof debugConfiguration.env === "object" &&
                Object.keys(debugConfiguration.env).length > 0,
