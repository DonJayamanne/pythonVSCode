...
export class BaseVirtualEnvService extends CacheableLocatorService {
    ...
    private async suggestionsFromKnownVenvs(resource?: Uri) {
        const searchPaths = await this.searchPathsProvider.getSearchPaths(
            resource
        );
        return Promise.all(
            searchPaths.map(dir =>
                this.lookForInterpretersInVenvs(dir, resource)
            )
        ).then(listOfInterpreters => flatten(listOfInterpreters));
    }
    private async lookForInterpretersInVenvs(
        pathToCheck: string,
        resource?: Uri
    ) {
        return this.fileSystem
            ...
            .then(subDirs =>
                Promise.all(this.getProspectiveDirectoriesForLookup(subDirs))
            )
            ...
            .then(interpreters =>
                Promise.all(
                    interpreters.map(interpreter =>
                        this.getVirtualEnvDetails(interpreter, resource)
                    )
                )
            )
            .then(interpreters =>
            ...
            .catch(err => {
                console.error(
                    "Python Extension (lookForInterpretersInVenvs):",
                    err
                );
                // Ignore exceptions.
            ...
            this.fileSystem
                ...
                .then(dirs => {
                    const scriptOrBinDirs = dirs.filter(dir => {
                        ...
                        return this.fileSystem.arePathsSame(
                            folderName,
                            dirToLookFor
                        );
                    });
                    return scriptOrBinDirs.length === 1
                        ? scriptOrBinDirs[0]
                        : "";
                })
                .catch(err => {
                    console.error(
                        "Python Extension (getProspectiveDirectoriesForLookup):",
                        err
                    );
                    // Ignore exceptions.
                    return "";
