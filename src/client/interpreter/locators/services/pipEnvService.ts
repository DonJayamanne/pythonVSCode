...
export class PipEnvService extends CacheableLocatorService
    ...
    constructor(
        ...
        this.helper = this.serviceContainer.get<IInterpreterHelper>(
            IInterpreterHelper
        );
        this.processServiceFactory = this.serviceContainer.get<
    ...
    public dispose() {}
    public async isRelatedPipEnvironment(
        dir: string,
        pythonPath: string
    ): Promise<boolean> {
    ...
    private async getInterpreterFromPipenv(
        pipenvCwd: string
    ): Promise<PythonInterpreter | undefined> {
        const interpreterPath = await this.getInterpreterPathFromPipenv(
            pipenvCwd
        );
        if (!interpreterPath) {
            return;
    ...
    private async getInterpreterPathFromPipenv(
        ...
        try {
            ...
            return pythonPath && (await this.fs.fileExists(pythonPath))
                ? pythonPath
                : undefined;
            // tslint:disable-next-line:no-empty
        } catch (error) {
    ...
    private async checkIfPipFileExists(cwd: string): Promise<boolean> {
        ...
        if (
            typeof pipFileName === "string" &&
            (await this.fs.fileExists(path.join(cwd, pipFileName)))
        ) {
            return true;
        }
    ...
    private async invokePipenv(
        ...
        try {
            ...
            const result = await processService.exec(execName, [arg], {
                cwd: rootPath
            });
            if (result) {
            ...
            this.logger.logWarning(
                `Relevant Environment Variables ${JSON.stringify(
                    enviromentVariableValues,
                    undefined,
                    4
                )}`
            );
