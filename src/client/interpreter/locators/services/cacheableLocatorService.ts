...
@injectable()
export abstract class CacheableLocatorService
    implements IInterpreterLocatorService {
    protected readonly _hasInterpreters: Deferred<boolean>;
    private readonly promisesPerResource = new Map<
        string,
        Deferred<PythonInterpreter[]>
    >();
    ...
    public async getInterpreters(
        ...
        if (!deferred || ignoreCache) {
            ...
            this.addHandlersForInterpreterWatchers(
                cacheKey,
                resource
            ).ignoreErrors();
        ...
        return Array.isArray(cachedInterpreters)
            ? cachedInterpreters
            : deferred.promise;
    }
    protected async addHandlersForInterpreterWatchers(
        ...
        const disposableRegisry = this.serviceContainer.get<Disposable[]>(
            IDisposableRegistry
        );
        watchers.forEach(watcher => {
            watcher.onDidCreate(
                () => {
                    Logger.verbose(
                        `Interpreter Watcher change handler for ${
                            this.cacheKeyPrefix
                        }`
                    );
    ...
    ): Promise<IInterpreterWatcher[]> {
        return [];
    }

    protected abstract getInterpretersImplementation(
        resource?: Uri
    ): Promise<PythonInterpreter[]>;
    protected createPersistenceStore(resource?: Uri) {
        const cacheKey = this.getCacheKey(resource);
        const persistentFactory = this.serviceContainer.get<
            IPersistentStateFactory
        >(IPersistentStateFactory);
        if (this.cachePerWorkspace) {
            return persistentFactory.createWorkspacePersistentState<
                PythonInterpreter[]
            >(cacheKey, undefined as any);
        } else {
            return persistentFactory.createGlobalPersistentState<
                PythonInterpreter[]
            >(cacheKey, undefined as any);
        }
    }
    protected getCachedInterpreters(
        resource?: Uri
    ...
    protected async cacheInterpreters(
    ...
    protected getCacheKey(resource?: Uri) {
        ...
        return workspace
            ? `${this.cacheKeyPrefix}:${md5(workspace.uri.fsPath)}`
            : this.cacheKeyPrefix;
    }
}
