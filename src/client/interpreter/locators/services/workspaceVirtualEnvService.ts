...
export class WorkspaceVirtualEnvService extends BaseVirtualEnvService {
    public constructor(
        ...
        @inject(IInterpreterWatcherBuilder)
        private readonly builder: IInterpreterWatcherBuilder
    ) {
        super(
            workspaceVirtualEnvPathProvider,
            serviceContainer,
    ...
    protected async getInterpreterWatchers(
        ...
        return [
            await this.builder.getWorkspaceVirtualEnvInterpreterWatcher(
                resource
            )
        ];
    }
...
export class WorkspaceVirtualEnvironmentsSearchPathProvider
    implements IVirtualEnvironmentsSearchPathProvider {
    public constructor(
    ...
    public async getSearchPaths(resource?: Uri): Promise<string[]> {
        ...
        if (
            ...
            workspaceService.workspaceFolders.length > 0
        ) {
            let wsPath: string | undefined;
            if (resource && workspaceService.workspaceFolders.length > 1) {
                const wkspaceFolder = workspaceService.getWorkspaceFolder(
                    resource
                );
                if (wkspaceFolder) {
