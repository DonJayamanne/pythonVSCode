...
type CachedPythonInterpreter = Partial<PythonInterpreter> & {
    fileHash: string;
};
...
export class InterpreterHelper implements IInterpreterHelper {
    ...
    constructor(
        ...
        this.persistentFactory = this.serviceContainer.get<
    ...
    public async getInterpreterInformation(
        pythonPath: string
    ): Promise<undefined | Partial<PythonInterpreter>> {
        let fileHash = await this.fs.getFileHash(pythonPath).catch(() => "");
        fileHash = fileHash ? fileHash : "";
        const store = this.persistentFactory.createGlobalPersistentState<
            CachedPythonInterpreter
        >(`${pythonPath}.v3`, undefined, EXPITY_DURATION);
        ...
        try {
            ...
            console.error(
                `Failed to get interpreter information for '${pythonPath}'`,
                ex
            );
            return;
        }
