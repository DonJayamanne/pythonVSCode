...
export class EnvironmentActivationService
    implements IEnvironmentActivationService, IDisposable {
    ...
    public async getActivatedEnvironmentVariables(
        ...
        const activationCommand = this.fixActivationCommands(
            activationCommands
        ).join(" && ");
    ...
    protected parseEnvironmentOutput(
        output: string
    ): NodeJS.ProcessEnv | undefined {
        output = output.substring(
            output.indexOf(getEnvironmentPrefix) + getEnvironmentPrefix.length
        );
