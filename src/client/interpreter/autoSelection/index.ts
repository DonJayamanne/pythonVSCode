...
export class InterpreterAutoSelectionService
    implements IInterpreterAutoSelectionService {
    private readonly didAutoSelectedInterpreterEmitter = new EventEmitter<
        void
    >();
    private readonly autoSelectedInterpreterByWorkspace = new Map<
        string,
    ...
    public async autoSelectInterpreter(resource: Resource): Promise<void> {
        ...
        Promise.all(
            this.rules.map(item => item.autoSelectInterpreter(undefined))
        ).ignoreErrors();
    }
    ...
    protected async storeAutoSelectedInterperter(
        resource: Resource,
        interpreter: PythonInterpreter | undefined
    ) {
        const workspaceFolderPath = this.getWorkspacePathKey(resource);
        if (workspaceFolderPath === workspacePathNameForGlobalWorkspaces) {
            ...
            if (
                ...
                interpreter.version &&
                compare(
                    this.globallyPreferredInterpreter.value.version.raw,
    ...
    private getWorkspaceState(
        resource: Resource
    ): undefined | IPersistentState<PythonInterpreter | undefined> {
        const workspaceUri = this.interpreterHelper.getActiveWorkspaceUri(
            resource
        );
        if (!workspaceUri) {
            return;
        }
        const key = `autoSelectedWorkspacePythonInterpreter-${
