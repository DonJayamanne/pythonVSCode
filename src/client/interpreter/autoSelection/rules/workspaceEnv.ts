...
export class WorkspaceVirtualEnvInterpretersAutoSelectionRule extends BaseRuleService {
    ...
    protected async onAutoSelectInterpreter(
        ...
        const pipEnvPromise = createDeferredFromPromise(
            this.pipEnvInterpreterLocator.getInterpreters(
                workspacePath.folderUri
            )
        );
        ...
        const interpreters = await Promise.race([
            pipEnvPromise.promise,
        ...
        if (Array.isArray(interpreters) && interpreters.length > 0) {
            ...
            const [pipEnv, virtualEnv] = await Promise.all([
                pipEnvPromise.promise,
                virtualEnvPromise.promise
            ]);
            const pipEnvList = Array.isArray(pipEnv) ? pipEnv : [];
            const virtualEnvList = Array.isArray(virtualEnv) ? virtualEnv : [];
            bestInterpreter = this.helper.getBestInterpreter(
                pipEnvList.concat(virtualEnvList)
            );
        }
        if (bestInterpreter && manager) {
            await this.cacheSelectedInterpreter(
                workspacePath.folderUri,
                bestInterpreter
            );
            await manager.setWorkspaceInterpreter(
                workspacePath.folderUri!,
                bestInterpreter
            );
        }
        traceVerbose(
            `Selected Interpreter from ${this.ruleName}, ${
                bestInterpreter
                    ? JSON.stringify(bestInterpreter)
                    : "Nothing Selected"
            }`
        );
        return NextAction.runNextRule;
    ...
    protected async getWorkspaceVirtualEnvInterpreters(
        ...
        const interpreters = await this.workspaceVirtualEnvInterpreterLocator.getInterpreters(
            resource,
            true
        );
        const workspacePath =
