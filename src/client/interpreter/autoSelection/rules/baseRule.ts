...
export abstract class BaseRuleService implements IInterpreterAutoSelectionRule {
    ...
    private readonly stateStore: IPersistentState<
    ...
    constructor(
        ...
        this.stateStore = stateFactory.createGlobalPersistentState<
            PythonInterpreter | undefined
        >(`InterpreterAutoSeletionRule-${this.ruleName}`, undefined);
    }
    public setNextRule(rule: IInterpreterAutoSelectionRule): void {
        this.nextRule = rule;
    }
    @traceDecorators.verbose("autoSelectInterpreter")
    public async autoSelectInterpreter(
        resource: Resource,
    ...
    public getPreviouslyAutoSelectedInterpreter(
        _resource: Resource
    ): PythonInterpreter | undefined {
        const value = this.stateStore.value;
        traceVerbose(
            `Current value for rule ${this.ruleName} is ${
    ...
    protected async setGlobalInterpreter(
        ...
        const preferredInterpreter = manager.getAutoSelectedInterpreter(
            undefined
        );
        const comparison =
            preferredInterpreter && preferredInterpreter.version
                ? compare(
                      ...
                      preferredInterpreter.version.raw
                  )
                : 1;
        if (comparison > 0) {
    ...
    protected async clearCachedInterpreterIfInvalid(resource: Resource) {
        if (
            ...
            (await this.fs.fileExists(this.stateStore.value.path))
        ) {
            return;
        }
        sendTelemetryEvent(
    ...
    protected async cacheSelectedInterpreter(
        ...
        const interpreterPathInCache = this.stateStore.value
            ? this.stateStore.value.path
            : "";
        const updated = interpreterPath === interpreterPathInCache;
        sendTelemetryEvent(
            PYTHON_INTERPRETER_AUTO_SELECTION,
            {},
            { rule: this.ruleName, updated }
        );
    ...
    protected async next(
        ...
        return this.nextRule && manager
            ? this.nextRule.autoSelectInterpreter(resource, manager)
            : undefined;
    }
}
