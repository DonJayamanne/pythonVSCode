...
export class PythonPathUpdaterService
    implements IPythonPathUpdaterServiceManager {
    ...
    constructor(
        ...
        this.pythonPathSettingsUpdaterFactory = serviceContainer.get<
            IPythonPathUpdaterServiceFactory
        >(IPythonPathUpdaterServiceFactory);
        this.interpreterVersionService = serviceContainer.get<
            IInterpreterVersionService
        >(IInterpreterVersionService);
        this.executionFactory = serviceContainer.get<IPythonExecutionFactory>(
            IPythonExecutionFactory
        );
    }
    public async updatePythonPath(
        ...
        try {
            ...
            window.showErrorMessage(
                `Failed to set 'pythonPath'. Error: ${message}`
            );
    ...
    private async sendTelemetry(
        ...
        if (!failed) {
            const processService = await this.executionFactory.create({
                pythonPath
            });
            ...
            const pipVersionPromise = this.interpreterVersionService
                ...
                .catch<string>(() => "");
            const [info, pipVersion] = await Promise.all([
                infoPromise,
                pipVersionPromise
            ]);
