...
export function activateComponents(
    ...
    // const legacyActivationResult = activateLegacy(ext, startupStopWatch);
    activateLegacy(ext, startupStopWatch);
...
export function activateFeatures(_ext: ExtensionState, _components: Components): void {
...
function activateLegacy(ext: ExtensionState, startupStopWatch: StopWatch): ActivationResult {
    const { legacyIOC } = ext;
    ...
    void setExtensionInstallTelemetryProperties(fs);
    ...
    manager.activate(startupStopWatch);

    return { fullyReady: Promise.resolve() };
}
