...
function generateMarkdownCell(
    ...
    const markdown = appendLineFeed(
        code.slice(1).filter(s => s.includes("#")),
        s =>
            s
                .trim()
                .slice(1)
...
export function generateCells(
    ...
    if (RegExpValues.PythonMarkdownCellMarker.test(firstLine)) {
        ...
        // with #
        const firstNonMarkdown = splitMarkdown
            ? split.findIndex(
                  (l: string) =>
        ...
        if (firstNonMarkdown >= 0) {
            return [
                generateMarkdownCell(
                    split.slice(0, firstNonMarkdown),
                    file,
                    line
                ),
                generateCodeCell(
...
export function generateCellRanges(
    document: TextDocument
): { range: Range; title: string }[] {
