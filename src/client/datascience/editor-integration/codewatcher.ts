...
export class CodeWatcher implements ICodeWatcher {
    ...
    public async runCurrentCell() {
        ...
        for (const lens of this.codeLenses) {
            ...
            if (
                lens.range.contains(
                    this.documentManager.activeTextEditor.selection.start
                ) &&
                lens.command &&
                lens.command.command === Commands.RunCell
            ) {
    ...
    private handleError = (err: any) => {
        if (err instanceof JupyterInstallError) {
            ...
            this.applicationShell
                .showErrorMessage(
                    jupyterError.message,
                    jupyterError.actionTitle
                )
                .then(v => {
