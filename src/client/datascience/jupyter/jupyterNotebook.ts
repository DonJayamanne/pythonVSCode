...
import { CellState, ICell, IJupyterSession, INotebook, INotebookCompletion, INotebookExecutionLogger, INotebookServer, INotebookServerLaunchInfo, InterruptResult } from '../types';
...
export class JupyterNotebookBase implements INotebook {
        ...
        if (subscriber.isValid(this.sessionStartTime)) {
            ...
            if (this.launchInfo && this.launchInfo.connectionInfo && this.launchInfo.connectionInfo.localProcExitCode) {
                ...
                if (request) {
                    ...
                    request.onStdin = (msg: KernelMessage.IStdinMessage) => {
                        if (jupyterLab.KernelMessage.isInputRequestMsg(msg)) {
                            this.handleInputRequest(msg as KernelMessage.IInputRequestMsg, clearState, subscriber.cell, request).ignoreErrors();
                        }
                    };
                    // Listen to messages.
    ...
    private async handleInputRequest(msg: KernelMessage.IInputRequestMsg, _clearState: Map<string, boolean>, cell: ICell, request: Kernel.IFuture) {
        cell.data.execution_count = msg.content.execution_count;
        sleep(4000)
            .then(() => request.sendInputReply({ value: 'Hello Don' }))
            .ignoreErrors();
    }
