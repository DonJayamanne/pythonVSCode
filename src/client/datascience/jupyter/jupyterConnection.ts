...
class JupyterConnectionWaiter {
    ...
    constructor(
        ...
        if (cancelToken) {
            cancelToken.onCancellationRequested(() =>
                this.startPromise.reject(new CancellationError())
            );
        }
        ...
        if (urlMatch && !this.startPromise.completed) {
            ...
            try {
                ...
                this.rejectStartPromise(
                    localize.DataScience.jupyterLaunchNoURL()
                );
                return;
            }
    ...
    private resolveStartPromise = (baseUrl: string, token: string) => {
        ...
        this.startPromise.resolve(
            this.createConnection(baseUrl, token, this.launchResult)
        );
    };
...
export class JupyterConnection implements IConnection {
    ...
    public static waitForConnection(
        ...
        const waiter = new JupyterConnectionWaiter(
            ...
            notebookFile,
            getServerInfo,
            (baseUrl: string, token: string, processDisposable: Disposable) =>
                new JupyterConnection(baseUrl, token, processDisposable),
            serviceContainer,
            cancelToken
        );
