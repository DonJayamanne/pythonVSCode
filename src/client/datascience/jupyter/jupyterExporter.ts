...
export class JupyterExporter implements INotebookExporter {
    ...
    private addDirectoryChangeCell = async (
        cells: ICell[],
        file: string
    ): Promise<ICell[]> => {
        const changeDirectory = await this.calculateDirectoryChange(
            file,
            cells
        );

        if (changeDirectory) {
            const exportChangeDirectory = CodeSnippits.ChangeDirectory.join(
                os.EOL
            ).format(
    ...
    private firstWorkspaceFolder = async (
        cells: ICell[]
        ...
        for (const cell of cells) {
            ...
            if (
                ...
                (await this.fileSystem.fileExists(filename))
            ) {
    ...
    private calculateDirectoryChange = async (
        notebookFile: string,
        cells: ICell[]
    ): Promise<string | undefined> => {
    ...
    private pruneCells = (cells: ICell[]): nbformat.IBaseCell[] => {
        ...
        return (
            cells
                .filter(c => c.data.cell_type !== "sys_info")
    ...
    private pruneSource = (
        ...
        if (Array.isArray(source) && source.length > 0) {
            ...
            if (
                array.length > 0 &&
                RegExpValues.PythonCellMarker.test(array[0])
            ) {
    ...
    private extractPythonMainVersion = async (
        cells: ICell[]
    ): Promise<number> => {
        let pythonVersion;
        ...
        if (sysInfoCells.length > 0) {
            ...
            if (fullVersionString) {
                pythonVersion = fullVersionString.substr(
                    0,
                    fullVersionString.indexOf(".")
                );
        ...
        this.logger.logInformation(
            "Failed to find python main version from sys_info cell"
        );
