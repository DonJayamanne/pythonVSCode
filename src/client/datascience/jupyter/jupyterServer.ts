...
class CellSubscriber {
    ...
    constructor(
        cell: ICell,
        subscriber: Subscriber<ICell>,
        promiseComplete: (self: CellSubscriber) => void
    ) {
        this.cellRef = cell;
    ...
    private attemptToFinish() {
        if (
            ...
            (this.cell.state === CellState.finished ||
                this.cell.state === CellState.error)
        ) {
...
export class JupyterServer implements INotebookServer, IAsyncDisposable {
    ...
    private onStatusChangedEvent: vscode.EventEmitter<
        boolean
                    ...
                    request.done
                        .then(() => {
                            this.logger.logInformation(
                                `Execute for ${code} silently finished.`
                            );
                            resolve();
                        })
                        .catch(reject);
        ...
        if (this.session) {
            ...
            const finished =
                copyPending.length > 0
                    ? Promise.all(copyPending.map(d => d.promise))
                    : Promise.resolve([CellState.finished]);
            ...
            const restartHandlerToken = this.session.onRestarted(
                restartHandler
            );
            ...
            try {
                ...
                if (
                    ...
                    interruptBeginTime < this.sessionStartTime
                ) {
                    return InterruptResult.Restarted;
                }
    ...
    private initialNotebookSetup = (cancelToken?: CancellationToken) => {
        ...
        this.executeSilently(
            `%matplotlib inline${os.EOL}import matplotlib.pyplot as plt${
                darkTheme
                    ? `${os.EOL}from matplotlib import style${
                          os.EOL
                    ...
                    c => {
                        ...
                        if (array.length === args.length) {
                            ...
                            if (
                                array.every(
                                    a =>
                                        a.state === CellState.finished ||
                                        a.state === CellState.error
                                )
                            ) {
                                subscriber.complete();
                            }
    ...
    private changeDirectoryIfPossible = async (
        directory: string
    ): Promise<void> => {
        if (
            this.connInfo &&
    ...
    private handleCodeRequest = (subscriber: CellSubscriber) => {
        ...
        if (subscriber.isValid(this.sessionStartTime)) {
            const request = this.generateRequest(
                concatMultilineString(subscriber.cell.data.source),
                false
            );
            ...
            if (request) {
                    ...
                    try {
                        if (jupyterLab.KernelMessage.isExecuteResultMsg(msg)) {
                            ...
                            this.logger.logWarning(
                                `Unknown message ${
                                    msg.header.msg_type
                                } : hasData=${"data" in msg.content}`
                            );
                        }

                        // Set execution count, all messages should have it
                        if (msg.content.execution_count) {
                            subscriber.cell.data.execution_count = msg.content
                                .execution_count as number;
                        }
                ...
                subscriber.error(
                    this.sessionStartTime,
                    new Error(localize.DataScience.sessionDisposed())
                );
            }
        } else {
    ...
    private executeCodeObservable(cell: ICell): Observable<ICell> {
        return new Observable<ICell>(subscriber => {
            ...
            const cellSubscriber = new CellSubscriber(
                cell,
                subscriber,
                (self: CellSubscriber) => {
                    this.pendingCellSubscriptions = this.pendingCellSubscriptions.filter(
                        p => p !== self
                    );
                }
            );
