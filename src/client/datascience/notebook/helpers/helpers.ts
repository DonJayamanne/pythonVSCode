...
import { getCellCodeForMagicCells, getCellCodeLanguageAndUpdateMetadata } from './cellMagicHelper';
...
export function notebookModelToVSCNotebookData(
    ...
    const pythonNb = isPythonNotebook(metadata);
    const cells = nbCells
        .map((cell) => createVSCNotebookCellDataFromCell(pythonNb, isNotebookTrusted, preferredLanguage, cell))
        .filter((item) => !!item)
        .map((item) => item!);
...
export function createJupyterCellFromVSCNotebookCell(
    ...
    if (vscCell.cellKind === vscodeNotebookEnums.CellKind.Markdown) {
        ...
        cell = createCodeCellFromNotebookCell(vscCell, isPythonNb);
    }
...
function createNotebookCellDataFromCodeCell(
    ...
    // Special casing for treating cells with magics as cells with a specific language type (other than python).
    if (isPythonNb && cellLanguage === PYTHON_LANGUAGE) {
        const info = getCellCodeLanguageAndUpdateMetadata(cell, notebookCellMetadata);
        source = info.source;
        cellLanguage = info.language;
    }
...
function createCodeCellFromNotebookCell(cell: NotebookCell, isPythonNb: boolean): nbformat.ICodeCell {
    const metadata = cell.metadata.custom?.metadata || {};
    const code = isPythonNb ? getCellCodeForMagicCells(cell) : cell.document.getText();
    return {
        cell_type: 'code',
