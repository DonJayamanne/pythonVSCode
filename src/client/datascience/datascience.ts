...
export class DataScience implements IDataScience {
    ...
    constructor(
        ...
        this.commandListeners = this.serviceContainer.getAll<
            IDataScienceCommandListener
        >(IDataScienceCommandListener);
        this.dataScienceSurveyBanner = this.serviceContainer.get<
            IPythonExtensionBanner
        >(IPythonExtensionBanner, BANNER_NAME_DS_SURVEY);
    }
    ...
    public async selectJupyterURI(): Promise<void> {
        const quickPickOptions = [
            localize.DataScience.jupyterSelectURILaunchLocal(),
            localize.DataScience.jupyterSelectURISpecifyURI()
        ];
    ...
    private async selectJupyterLaunchURI(): Promise<void> {
        ...
        const userURI = await this.appShell.showInputBox({
            ...
            validateInput: this.validateURI,
            ignoreFocusOut: true
        });
    ...
    private onSettingsChanged = () => {
        ...
        const editorContext = new ContextKey(
            EditorContexts.DataScienceEnabled,
            this.commandManager
        );
    ...
    private getCurrentCodeWatcher(): ICodeWatcher | undefined {
        ...
        return this.dataScienceCodeLensProvider.getCodeWatcher(
    ...
    private registerCommands(): void {
        ...
        this.disposableRegistry.push(disposable);
        this.commandListeners.forEach(
            (listener: IDataScienceCommandListener) => {
