...
export class LineFormatter {
    ...
    private tokens: ITextRangeCollection<IToken> = new TextRangeCollection<
        IToken
    >([]);
    ...
    public formatLine(document: TextDocument, lineNumber: number): string {
        ...
        for (let i = 0; i < this.tokens.count; i += 1) {
            ...
            const next =
                i < this.tokens.count - 1
                    ? this.tokens.getItemAt(i + 1)
                    : undefined;
            ...
            switch (t.type) {
                ...
                case TokenType.Comma:
                    ...
                    if (
                        next &&
                        !this.isCloseBraceType(next.type) &&
                ...
                case TokenType.Identifier:
                    ...
                    if (
                        this.isKeywordWithSpaceBeforeBrace(id) &&
                        next &&
                        this.isOpenBraceType(next.type)
                    ) {
                        // for x in ()
                        this.builder.softAppendSpace();
                    }
                ...
                case TokenType.Colon:
                    ...
                    this.builder.append(":");
                    if (
                        !this.braceCounter.isOpened(TokenType.OpenBracket) &&
                        (next && next.type !== TokenType.Colon)
                    ) {
    ...
    private handleOperator(index: number): void {
        ...
        if (
            prev &&
            (this.isOpenBraceType(prev.type) || prev.type === TokenType.Comma)
        ) {
            this.builder.append(this.text.substring(t.start, t.end));
            return;
        }
    ...
    private handleStarOperator(current: IToken, prev: IToken): boolean {
        ...
        const lastToken =
            lastLine && lastLine.count > 0
                ? lastLine.getItemAt(lastLine.count - 1)
                : undefined;
        if (
            lastToken &&
            (this.isOpenBraceType(lastToken.type) ||
                lastToken.type === TokenType.Comma)
        ) {
            this.builder.append(
                this.text.substring(current.start, current.end)
            );
            return true;
        }
        return false;
    ...
    private handleEqual(t: IToken, index: number): void {
        if (
            this.isMultipleStatements(index) &&
            !this.braceCounter.isOpened(TokenType.OpenBrace)
        ) {
            // x = 1; x, y = y, x
            this.builder.softAppendSpace();
            this.builder.append("=");
    ...
    private handleOther(t: IToken, index: number): void {
        ...
        if (
            prev &&
            (this.isOpenBraceType(prev.type) || prev.type === TokenType.Colon)
        ) {
    ...
    private handleUnknown(t: IToken): void {
        ...
        const nextChar =
            t.end < this.text.length - 1 ? this.text.charCodeAt(t.end) : 0;
    ...
    private isEqualsInsideArguments(index: number): boolean {
        ...
        if (
            index > 1 &&
            this.tokens.getItemAt(index - 2).type === TokenType.Colon
        ) {
            return false; // Type hint should have spaces around like foo(x: int = 1) per PEP 8
        }

        return this.isInsideFunctionArguments(
            this.tokens.getItemAt(index).start
        );
    }
    ...
    private isInsideFunctionArguments(position: number): boolean {
        ...
        for (let i = this.lineNumber; i >= 0; i -= 1) {
            ...
            if (lineTokens.count > 0 && i < this.lineNumber) {
                ...
                if (last.type === TokenType.Colon) {
                    ...
                    const beforeLast = lineTokens.getItemAt(
                        lineTokens.count - 2
                    );
                    if (
                        beforeLast.type === TokenType.Colon &&
                        last.type === TokenType.Comment
                    ) {
                        start = this.document.lineAt(i + 1).range.start;
                        break;
                    }
        ...
        const text = this.document.getText(
            new Range(start, currentLine.range.end)
        );
        ...
        position =
            this.document.offsetAt(currentLine.range.start) +
            position -
        ...
        for (let i = 0; i < tokens.count - 1; i += 1) {
            ...
            if (t.type === TokenType.Identifier) {
                ...
                if (
                    ...
                    !this.isKeywordWithSpaceBeforeBrace(
                        text.substr(t.start, t.length)
                    )
                ) {
                    ...
                    if (
                        position > next.start &&
                        position < tokens.getItemAt(closeBraceIndex).start
                    ) {
        ...
        if (funcCallStartIndex < 0) {
            ...
            for (let i = 0; i < tokens.count; i += 1) {
                ...
                if (
                    t.type === TokenType.Identifier &&
                    text.substr(t.start, t.length) === "lambda"
                ) {
                    if (position < t.start) {
                    ...
                    if (
                        position > t.start &&
                        position < tokens.getItemAt(colonIndex).start
                    ) {
                        funcCallStartIndex = i;
    ...
    private findNearestColon(
        tokens: ITextRangeCollection<IToken>,
        index: number
    ): number {
