...
export class AutoPep8Formatter extends BaseFormatter {
    ...
    public formatDocument(
        ...
        const hasCustomArgs =
            Array.isArray(settings.formatting.autopep8Args) &&
            settings.formatting.autopep8Args.length > 0;
        ...
        if (formatSelection) {
            ...
            autoPep8Args.push(
                ...[
                    ...
                    (range!.end.line + 1).toString()
                ]
            );
        }
        const promise = super.provideDocumentFormattingEdits(
            document,
            options,
            token,
            autoPep8Args
        );
        sendTelemetryWhenDone(FORMAT, promise, stopWatch, {
            tool: "autopep8",
            hasCustomArgs,
            formatSelection
        });
