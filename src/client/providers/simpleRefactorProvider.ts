...
// tslint:disable-next-line:no-any
function validateDocumentForRefactor(
    textEditor: vscode.TextEditor
): Promise<any> {
    ...
    return new Promise<any>((resolve, reject) => {
        vscode.window
            .showInformationMessage(
                "Please save changes before refactoring",
                "Save"
            )
            .then(item => {
                if (item === "Save") {
    ...
    return renameResponse
        ...
        .then(done => {
            if (done && changeStartsAtLine >= 0) {
                ...
                for (
                    let lineNumber = changeStartsAtLine;
                    ...
                    if (indexOfWord >= 0) {
                        newWordPosition = new vscode.Position(
                ...
                if (newWordPosition) {
                    textEditor.selections = [
                        new vscode.Selection(
                            newWordPosition,
                            new vscode.Position(
                                newWordPosition.line,
                                newWordPosition.character + newName.length
                            )
                        )
                    ];
        ...
        .catch(error => {
            if (error === "Not installed") {
                installer
                    ...
                    .catch(ex =>
                        console.error(
                            "Python Extension: simpleRefactorProvider.promptToInstall",
                            ex
                        )
                    );
            ...
            outputChannel.appendLine(
                `${"#".repeat(10)}Refactor Output${"#".repeat(10)}`
            );
            outputChannel.appendLine(`Error in refactoring:\n${errorMessage}`);
            vscode.window.showErrorMessage(
                `Cannot perform refactoring using selected element(s). (${errorMessage})`
            );
