...
export class ItemInfoSource implements IItemInfoSource {
    ...
    public async getItemInfoFromText(
        ...
        const result = await this.getHoverResultFromTextRange(
            documentUri,
            fileName,
            range,
            sourceText,
    ...
    public async getItemInfoFromDocument(
        ...
        const result = await this.getHoverResultFromDocument(
            document,
            position,
            token
        );
    ...
    private async getHoverResultFromDocument(
        ...
        if (
            ...
            document.lineAt(position.line).text.match(/^\s*\/\//)
        ) {
            return;
        }
        const range = document.getWordRangeAtPosition(position);
    ...
    private async getHoverResultFromDocumentRange(
        ...
        return this.jediFactory
            .getJediProxyHandler<proxy.IHoverResult>(document.uri)
    ...
    private async getHoverResultFromTextRange(
        ...
        return this.jediFactory
            .getJediProxyHandler<proxy.IHoverResult>(documentUri)
            .sendCommand(cmd, token);
    }

    private getItemInfoFromHoverResult(
        data: proxy.IHoverResult,
        currentWord: string
    ): LanguageItemInfo[] {
        ...
        data.items.forEach(item => {
            ...
            if (item.docstring) {
                ...
                if (
                    lines.length > 0 &&
                    item.signature.indexOf(lines[0]) === 0
                ) {
                    lines.shift();
                    const endIndex = lines.findIndex(line =>
                        item.signature.endsWith(line)
                    );
                    if (endIndex >= 0) {
                ...
                const description = this.textConverter.toMarkdown(
                    lines.join(EOL)
                );
                tooltip = tooltip.appendMarkdown(description);

                infos.push(
                    new LanguageItemInfo(
                        tooltip,
                        item.description,
                        new vscode.MarkdownString(signature)
                    )
                );
                return;
            }
