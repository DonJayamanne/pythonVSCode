...
export class PythonObjectDefinitionProvider {
    ...
    public constructor(jediFactory: JediFactory) {
        this._defProvider = new defProvider.PythonDefinitionProvider(
            jediFactory
        );
    }
    ...
    public async goToObjectDefinition() {
        ...
        let defs = await this._defProvider.provideDefinition(
            doc,
            range.start,
            tokenSource.token
        );
        ...
        if (defs === null) {
            await vscode.window.showInformationMessage(
                `Definition not found for '${pathDef}'`
            );
            return;
        }
