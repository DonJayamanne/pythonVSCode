...
export class PythonCompletionItemProvider
    ...
    constructor(jediFactory: JediFactory, serviceContainer: IServiceContainer) {
        this.completionSource = new CompletionSource(
            jediFactory,
            serviceContainer,
            new ItemInfoSource(jediFactory)
        );
    ...
    public async provideCompletionItems(
        ...
        const items = await this.completionSource.getVsCodeCompletionItems(
            document,
            position,
            token
        );
    ...
    public async resolveCompletionItem(
        ...
        if (!item.documentation) {
            const itemInfos = await this.completionSource.getDocumentation(
                item,
                token
            );
