...
export class TerminalProvider implements Disposable {
    ...
    private async onCreateTerminal() {
        const terminalService = this.serviceContainer.get<
    ...
    private getActiveResource(): Uri | undefined {
        ...
        if (
            ...
            !documentManager.activeTextEditor.document.isUntitled
        ) {
            return documentManager.activeTextEditor.document.uri;
        }
        const workspace = this.serviceContainer.get<IWorkspaceService>(
            IWorkspaceService
        );
