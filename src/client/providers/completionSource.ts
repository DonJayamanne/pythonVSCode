...
export class CompletionSource {
    ...
    public async getVsCodeCompletionItems(
        ...
        const result = await this.getCompletionResult(
            document,
            position,
            token
        );
        ...
        return this.toVsCodeCompletions(
            new DocumentPosition(document, position),
            result,
            document.uri
        );
    }
    ...
    public async getDocumentation(
        ...
        const range = new vscode.Range(
            leadingRange.end,
            leadingRange.end.translate(0, itemString.length)
        );

        return this.itemInfoSource.getItemInfoFromText(
            document.uri,
            document.fileName,
            range,
            sourceText,
    ...
    private async getCompletionResult(
        ...
        return this.jediFactory
            .getJediProxyHandler<proxy.ICompletionResult>(document.uri)
    ...
    private toVsCodeCompletions(
        ...
        return data && data.items.length > 0
            ? data.items.map(item =>
                  this.toVsCodeCompletion(documentPosition, item, resource)
              )
            : [];
    }
    ...
    private toVsCodeCompletion(
        ...
        completionItem.sortText =
            (completionItem.label.startsWith("__")
                ? "z"
                : completionItem.label.startsWith("_")
                    ? "y"
                    : "__") + completionItem.label;
