...
export class RefactorProxy extends Disposable {
    ...
    public extractMethod<T>(
        ...
        if (
            ...
            return Promise.reject<T>(
                "Missing blank line at the end of document (PEP8)."
            );
        }
        const command = {
    ...
    private handleStdError(data: string) {
        ...
        let dataStr = (this._previousStdErrData =
            this._previousStdErrData + data + "");
        let errorResponse: {
            message: string;
            traceback: string;
            type: string;
        ...
        if (
            typeof errorResponse[0].message !== "string" ||
            errorResponse[0].message.length === 0
        ) {
            errorResponse[0].message = errorResponse[0].traceback
                .splitLines()
                .pop()!;
        }
        let errorMessage =
        ...
        if (this._startedSuccessfully) {
            ...
            if (
                typeof errorResponse[0].type === "string" &&
                errorResponse[0].type === "ModuleNotFoundError"
            ) {
    ...
    private onData(data: string) {
        ...
        // Possible there was an exception in parsing the data returned
        // So append the data then parse it
        let dataStr = (this._previousOutData =
            this._previousOutData + data + "");
        let response: any;
        try {
            response = dataStr
                .split(/\r?\n/g)
