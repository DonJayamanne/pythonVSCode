...
import { Breakpoint, BreakpointsChangeEvent, debug, DebugConfiguration, DebugConsole, DebugSession, DebugSessionCustomEvent, Event, WorkspaceFolder } from 'vscode';
import { Disposable } from 'vscode-jsonrpc';
import { IDebugManager } from './types';

@injectable()
export class DebugManager implements IDebugManager {
    public static instance = new DebugManager();
    public get activeDebugConsole(): DebugConsole {
        return debug.activeDebugConsole;
    }
    public get activeDebugSession(): DebugSession {
        return debug.activeDebugSession;
    }
    public get breakpoints(): Breakpoint[] {
        return debug.breakpoints;
    }
    public get onDidChangeActiveDebugSession(): Event<DebugSession> {
    ...
    public startDebugging(folder: WorkspaceFolder, nameOrConfiguration: string | DebugConfiguration): Thenable<boolean> {
        return this.startDebugging(folder, nameOrConfiguration);
    }
