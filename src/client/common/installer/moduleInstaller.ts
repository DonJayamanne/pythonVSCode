...
export abstract class ModuleInstaller {
    ...
    public async installModule(
        ...
        if (executionInfo.moduleName) {
            const configService = this.serviceContainer.get<
                IConfigurationService
            >(IConfigurationService);
            const settings = configService.getSettings(resource);
            const args = ["-m", executionInfo.moduleName].concat(
                executionInfoArgs
            );
            ...
            if (
                ...
                await terminalService.sendCommand(
                    pythonPath,
                    args.concat(["--user"])
                );
            }
    ...
    private async processInstallArgs(
        ...
        if (
            currentInterpreter &&
            currentInterpreter.version &&
            currentInterpreter.version.major === 2
        ) {
            const newArgs = [...args];
    ...
    private elevatedInstall(execPath: string, args: string[]) {
        ...
        const outputChannel = this.serviceContainer.get<vscode.OutputChannel>(
            IOutputChannel,
