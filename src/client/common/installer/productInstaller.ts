...
const CTagsInsllationScript =
    os.platform() === "darwin"
        ? "brew install ctags"
        : "sudo apt-get install exuberant-ctags";

export abstract class BaseInstaller {
    private static readonly PromptPromises = new Map<
        string,
    ...
    public async install(
        ...
        const moduleName = translateProductToModule(
            product,
            ModuleNamePurpose.install
        );
        const logger = this.serviceContainer.get<ILogger>(ILogger);
        await installer
            .installModule(moduleName, resource)
            .catch(
                logger.logError.bind(
                    logger,
                    `Error in installing the module '${moduleName}'`
                )
            );

        return this.isInstalled(product, resource).then(
            isInstalled =>
                isInstalled
    ...
    protected getExecutableNameFromSettings(
        ...
        const productPathService = this.serviceContainer.get<
            IProductPathService
        >(IProductPathService, productType);
        ...
        if (
            ...
            terminalService
                ...
                .catch(
                    logger.logError.bind(
                        logger,
                        `Failed to install ctags. Script sent '${CTagsInsllationScript}'.`
                    )
                );
        }
        return InstallerResponse.Ignore;
...
export class LinterInstaller extends BaseInstaller {
    protected async promptToInstallImplementation(
        ...
        if (
            isPylint &&
            this.getStoredResponse(disableLinterInstallPromptKey) === true
        ) {
            return InstallerResponse.Ignore;
        }

        const options = isPylint
            ? [selectLinter, disableInstallPrompt]
            : [selectLinter];
        ...
        if (response === install) {
            sendTelemetryEvent(LINTER_NOT_INSTALLED_PROMPT, undefined, {
     ...
     * @returns Boolean: The current state of the stored response key given.
     */
    private async setStoredResponse(
        key: string,
        value: boolean
    ): Promise<void> {
        const factory = this.serviceContainer.get<IPersistentStateFactory>(
...
export class ProductInstaller implements IInstaller {
    ...
    public translateProductToModuleName(
        product: Product,
    ...
    private createInstaller(product: Product): BaseInstaller {
        ...
        switch (productType) {
            case ProductType.Formatter:
                return new FormatterInstaller(
                    this.serviceContainer,
                    this.outputChannel
                );
            case ProductType.Linter:
                return new LinterInstaller(
                    this.serviceContainer,
                    this.outputChannel
                );
            case ProductType.WorkspaceSymbols:
                return new CTagsInstaller(
                    this.serviceContainer,
                    this.outputChannel
                );
            case ProductType.TestFramework:
                return new TestFrameworkInstaller(
                    this.serviceContainer,
                    this.outputChannel
                );
            case ProductType.RefactoringLibrary:
                return new RefactoringLibraryInstaller(
                    this.serviceContainer,
                    this.outputChannel
                );
            default:
...
function translateProductToModule(
    product: Product,
    purpose: ModuleNamePurpose
): string {
    switch (product) {
