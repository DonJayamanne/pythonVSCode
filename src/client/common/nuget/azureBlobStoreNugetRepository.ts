...
export class AzureBlobStoreNugetRepository implements INugetRepository {
    ...
    protected async listPackages(
        ...
        const blobStore = az.createBlobServiceAnonymous(
        ...
        return new Promise<NugetPackage[]>((resolve, reject) => {
            ...
            blobStore.listBlobsSegmentedWithPrefix(
                ...
                (error, result) => {
                    ...
                    resolve(
                        result.entries.map(item => {
                            return {
                                package: item.name,
                                uri: `${azureCDNBlobStorageAccount}/${azureBlobStorageContainer}/${
                                    item.name
                                }`,
                                version: nugetService.getVersionFromPackageFileName(
                                    item.name
                                )
                            };
                        })
