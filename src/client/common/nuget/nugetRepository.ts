...
export class NugetRepository implements INugetRepository {
    ...
    public async getPackages(packageName: string): Promise<NugetPackage[]> {
        const versions = await this.getVersions(
            nugetPackageBaseAddress,
            packageName
        );
        return versions.map(version => {
            const uri = this.getNugetPackageUri(
                nugetPackageBaseAddress,
                packageName,
                version
            );
    ...
    public async getVersions(
        ...
        const uri = `${packageBaseAddress}/${packageName
            .toLowerCase()
            .trim()}/index.json`;
    ...
    public getNugetPackageUri(
        packageBaseAddress: string,
        packageName: string,
        version: SemVer
    ): string {
        return `${packageBaseAddress}/${packageName}/${
            version.raw
        }/${packageName}.${version.raw}.nupkg`;
    }
}
