...
export class DotNetCompatibilityService implements IDotNetCompatibilityService {
    private readonly mappedServices = new Map<
        OSType,
        IDotNetCompatibilityService
    >();
    constructor(
        @inject(IOSDotNetCompatibilityService)
        @named(OSType.Unknown)
        unknownOsService: IOSDotNetCompatibilityService,
        @inject(IOSDotNetCompatibilityService)
        @named(OSType.OSX)
        macService: IOSDotNetCompatibilityService,
        @inject(IOSDotNetCompatibilityService)
        @named(OSType.Windows)
        winService: IOSDotNetCompatibilityService,
        @inject(IOSDotNetCompatibilityService)
        @named(OSType.Linux)
        linuxService: IOSDotNetCompatibilityService,
    ...
    public isSupported() {
        return this.mappedServices
            .get(this.platformService.osType)!
            .isSupported();
    }
}
