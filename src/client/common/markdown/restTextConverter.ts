...
export class RestTextConverter {
    ...
    private inPreformattedState(lines: string[], i: number): number {
        ...
        if (
            line.length > 0 &&
            !isWhiteSpace(line.charCodeAt(0)) &&
        ...
        const prevLine =
            this.md.length > 0 ? this.md[this.md.length - 1] : undefined;
        if (
            line.length === 0 &&
            prevLine &&
            (prevLine.length === 0 || prevLine.startsWith("```"))
        ) {
    ...
    private tryRemovePrecedingEmptyLines(): void {
        while (
            this.md.length > 0 &&
            this.md[this.md.length - 1].trim().length === 0
        ) {
            this.md.pop();
        }
    }
