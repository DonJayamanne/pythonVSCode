...
export class PythonExecutionService implements IPythonExecutionService {
    ...
    public async getInterpreterInformation(): Promise<
        InterpreterInfomation | undefined
    > {
        ...
        try {
            const jsonValue = await this.procService
                .exec(this.pythonPath, [file], { mergeStdOutErr: true })
            ...
            const versionValue =
                json.versionInfo.length === 4
                    ? `${json.versionInfo.slice(0, 3).join(".")}-${
                          json.versionInfo[3]
                      }`
                    : json.versionInfo.join(".");
            return {
                architecture: json.is64Bit
                    ? Architecture.x64
                    : Architecture.x86,
            ...
            traceError(
                `Failed to get interpreter information for '${
                    this.pythonPath
                }'`,
                ex
            );
    ...
    public async getExecutablePath(): Promise<string> {
        ...
        return this.procService
            .exec(this.pythonPath, ["-c", "import sys;print(sys.executable)"], {
                throwOnStdErr: true
            })
            .then(output => output.stdout.trim());
    }
    public async isModuleInstalled(moduleName: string): Promise<boolean> {
        return this.procService
            .exec(this.pythonPath, ["-c", `import ${moduleName}`], {
                throwOnStdErr: true
            })
            .then(() => true)
            .catch(() => false);
    ...
    public async execModule(
        ...
        const opts: SpawnOptions = { ...options };
        const result = await this.procService.exec(
            this.pythonPath,
            ["-m", moduleName, ...args],
            opts
        );
