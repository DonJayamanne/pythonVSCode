...
export class CommandPromptAndPowerShell extends BaseActivationCommandProvider {
    ...
    public async getActivationCommandsForInterpreter(
        ...
        const scriptFile = await this.findScriptFile(
            pythonPath,
            this.getScriptsInOrderOfPreference(targetShell)
        );
        if (!scriptFile) {
            return;
        }
        ...
        if (
            targetShell === TerminalShellType.commandPrompt &&
            scriptFile.endsWith("activate.bat")
        ) {
    ...
    private getScriptsInOrderOfPreference(
        targetShell: TerminalShellType
    ): string[] {
        const batchFiles = [
            "activate.bat",
