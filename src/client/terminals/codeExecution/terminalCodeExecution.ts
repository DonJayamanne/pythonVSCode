...
export class TerminalCodeExecutionProvider implements ICodeExecutionService {
    ...
    public async executeFile(file: Uri) {
        ...
        await this.getTerminalService(file).sendCommand(
            command,
            launchArgs.concat(file.fsPath.fileToCommandArgument())
        );
    }
    ...
    private async setCwdForFileExecution(file: Uri) {
        ...
        if (
            wkspace &&
            fileDirPath !== wkspace.uri.fsPath &&
