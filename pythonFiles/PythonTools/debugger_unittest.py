    ...
    def get_next_message(self, context_messag):
        try:
            ...
            raise AssertionError(
                'No message was written in 15 seconds. Error message:\n%s' %
                (context_messag, ))
        else:
...
class DebuggerRunner(object):
    ...
    def check_case(self, writer_thread_class):
        ...
        stdout = ret['stdout']
        stderr = ret['stderr']
        writer_thread.additional_output_checks(''.join(stdout),
                                               ''.join(stderr))
        return ret
    ...
    def create_process(self, args, writer_thread):
        process = subprocess.Popen(
            ...
            env=writer_thread.get_environ()
            if writer_thread is not None else None, )
        return process
        ...
        try:
                ...
                if process.poll() is not None:
                    ...
                    if writer_thread is not None:
                        if not writer_thread.isAlive():
                            ...
                            if not shown_intermediate and (
                                    time.time() - initial_time > 10):
                                print(
                                    'Warning: writer thread exited and process still did not (%.2fs seconds elapsed).'
                            ...
                            if time.time() - initial_time > 20:
                                ...
                                self.fail_with_message(
                                    "The other process should've exited but still didn't (%.2fs seconds timeout for process to exit)."
                                    % (time.time() - initial_time, ), stdout,
            ...
            if writer_thread is not None:
                if not writer_thread.FORCE_KILL_PROCESS_WHEN_FINISHED_OK:
                    ...
                    if poll < 0:
                        self.fail_with_message(
                            "The other process exited with error code: " +
                            str(poll), stdout, stderr, writer_thread)
                    ...
                    while 'TEST SUCEEDED' not in ''.join(stdout):
                        ...
                        if check == 50:
                            self.fail_with_message(
                                "TEST SUCEEDED not found in stdout.", stdout,
                                stderr, writer_thread)
                        time.sleep(.1)
...
class AbstractWriterThread(threading.Thread):
    ...
    def wait_for_breakpoint_hit(self,
                                reason='111',
                                get_line=False,
                                get_name=False):
        '''
        ...
        while not ('stop_reason="%s"' % reason) in last:
            last = self.reader_thread.get_next_message(
                'wait_for_breakpoint_hit. reason=%s' % (reason, ))
    ...
    def wait_for_custom_operation(self, expected):
        ...
        while not expected_encoded in last:
            last = self.reader_thread.get_next_message(
                'wait_for_custom_operation. Expected (encoded): %s' %
                (expected_encoded, ))
    ...
    def wait_for_multiple_vars(self, expected_vars):
        ...
        while True:
            try:
                last = self.reader_thread.get_next_message(
                    'wait_for_multiple_vars: %s' % (expected_vars, ))
            except:
                missing = []
                ...
                raise ValueError(
                    ...
                    % ('\n'.join(missing), len(missing), len(all_found),
                       len(expected_vars)))
            found = 0
    ...
    def write_add_breakpoint(self, line, func):
        ...
        self.write("111\t%s\t%s\t%s\t%s\t%s\t%s\tNone\tNone" %
                   (self.next_seq(), breakpoint_id, 'python-line',
                    self.get_main_filename(), line, func))
        self.log.append('write_add_breakpoint: %s line: %s func: %s' %
    ...
    def write_remove_breakpoint(self, breakpoint_id):
        self.write("112\t%s\t%s\t%s\t%s" %
                   (self.next_seq(), 'python-line', self.get_main_filename(),
                    breakpoint_id))

    def write_change_variable(self, thread_id, frame_id, varname, value):
        self.write("117\t%s\t%s\t%s\t%s\t%s\t%s" %
                   (self.next_seq(), thread_id, frame_id, 'FRAME', varname,
                    value))

    def write_get_frame(self, thread_id, frameId):
        self.write("114\t%s\t%s\t%s\tFRAME" % (self.next_seq(), thread_id,
                                               frameId))
        self.log.append('write_get_frame')

    def write_get_variable(self, thread_id, frameId, var_attrs):
        self.write("110\t%s\t%s\t%s\tFRAME\t%s" % (self.next_seq(), thread_id,
                                                   frameId, var_attrs))
    ...
    def write_set_next_statement(self, thread_id, line, func_name):
        self.write("%s\t%s\t%s\t%s\t%s" %
                   (CMD_SET_NEXT_STATEMENT, self.next_seq(), thread_id, line,
                    func_name, ))

    def write_debug_console_expression(self, locator):
        self.write("%s\t%s\t%s" % (CMD_EVALUATE_CONSOLE_EXPRESSION,
                                   self.next_seq(), locator))

    def write_custom_operation(self, locator, style, codeOrFile,
                               operation_fn_name):
        self.write("%s\t%s\t%s||%s\t%s\t%s" %
                   (CMD_RUN_CUSTOM_OPERATION, self.next_seq(), locator, style,
                    codeOrFile, operation_fn_name))

    def write_evaluate_expression(self, locator, expression):
        self.write("113\t%s\t%s\t%s\t1" % (self.next_seq(), locator,
                                           expression))
    ...
    def write_enable_dont_trace(self, enable):
        if enable:
            ...
            enable = 'false'
        self.write("%s\t%s\t%s" % (CMD_ENABLE_DONT_TRACE, self.next_seq(),
                                   enable))
...
def _get_debugger_test_file(filename):
    ...
    return os.path.normcase(
        rPath(os.path.join(os.path.dirname(__file__), filename)))
